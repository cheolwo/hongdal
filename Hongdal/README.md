...
현재 릴리즈 집중 대상도 `1.0`입니다. `1.5` 이후 기능은 방향을 잃지 않기 위해 문서와 플래그로 정리하되, 기본 구현 판단은 1.0 운송 흐름을 닫는지에 맞춥니다. 화주 의뢰, 기사 배차, 상차/하차 증빙, 수령자 인수, 결제/정산 기본에 직접 도움이 되지 않으면 기본 노출하지 않습니다.
...
| 화주 요금 정책 | 기본운임, km당 단가, 최소운임, 기사 지급 예정 운임, 알선 단계를 기록해 기준운임 미달과 재알선을 정책 이벤트로 판정합니다. |
| 알선소/재알선 방지 | 재알선금지 의뢰에서 2차 이상 알선 단계가 들어오면 `재알선차단필요` 이벤트로 의뢰 생성을 막습니다. |
...
코드에서는 `음식배달배차흐름Resolver`가 `배차대기.원본의뢰유형`을 해석합니다. 홍달마트 주문은 포장 완료 전에도 서버 사전 배차를 만들 수 있고, 실제 픽업은 포장 완료 후 픽업 가능 상태에서 진행합니다.
