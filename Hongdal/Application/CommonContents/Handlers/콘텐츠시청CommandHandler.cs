...
public sealed class 콘텐츠시청시작CommandHandler : IRequestHandler<콘텐츠시청시작Command, 콘텐츠시청시작Result?>
{
    ...
    public 콘텐츠시청시작CommandHandler(HongdalContext db, ICurrentUserAccessor currentUserAccessor)
    {
        _db = db;
...
public sealed class 콘텐츠시청진행CommandHandler : IRequestHandler<콘텐츠시청진행Command, bool>
{
    ...
    public 콘텐츠시청진행CommandHandler(HongdalContext db, ICurrentUserAccessor currentUserAccessor)
    {
        _db = db;
        _currentUserAccessor = currentUserAccessor;
    }
...
public sealed class 콘텐츠시청완료CommandHandler : IRequestHandler<콘텐츠시청완료Command, 콘텐츠시청완료Result?>
{
    ...
    public 콘텐츠시청완료CommandHandler(HongdalContext db, ICurrentUserAccessor currentUserAccessor)
    {
        _db = db;
        _currentUserAccessor = currentUserAccessor;
    }
    ...
    public async Task<콘텐츠시청완료Result?> Handle(콘텐츠시청완료Command request, CancellationToken cancellationToken)
    {
        ...
        var 정책 = 세션.콘텐츠.보상정책;
        if (정책 is null || 정책.보상유형 == 홍달보상유형.없음)
        {
            세션.완료여부 = true;
        ...
        var 할인금액 = 정책.할인금액;
        if (정책.보상유형 == 홍달보상유형.할인금액 && 정책.최대할인금액.HasValue)
        {
            할인금액 = Math.Min(정책.할인금액, 정책.최대할인금액.Value);
        }

        var 보상 = new 홍달콘텐츠보상지급
        {
            사용자Id = 사용자Id,
            콘텐츠Id = 세션.콘텐츠Id,
            보상유형 = 정책.보상유형,
