...
namespace Hongdal.Services.Notifications;

public interface IHongdalMobilePushInstallationService
{
    Task<HongdalMobilePushInstallationResponse> UpsertAsync(
        string userId,
...
public sealed class HongdalMobilePushInstallationService : IHongdalMobilePushInstallationService
{
    private readonly HongdalContext _db;

    public HongdalMobilePushInstallationService(HongdalContext db)
    {
        _db = db;
    }

    public async Task<HongdalMobilePushInstallationResponse> UpsertAsync(
        string userId,
        HongdalMobilePushInstallationUpsertRequest request,
        CancellationToken cancellationToken)
    {
        ...
        var installation = await _db.HongdalMobilePushInstallations
            .SingleOrDefaultAsync(
                x => x.AppKey == appKey && x.InstallationId == installationId,
                cancellationToken);
        if (installation is null)
        {
            installation = new HongdalMobilePushInstallation
            {
                AppKey = appKey,
                InstallationId = installationId,
                CreatedAtUtc = now
            };
            _db.HongdalMobilePushInstallations.Add(installation);
        }

        var duplicateTokens = await _db.HongdalMobilePushInstallations
    ...
    private static HongdalMobilePushInstallationResponse ToResponse(HongdalMobilePushInstallation installation)
        => new(
            installation.Id,
    ...
    private static string NormalizePlatform(string value)
        ...
        {
            "android" => HongdalMobilePlatforms.Android,
            "ios" => HongdalMobilePlatforms.Ios,
