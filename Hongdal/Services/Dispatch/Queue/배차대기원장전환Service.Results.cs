...
namespace 홍달.Services.Dispatch.Queue
{
    public sealed partial class 배차대기원장전환Service
    {
        ...
        private static 배차대기원장전환결과 대기상태아님(운송원장 queue, string? driverId = null)
            => 전환안됨(
                queue,
                배차대기원장전환결과코드.대기상태아님,
        ...
        private static 배차대기원장전환결과 전환됨(운송원장 queue, string code, string message, string? driverId = null)
            => 배차대기원장전환결과.전환됨(queue.의뢰Id, code, message, driverId ?? queue.현재추천대상기사Id ?? queue.확정기사Id);

        private static 배차대기원장전환결과 전환안됨(운송원장 queue, string code, string message, string? driverId = null)
