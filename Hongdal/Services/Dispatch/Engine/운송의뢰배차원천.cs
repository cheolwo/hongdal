...
public static class 운송의뢰배차원천유형
{
    ...
    public const string 홍달마트출고 = "HongdalMartOutboundCargo";
    ...
    public const string 홍달마트음식주문 = "MartFoodOrder";
    ...
    public static bool Is창고출고연계운송(string? sourceType)
        => IsAny(sourceType, 창고출고연계운송, 판매채널출고, 홍달마트출고, 화주운송의뢰, 주문자화물주문);
    ...
    public static bool Is음식배달운송(string? sourceType)
        => Is음식점주문(sourceType) || Is홍달마트음식주문(sourceType);

    public static bool Is음식점주문(string? sourceType)
        => IsAny(sourceType, 음식점주문, 음식주문);

    public static bool Is홍달마트음식주문(string? sourceType)
        => IsAny(sourceType, 홍달마트주문, 홍달마트음식주문, 홍달마트포장완료주문);
    ...
    private static 운송의뢰배차원천분류 분류화물용달(배차대기 queue)
    {
        ...
        if (운송의뢰배차원천유형.Is창고출고연계운송(queue.원본의뢰유형))
        {
            return new 운송의뢰배차원천분류(
                ...
                "창고 출고 연계 운송",
                "출고 예정 상품 운송",
                "화주 의뢰, 판매채널 출고, 창고 출고, 홍달마트 출고 화물",
                출고예정대상여부: true,
                창고선행작업필요: true);
        }
