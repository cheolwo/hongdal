...
public interface IOrdererGroupAutoAssignmentService
{
    주문자집단자동배정응답? Resolve(주문자회원가입요청 request);
    주문자집단자동배정응답? Resolve(주문자집단온보딩요청 request);
}

public sealed class OrdererGroupAutoAssignmentService : IOrdererGroupAutoAssignmentService
{
    private readonly IOrdererGroupScopeLookupService _scopeLookupService;

    public OrdererGroupAutoAssignmentService(IOrdererGroupScopeLookupService scopeLookupService)
    {
        _scopeLookupService = scopeLookupService;
    }

    public 주문자집단자동배정응답? Resolve(주문자회원가입요청 request)
        => Resolve(new OrdererGroupAutoAssignmentInput(
            request.RoadAddress,
    ...
    private 주문자집단자동배정응답? Resolve(OrdererGroupAutoAssignmentInput input)
    {
        ...
        var candidates = _scopeLookupService.FindCandidates(new OrdererGroupScopeLookupRequest
        {
    ...
    private static 주문자집단자동배정응답 ResolveApartmentScope(OrdererGroupAutoAssignmentInput input)
    {
    ...
    private sealed record OrdererGroupAutoAssignmentInput(
        string RoadAddress,
        string? JibunAddress,
