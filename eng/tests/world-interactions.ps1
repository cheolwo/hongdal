...
if ([string] $catalog.schemaVersion -ne "3" -or [string] $catalog.revision -ne "simulation-world-interactions.r8") {
    throw "WorldInteractionCatalogRevisionMustBeR8"
}
$natureIds = @(
...
if ($natureItems.Count -ne 12 -or (@($natureItems.id) -join ",") -ne ($natureIds -join ",")) {
...
if ((@($natureItems.actionCode) -join ",") -ne "RegionalThreatObservation,EmergencyRetreat,NatureRestoration,PartyRecovery,AcquireAxe,BeginHarvest,PlaceCabinBlueprint,BeginCabinBuild,EnterCabin,LeaveCabin,ResolveEncounter,CancelActiveWork") {
...
if (@(($e4Items + $e6Items).integration.e4SeedbedRefs | Select-Object -Unique).Count -ne 7) {
...
if ($check -notmatch "WorldInteractionCatalogValid:49") {
