...
if ($ledgerCheck -notmatch "TheorySemanticRelationsValid:H2=38;H3=20;AreaSets=4;WorldRelations=5") { throw "SemanticRelationLedgerCheckFailed" }
$ledger = Get-Content -LiteralPath $ledgerPath -Raw -Encoding UTF8 | ConvertFrom-Json
if (@($ledger.h2RelationRecipes).Count -ne 38 -or @($ledger.h3RelationRecipes).Count -ne 20 -or @($ledger.areaSetRelationRecipes).Count -ne 4) { throw "SemanticRelationRecipeCoverageInvalid" }
...
$shuffleRelative = $shufflePath.Substring($repositoryRoot.Length + 1).Replace("\", "/")
...
$malformedRelative = $malformedPath.Substring($repositoryRoot.Length + 1).Replace("\", "/")
...
if ($malformedExitCode -eq 0) { throw "MalformedSemanticContractUnexpectedSuccess" }
...
$unresolvedRelative = $unresolvedPath.Substring($repositoryRoot.Length + 1).Replace("\", "/")
$ErrorActionPreference = "Continue"
...
$unresolvedExitCode = $LASTEXITCODE
$ErrorActionPreference = $previousErrorActionPreference
if ($unresolvedExitCode -eq 0) { throw "SemanticRelationUnresolvedUnexpectedSuccess" }
...
Write-Output "TheorySemanticSpatialRelationTestsPassed:H2=38;H3=20;AreaSets=4;World=Qualified;OrderIndependent=True;NegativeFixtures=2"
