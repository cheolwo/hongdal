...
if ($check -notmatch "H2CompositionReadinessValid:H1=52;Used=51;H2=38/38;Theory=38;Authored=6;Derived=32;Unity=6;Review=6;Gameplay=12;TheoryPriority=28;ReviewPriority=5;StrictMissing=8;Warnings=16") { throw "H2ReadinessSummaryUnexpected:$check" }
...
if ([string] $readiness.theoryFactoryRevision -ne "simulation-world-theory-spatial-factory-output.r3") { throw "H2TheoryFactoryRevisionInvalid" }
...
if ([int] $readiness.counts.h2TheoryQualifiedCount -ne 38) { throw "H2TheoryQualifiedCountInvalid" }
if ([int] $readiness.counts.h2AuthoredTheoryRecipeCount -ne 6) { throw "H2AuthoredTheoryRecipeCountInvalid" }
if ([int] $readiness.counts.h2DerivedTheoryRecipeCount -ne 32) { throw "H2DerivedTheoryRecipeCountInvalid" }
...
if ([int] $readiness.counts.h2GameplayTheoryPriorityReadyCount -ne 28) { throw "H2GameplayTheoryPriorityReadyCountInvalid" }
...
if ([int] $readiness.counts.h2WarningOnlyGameplayTraceMissingCount -ne 16) { throw "H2WarningOnlyGameplayTraceCountInvalid" }
...
if ($warningTheoryReady.Count -ne 16) { throw "H2WarningTheoryReadyCountInvalid" }
