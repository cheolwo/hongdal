# 생활권 반품·폐기물 블록

@spatial-knowledge h2-candidate:town-returns-waste
@hierarchy H2
@state IdeaInventory
@required-h1 h1-stock:town-returns
@required-h1 h1-stock:town-waste
@required-h1 h1-stock:road-facility-access
@optional-h1 h1-stock:town-staff-rest
@connector CustomerReturnInput
@connector ServiceVehicleOutput

## 존재 이유

마트 반품과 폐기물 분류·차량 인계를 후방 서비스 동선으로 묶는 레시피다.

## 설계 상태

- 재고 상태: `IdeaInventory`
- 공간 계층: `H2`
- 실제 지역 권위: 없음

## 미해결

- 실제 Block 경계와 배치 방향은 현실 근거 적용 단계에서 결정한다.

이 문서는 상향식 공간 설계 지식이며 실제 좌표·AreaSet·LandscapeGraph·Unity 자산 권위를 만들지 않는다.
