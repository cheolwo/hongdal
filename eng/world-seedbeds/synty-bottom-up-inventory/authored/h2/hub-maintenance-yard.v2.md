# Hub 차량·시설 정비 블록

@spatial-knowledge h2-candidate:hub-maintenance-yard
@hierarchy H2
@state ExploratoryInventory
@required-h1 h1-stock:hub-vehicle-yard
@required-h1 h1-stock:hub-service-maintenance
@optional-h1 h1-stock:hub-temporary-staging
@connector VehicleInput
@connector MaintenanceLoop
@connector OperationsReturn

## 존재 이유

차량 상차·대기 공간과 시설 정비 공간을 연결해 물류 중단 뒤 복구 작업을 수용하는 City 단독 블록이다.

## 설계 상태

- 재고 상태: `ExploratoryInventory`
- 공간 계층: `H2`
- 실제 지역 권위: 없음

## 미해결

- 기준 크기·배치 방향과 연결구 조합은 설계 검토에서 확정한다.

이 문서는 상향식 공간 설계 지식이며 실제 좌표·AreaSet·LandscapeGraph·Unity 자산 권위를 만들지 않는다.
