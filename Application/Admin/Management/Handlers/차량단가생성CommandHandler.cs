...
public sealed class 차량단가생성CommandHandler : IRequestHandler<차량단가생성Command, 차량단가>
{
    ...
    public async Task<차량단가> Handle(차량단가생성Command request, CancellationToken cancellationToken)
    {
        var entity = new 차량단가
        {
            차량종류 = request.차량종류,
