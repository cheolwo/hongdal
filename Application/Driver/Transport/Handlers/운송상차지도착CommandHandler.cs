...
public sealed class 운송상차지도착CommandHandler : IRequestHandler<운송상차지도착Command, Result<기사운송상태변경응답>>
{
    ...
    public 운송상차지도착CommandHandler(HongdalContext db, I기사운송상태전이Service 상태전이Service, ILogger<운송상차지도착CommandHandler> logger)
    {
        _db = db;
    ...
    public async Task<Result<기사운송상태변경응답>> Handle(운송상차지도착Command request, CancellationToken cancellationToken)
    {
        ...
        var 상태변경 = _상태전이Service.상태변경(entity, "상차지도착", now);
        if (상태변경.IsFailed)
        {
