...
| 검증 | 상태 | 근거 또는 제한 |
| --- | --- | --- |
| `Ssalddel.Unity.Tests` | 72/72 통과 | 2026-08-08 headless .NET test; 공공 관측·커뮤니티 광장·창고 World mapping과 증분 갱신·실패 유지 정책 포함 |
...
| Urban Logistics Center sample compile | 통과 | Unity 6 + VContainer에서 Role/NPC sample assembly compile 확인 |
| Urban Logistics Center scene wiring | 통과 | Editor builder 생성 후 scene reload에서 Role target·waypoint·NPC wiring 확인 |
...
P0의 저장소 내 구현 가능 범위와 P1 공공데이터 정보관, P2 커뮤니티·시장 광장, P3 창고·재고의 server API→Unity primitive 코드 경로가 완료됐다. 실제 제품 Unity 프로젝트 runtime이 준비되기 전 다음 코드 우선순위는 P4 운송 World 연결과 TruckView다.
...
6. 물류센터 밖 transport corridor와 TruckView
7. 도심마트 operational aggregate와 주문자 관점
8. 주거공동체 공동수령 Role Perspective
9. Farm·Plot·Cultivation·Sensor canonical server contract와 생산자 관점
10. 협동조합·공동원장 공간 뒤 Synty 최소 팩 검증
