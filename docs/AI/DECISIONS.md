...
## 2026-09-05 스토리 영감과 진행 권위 분리 — Accepted

사용자 요청에 따라 [스토리 영감과 플레이 진행 분리](../Architecture/스토리영감과플레이진행분리.md) `story-inspiration.r1`을 공통 기준으로 채택한다. 괘·효는 선택적 영감·검색 메타데이터이며 제작 순서·사건 수·플레이 해금 권위가 아니다. 과거 괘→육효 고정 제작·실행 규칙은 이 범위에서 대체하며 기존 D 번호·PLAN·HEX·API·저장 식별자와 구체 사건의 승인 이력은 보존한다. 신규 캠페인 단계 수는 이야기에서 정하고 기존 6단계 저장은 호환한다.
