...
- 상태: `ConfirmedGamePurpose / ScopedContributionModelDefined / ExactContributionProfilesPending` (2026-09-02). 게임의 상위 목적은 플레이어인 나와 내가 대하는 상대의 필요한 오행 관계가 실제 행동·결과·회복·귀환으로 이어지게 하고, 그 순환 개선이 권위 결과로 확인될 때 관련 회복역량을 높여 개인과 공동체가 광복기(회복 우세기)를 맞이할 가능성을 넓히는 것이다.
...
- `회복역량`은 새 단일 권위 수치가 아니다. 사람은 개인 회복·위협·관계·숙련, 공동체는 별도 관점별 회복, 사물·시설은 내구·기능·용량·복구 가능성, 환경은 생태·경로·안전·생산 회복성처럼 기존 상태를 통해 구체화한다. 적대 상황의 순환 회복도 상대를 무조건 강화하는 것이 아니라 피해 중단·격리·분리·퇴로 확보·생태 관계 복구처럼 맥락에 맞는 결과를 뜻할 수 있다.
