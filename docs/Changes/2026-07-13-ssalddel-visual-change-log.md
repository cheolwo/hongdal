...
검증용 원본은 `artifacts/page-capture-check`와 `artifacts/android-render-check`에 남기고, 문서에 오래 보관할 대표 PNG만 `docs/assets/changes/2026-07-13-ssalddel-visual-summary`에 선별했습니다. 사용자 정보, 실제 결제 정보, 실제 배송 주소는 포함하지 않습니다.
...
| `8c61319b` | 역할 기반 통합 살뜰 앱과 다이어그램 작업공간 | 있음 | 통합 홈, 역할 전환, 후천 사방 이동, 모바일 다이어그램, 꾸미기 FakePG |
| `1736c126` | 웹 다이어그램 팔레트의 원장 중심 재구성 | 있음 | 원장별 노드 묶음과 살뜰 1.0 캔버스 |
| `d1098d4f` | 웹 홈 실시간 베스트 | 있음 | 업무 홈 하단 실시간 베스트 목록 |
| `a31bec92` | 이벤트·원장·교육·음성화 설계 기록 | 없음 | 설계 문서 변경 |
| `3228236e` | 통합 살뜰 앱 화면 흐름과 캡처 정리 | 문서 자산 | 페이지 문서와 장기 보관용 화면 캡처 |
| `2251656` | 살뜰 1.0 커뮤니티 중심 제품 원칙 | 없음 | 제품·운영 정책 문서 변경, 기존 통합 커뮤니티 홈 캡처 유지 |

## 역할 기반 통합 살뜰 앱

`8c61319b`에서는 화주 앱이라는 한정된 이름 대신 `Ssalddel`을 표시하고, 어드민을 제외한 여러 역할을 같은 클라이언트에서 전환하는 구조를 추가했습니다. 기존 커뮤니티 홈, 게시판, 글쓰기, 업무 진입은 유지하면서 역할 전환과 다이어그램 진입을 결합했습니다.

![역할 기반 통합 살뜰 커뮤니티 홈](../assets/changes/2026-07-13-ssalddel-visual-summary/unified-community-home.png)

살뜰 생활 게시판은 커뮤니티 글을 추천, 분류, 제목, 글쓴이, 추천 수, 댓글 수로 빠르게 훑는 게시판형 목록입니다. README 첫 화면에서는 업무 구조 설명보다 이 게시판 화면을 먼저 보여 주고, 데스크톱과 모바일 캡처를 함께 둡니다.

![살뜰 생활 게시판](../assets/changes/2026-07-13-ssalddel-visual-summary/community-board-desktop.png)

![모바일 살뜰 생활 게시판](../assets/changes/2026-07-13-ssalddel-visual-summary/community-board-mobile.png)

역할 패널에서는 화주, 기사, 창고 관리자처럼 현재 사용할 업무 관점을 선택합니다. 역할이 바뀌면 홈과 내비게이션, 업무 진입점이 같은 앱 안에서 함께 바뀝니다.

![통합 살뜰 앱 역할 전환 패널](../assets/changes/2026-07-13-ssalddel-visual-summary/role-switch-panel.png)

후천 사방 이동판은 역할별 주요 업무 방향을 화면 오른쪽 하단에서 펼쳐 선택하는 보조 내비게이션입니다. 가운데는 공통 태극 중심으로 커뮤니티 글 목록과 꾸미기 상점을 나누고, 바깥 사방은 역할별 업무 방향을 고릅니다.

![태극 중심 후천 사방 이동판](../assets/changes/2026-07-13-ssalddel-visual-summary/bagua-navigation-taegeuk-open.png)

## 모바일 원장 다이어그램

모바일 다이어그램은 운송 의뢰, 상차, 하차, 결제·정산처럼 묶을 수 있는 절차를 세로 흐름으로 표시합니다. 각 노드는 입력 준비도, API 연결 수, 상세 진입과 연결 동작을 함께 제공합니다.

![모바일 원장 다이어그램](../assets/changes/2026-07-13-ssalddel-visual-summary/mobile-ledger-diagram.png)
...
![꾸미기 FakePG 결제 완료](../assets/changes/2026-07-13-ssalddel-visual-summary/decoration-fakepg-complete.png)
...
![원장 중심 웹 다이어그램 팔레트](../assets/changes/2026-07-13-ssalddel-visual-summary/ledger-centered-diagram-palette.png)

캡처의 중앙 캔버스에는 살뜰 1.0의 운송 의뢰, 상차, 하차, 결제·정산 네 노드가 표시됩니다. 왼쪽 팔레트와 중앙 캔버스가 같은 원장 템플릿 키를 사용하므로, 메뉴 선택과 다이어그램 문맥을 함께 추적할 수 있습니다.
...
![웹 홈 실시간 베스트](../assets/changes/2026-07-13-ssalddel-visual-summary/realtime-best-web-home.png)
...
`2251656`은 살뜰 1.0의 제품 중심을 유상 화물 배차·주선이 아니라 커뮤니티와 공동 원장으로 다시 명시한 문서 변경입니다. 화면 코드는 바뀌지 않았으므로 새 캡처를 만들지 않으며, 이 문서 위쪽의 **역할 기반 통합 살뜰 커뮤니티 홈** 캡처를 현재 제품 방향의 대표 화면으로 계속 사용합니다.

## 검증 기록

- `Ssalddel`, `Ssalddel.Ui.Common`, `Ssalddel.WebApp`, `SsalddelApp` 빌드: 경고 0, 오류 0
- 이벤트·교육·Typecast·원장·다이어그램 관련 대상 테스트: 52개 통과
- 원장별 웹 팔레트: 2026-07-13 로컬 렌더링에서 화물 운송 원장과 살뜰 1.0 네 노드 확인
