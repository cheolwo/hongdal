...
- 음식에서 고른 재료는 `국내 공동구매 검토` 또는 `같이 수입 검토`로 명시적으로 분기한다. 음식 문화 국가는 실제 상품 원산지나 출발국으로 자동 사용하지 않으며, 같이 수입 여부는 후속 거래경로 단계에서 출발국·배송국·통관 상태·HS 코드로 다시 판정한다.
