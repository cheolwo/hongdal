...
- `ConnectionRequestEligible` 기록에서만 인연 연결 요청을 보낼 수 있으며 client에는 상대의 실제 사용자 ID를 전달하지 않는다.
...
업무 수행, 익명 인연 기록, 커뮤니티 재회, 상호 동의의 네 단계를 먼저 설명하고 그 아래에서 본인의 업무 접점을 조회한다. 로컬 API가 없는 실제 Windows MAUI 실행에서는 sample 관계로 대체하지 않고 연결 오류와 재시도를 표시한다.

![MAUI 업무에서 이어진 인연](../assets/changes/2026-07-24-community-work-relationships/community-work-relationships-error.png)

## 확인

- 업무 인연 커뮤니티 Route·익명 표시·client 계약·ViewModel 테스트 통과
- snapshot 소유권, 연결 가능 privacy, 양쪽 당사자 관점 조회와 인연 요청 생성 테스트 통과
