...
- 공동구매 원장의 조건·참여 내용 또는 진행 상태가 바뀌면 생성자와 참여자에게 Push 알림을 보내되, 변경자 본인은 제외하고 원장 Event ID로 중복 적재를 막는다.
- 공동구매 변경 알림에는 연락처·주소를 넣지 않고 원장명, 상태·단계 요약, 원장 식별자와 공동구매 화면 링크만 담는다.
- 원장 참여자는 `UserId`, 표시 이름, 역할, 참여 상태로 등록하고 각 블록은 그 참여자 중 주담당 1명과 협업·검토 담당자를 참조한다.
- 블록 담당자는 원장 생성자 또는 대표·담당·결정 역할의 참여자가 지정하며, 공개 원장 요약에서는 담당자 신원을 노출하지 않는다.
