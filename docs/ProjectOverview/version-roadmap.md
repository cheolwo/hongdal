...
소스 프로젝트는 앱/도메인 기준으로 유지하고, 버전 관리는 문서, 브랜치, 태그, 마이그레이션, 기능 플래그로 분리합니다. `DriverApp`, `HongdalApp`, `WarehouseManagerApp`, `Hongdal.Domain` 같은 실행 코드를 버전별 폴더로 복제하지 않습니다.
...
| --- | --- | --- |
| 화주 | 의뢰 등록, 상차/하차 정보, 화물 정보, 결제/정산 조건, 배차 상태 | `HongdalApp` |
