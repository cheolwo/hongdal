...
Ssalddel은 기능을 한 번에 넓히기보다 버전별 안정화 대상을 분리합니다.

소스 프로젝트는 앱/도메인 기준으로 유지하고, 버전 관리는 문서, 브랜치, 태그, 마이그레이션, 기능 플래그로 분리합니다. `DriverApp`, `SsalddelApp`, `WarehouseManagerApp`, `Ssalddel.Domain` 같은 실행 코드를 버전별 폴더로 복제하지 않습니다.

## 출시 순서

살뜰은 커뮤니티 기반을 먼저 `0.0`으로 독립 안정화하고, 그 위에 국내 화물/용달 `1.0`을 첫 실행 모듈로 올립니다. 이후 버전도 0.0의 계정, 동의, 원장, 신고와 신뢰 기록을 공통 기반으로 사용합니다.
...
커뮤니티는 0.0에서 독립 제품으로 시작하면서 동시에 이후 모든 버전을 관통하는 장기 성장 축입니다. 따라서 `SsalddelApiVersionAttribute`는 커뮤니티 기반 API의 최초 안정화 버전을 `0.0`으로 기록하고, `SsalddelApiGrowthTrackAttribute`는 이후에도 계속 성장하는 기능 계열임을 기록합니다.
