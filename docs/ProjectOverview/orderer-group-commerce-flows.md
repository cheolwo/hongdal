...
현재 릴리즈 집중 대상은 `문화교통 1.5` 공급·가격·무역 준비다. 아래 기능은 하나의 버전에 묶지 않고 `문화교통 1.0 수요·집단화 → 문화교통 1.5 공급·무역 준비 → 2.0 운송 → 2.5 창고·판매 이행`으로 나누며, 각 단계의 UI 노출과 외부 효과는 기능 플래그와 실행 경계를 따른다.

`1.0`의 정지선은 비구속 수요 저장과 모집 상태 표시다. 기본 14일 모집에서 조건 충족 여부를 계산하되 결제, 개별 주문 원장, 창고, 수입 신고와 운송 요청은 만들지 않으며, 조건 충족 후 별도 확인을 거쳐야 `1.5` 이후 절차로 이동한다.
