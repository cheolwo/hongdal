...
현재 릴리즈 집중 대상은 `1.5` 공급·가격·무역 준비다. 아래 기능은 하나의 버전에 묶지 않고 `1.0 수요·집단화 → 1.5 공급·무역 준비 → 2.0 운송 → 2.5 창고·판매 이행`으로 나누며, 각 단계의 UI 노출과 외부 효과는 기능 플래그와 실행 경계를 따른다.
...
flowchart TD
    ...
    C0 --> C["조건이 맞는 주문 원장을 공동주문 묶음으로 구성"]
    C --> D{"판매자 출처"}
