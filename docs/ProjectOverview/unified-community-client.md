...
살뜰 앱은 `/community`의 정보 공개형 커뮤니티에서 시작한다. 사용자는 글·공공데이터·음식과 재료 근거를 먼저 둘러보고, 필요해졌을 때 `0.5` 개별주문 의향을 남기거나 `/`의 역할·업무 선택으로 이동한다. 게시판 상세, 개별 원장, 다이어그램과 업무 도구는 사용자가 해당 영역으로 들어간 뒤 표시한다. `SsalddelApp`은 전환 기간의 내부 프로젝트명일 뿐 사용자에게 별도 화주 앱으로 표시하지 않는다. 상세 설계 원칙은 [통합 클라이언트 3단계 내비게이션](../Architecture/ThreeStageClientNavigation.md)을 따른다.
...
| --- | --- | --- | --- |
| 공개 커뮤니티 홈 | `/community` · App 시작 경로 | 공통 셸 | 정보·이야기·공공데이터를 먼저 둘러보고 개별주문을 선택적으로 시작한다. |
| 역할·업무 선택 | `/` | 공통 셸 | 반야·방편 두 겹 태극과 현재 역할의 업무 진입점을 보여준다. |
| 내 개별주문 시작 | `/community/orders/new` | 3단계 Action | 공개 근거를 유지한 채 수량·수령 조건과 공동후보 동의를 확인한다. |
| 내 개별주문 원장 | `/community/orders` | 3단계 데이터 | 주문자App과 같은 본인 원장을 조회하고 철회한다. |
...
`MainPage.xaml`은 앱 시작 경로만 `/community`로 정하고, `UnifiedHome.razor`는 `/`에서 역할만 판별해 `RoleNeutralHome`, 화주 홈 또는 `WarehouseManagerRoleHome`을 선택한다. 커뮤니티, 다이어그램, 업무 데이터 조회, 결제를 이 파일에 넣지 않는 것이 단일 책임의 기준이다.
...
커뮤니티 모바일 하단 내비게이션은 `커뮤니티`, `내 정보`, `내 글`, `내 주문`과 글쓰기 버튼을 제공한다. 역할·업무 선택은 drawer에서 `/`로 열어 커뮤니티 탐색과 업무 도구의 경계를 유지한다.
...
커뮤니티는 단순 장식 화면이 아니라 업무 기록인 Business Case로 들어가는 접수 표면이다. 게시글·댓글·투표·활동 신호는 커뮤니티 API가 담당하고, 구체적인 실행은 Business Case 템플릿과 해당 `ProcessManager`·`WorkflowCoordinator`·API로 넘긴다.
