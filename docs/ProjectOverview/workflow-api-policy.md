...
| 개념 | 의미 |
| --- | --- |
| 제품 버전 | API나 기능이 속한 로드맵 단계. 커뮤니티 기반은 0.0, 개별주문은 0.5, 같이 주문 집단화는 1.0, 공급·무역 준비는 1.5, 운송 이행은 2.0부터 시작 |
| 업무 실행 책임 모델 | Business Case, Policy, Process Manager, 판단 도구와 실제 상태 변경 경계를 구분하는 현재 아키텍처 기준 |
| HIOPS·하위 OS | 초기 설계와 기존 설정·식별자에 남은 호환 용어. 새 코드에서는 실제 책임에 따라 `ProcessManager`, `WorkflowCoordinator`, `Scheduler` 등을 사용 |
...
워크플로우와 실행 조율자는 같은 개념이 아닙니다. 워크플로우는 업무 절차와 책임 경계를 설명하고, Process Manager나 Workflow Coordinator는 여러 단계의 순서·재시도·보류와 판단 도구 호출을 조율합니다. 예를 들어 `공동구매수요모집ProcessManager`는 `공동구매 수요·모집` 흐름의 장기 상태를 관리하며 주문자 집단화 판단 도구를 호출합니다.

Business Case, Case Section, Policy, 실행 조율, 판단 도구, API/UseCase와 저장소의 전체 책임 경계는 [업무 실행 책임 모델](../Architecture/BusinessWorkflowResponsibilityModel.md)을 기준으로 봅니다.

`GET /api/v1/version-feature-flags`의 `OperatingSystems` 항목은 기존 API 호환 이름입니다. 값은 워크플로우, 판단 도구와 스케줄링 정책 설명에 사용할 수 있지만, 새 내부 타입의 기술 역할을 `OS`로 명명하는 근거로 사용하지 않습니다.
...
| 같이 주문 기반 수입 | 주문자 집단 대표, 주문자 | 해외 판매자·배송대행지, 플랫폼 운영자 | 주문자 집단이 해외 상품을 같이 들여와 통관, 국내 반출, 분배 또는 3PL 입고로 넘기는 범위 |
...
| 국내 화물 운송 | 기사 | 기사 앱 | 추천 목록 | `/driver/recommendations` | 추천된 일반 화물, 같이 주문 운송, 배송 의뢰 확인 |
...
| 같이 주문 기반 수입 준비 | 주문자 집단 대표 | 주문자 앱 | 수입 원가 참고 | `/group-purchase/import-review/{ProductId}` | 1.5 Simulation 원가와 검토 필요 항목 확인 |
| 같이 주문 기반 수입 준비 | 주문자 | 주문자 앱 | 선적 조회 | `/group-purchase/shipments` | 문서관리번호 한 건의 공개 선적 상태 조회 |
...
## Controller/UseCase와 실행 조율

컨트롤러는 유스케이스를 주입받아 HTTP 요청을 넘기는 역할만 맡습니다. 컨트롤러에 업무 판단을 쌓지 않고, 유스케이스 이름이 다이어그램의 노드와 대응되도록 둡니다.

살뜰의 Controller API는 단순한 HTTP CRUD 목록이 아니라, Policy와 현재 상태가 허용한 업무 기능을 호출하는 입구로 본다. 실제 코드 호출은 `Controller -> UseCase/Command` 방향이며, 장기 실행이 필요하면 Process Manager가 같은 실행 경계를 호출한다.

예를 들어 `POST api/v1/driver/transports/{id}/pickup-complete`는 기사 앱에서 호출하는 Controller action이며, 운송 진행 UseCase나 Command가 권한과 현재 상태를 검증한 뒤 상태를 바꾸는 실행 단위다.

따라서 새 Controller API를 만들 때는 다음 항목을 함께 기록한다.

| 항목 | 기록 기준 |
| --- | --- |
| 실행 조율 | 연결된 Process Manager·Workflow Coordinator·Scheduler가 있는지 |
| 워크플로우 | 어떤 업무 절차와 화면에서 호출되는지 |
| 실행 UseCase/Command | 컨트롤러가 넘기는 내부 실행 단위 |
| Case Section | 참여자, 장소, 물건, 상태, 증빙, 정산, 인계 중 어떤 Section을 읽거나 바꾸는지 |
| 상태 전이 | API 성공 후 어떤 상태가 바뀌는지 |
| 선행 판단 | 호출 전 필요한 조합 규칙, 엔진 결과, 권한, 사람 확인 |
| 후속 이벤트 | 이벤트, outbox, 감사 로그, 경험치 후보, 원장 투영 갱신 여부 |

컨트롤러는 Policy, 장기 실행 조율이나 판단 도구를 직접 구현하지 않는다. 화면에서 직접 호출한 경우와 Process Manager가 후속 작업을 시작한 경우 모두 같은 UseCase 검증을 통과해야 한다.
...
| 같이 주문 기반 수입 | `공동구매자동집단화UseCase`, `공동구매커머스이행계획UseCase`, `공동구매해외선적추적UseCase`, `공공데이터조회UseCase` |
| 창고 입출고 | `창고작업UseCase` |
...
같이 주문 기반 수입 워크플로우에서도 같은 원칙을 적용합니다. `api/v1/orderer/group-purchase-overseas-shipments`와 관리자 원장 API 경로는 유지하되, 서버 코드에서는 `공동구매해외선적추적UseCase`가 문서관리번호 조회, BL 기반 공개 조회, 수입 물류 정규화, 통관 동기화, 원장 이벤트 추가를 담당합니다.
...
| 같이 주문 기반 수입 | 신호 공개 | 커뮤니티 신뢰 | 모집, 투표, 진행 상태, 분배 후기를 개인정보 보호 범위에서 보냅니다. |
| 판매채널 출고 | 신호 공개 | 커뮤니티 신뢰 | 판매 후기와 상품 여정 신호를 동의된 범위에서 보냅니다. |

```mermaid
flowchart LR
    Customs["통관·무역 데이터"]
    GroupImport["같이 주문 기반 수입"]
    Domestic["국내 화물 운송"]
    Warehouse["창고 입출고"]
    Sales["판매채널 출고"]
    Hr["참여 인력 관리"]
...
문화교통 0.0의 커뮤니티 신뢰는 모든 실행 워크플로우보다 먼저 독립적으로 동작하는 기반입니다. 문화교통 0.5는 한 사람의 철회 가능한 개별주문 원장을 만들고, 문화교통 1.0 같이 주문은 동의한 개별 원장만 주문자 집단으로 묶습니다. 문화교통 1.5 공급·무역 준비를 거쳐 실제 이동이 필요한 시점에 살뜰 2.0 국내 화물 운송으로 인계합니다. 창고 입출고와 판매채널 출고는 살뜰 2.5에서 결과를 이어받고, 음식 배달과 알뜰살뜰 마트도 실제 상차·하차·증빙이 필요할 때 운송 워크플로우를 재사용합니다.
