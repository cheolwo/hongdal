...
**HIOPS**는 Ssalddel Integrated Operations & Policy System의 약자이며 살뜰의 최상위 운영 체제 이름이다. HIOPS는 특정 업무 목적을 달성하기 위해 공동구매 수요·모집 OS, 공동주문 수입 OS, 국내 화물 운송 OS, 창고·커머스 이행 OS 같은 하위 OS와 워크플로우, 엔진을 조합하는 상위 실행 관점이다. 다만 HIOPS의 최종 목적은 기술 구성 자체가 아니라 여러 참여자의 입장을 조율하는 데 있다.
...
예를 들어 `공동구매 수요·모집 OS`는 주문자 집단화 엔진을 호출해 비구속 수요의 배치와 모집 진행을 설명한다. `공동주문 수입 OS`는 확인된 모집 결과를 받은 뒤 공급·무역 준비와 후속 운송·창고 인계를 조율하고, `국내 화물 운송 OS`는 실제 이동이 필요한 시점부터 운송 의뢰 배차 엔진을 사용한다.
...
| --- | --- | --- | --- |
| 공동구매 수요·모집 OS | 비구속 수요를 안전하게 묶고 모집 진행·마감·사람 검토와 1.5 인계를 조율 | 주문자 집단화 엔진 | Batching, EDF, Aging |
