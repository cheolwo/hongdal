...
이 문서는 현재 홍달에서 핵심 실행 단위로 보는 운영 체제와 엔진을 한 곳에 정리한다.

## 상위 개념: 홍달 운영 체제

홍달에서 운영 체제(OS)는 특정 업무 목적을 달성하기 위해 워크플로우와 엔진을 조합하는 상위 실행 관점이다. 다만 OS의 최종 목적은 기술 구성 자체가 아니라 여러 참여자의 입장을 조율하는 데 있다.

화주, 기사, 주문자, 창고 관리자, 관세사, 운영자는 같은 사건을 서로 다른 입장에서 본다. OS는 이 차이를 무시하지 않고, 누가 어떤 책임을 지는지, 누가 언제 기다리는지, 비용과 증빙이 어디에 남는지, 특정 참여자에게 부담이 과하게 몰리지 않는지를 계속 조정한다. 홍달에서 OS는 “내가 당하기 싫은 일을 남에게 쉽게 넘기지 않고, 내가 받고 싶은 배려를 다른 참여자에게도 줄 수 있게 하는 운영 기준”에 가깝다.

엔진은 직접 사용자에게 노출되는 제품 단위가 아니라, OS가 이런 조율을 실제 업무 안에서 실행하기 위해 호출하고 조정하는 판단 도구다.
...
```mermaid
flowchart TD
    OS["홍달 운영 체제<br/>참여자 입장 조율과 운영 정책"]
