...
예를 들어 `공동구매수요모집ProcessManager`는 주문자 집단화 엔진을 호출해 비구속 수요의 모집 진행을 조율한다. 같이 수입 준비 ApplicationService는 확인된 모집 결과를 공급·무역 준비로 연결하고, 운송 Workflow Coordinator는 실제 이동이 필요한 시점부터 배차 판단 도구를 사용한다.
...
현재 기본 공개 workflow는 `CommunityTrust`다. 공개 정보, 참여 동의, 공동 원장과 완료 사례 환류를 후속 실행 workflow 없이도 닫을 수 있어야 한다. [공동구매 수요·모집 Process Manager](GroupPurchaseDemandProcessManager.md), 국내 화물 운송과 이후 workflow도 3.5 전체 구축 범위로 계속 완성하되, 기본 공개와 외부 효과는 각 capability의 릴리즈 게이트를 통과한 뒤 승격한다.
...
이 판단 도구들은 서로 독립된 기능처럼 보이지만 실제 업무에서는 순서대로 이어진다. 주문자가 구매 의사를 남기면 서버가 같은 배송권 안의 수요를 묶고, 묶인 주문이나 판매채널 주문은 창고 출고 가능 상태로 바뀐다. 출고 배치가 어느 창고의 어느 재고를 쓸지 정하면, 피킹 배치가 적재대의 물건을 현장 작업자에게 나누고 피킹/포장 통합 또는 분리 방식을 결정한다. 실제 이동이 필요해지면 운송 의뢰와 배차, 차량 적재 판단으로 넘어간다.

## 주문자 목 중심 오행 관점

아래 내용은 주문자 경험을 이해하기 위한 설계 메모다. 서버 카탈로그, 엔진 배정 규칙, 상태 전이 또는 화면 노출 기준으로 구현하지 않는다. 주문자를 `목`으로 보되 주문자와 직접 연결되는 `목극토`, `금극목`, `목생화`, `수생목` 네 관계만 생각의 틀로 사용한다.

| 관계 | 주문자 관점의 잠정적 해석 |
| --- | --- |
| 목극토 | 여러 주문 의사가 상품·수량·수령 조건에 따라 하나의 주문 집단으로 정리되는 흐름 |
| 금극목 | 가격, 최소 주문량과 공급 가능 수량이 주문자의 선택 범위를 제한하는 흐름 |
| 목생화 | 주문자의 의사가 모집이나 주문 진행으로 드러나는 흐름 |
| 수생목 | 주문한 뒤 운송 정보를 받아 현재 진행 상태를 아는 흐름 |

특히 `수생목`의 `수`는 새 배송권이나 수요 집단을 만드는 엔진보다, 이미 주문한 건의 운송 정보가 주문자에게 되돌아오는 의미에 가깝다. 주문자는 준비 중, 출고, 운송 중, 도착 예정과 수령 완료 같은 정보를 확인한다. 창고 배치, 피킹, 기사·차량 배차와 적재는 내부 업무로 유지하고 주문자에게는 필요한 진행 정보만 전달한다.

이 해석은 향후 화면과 원장 흐름을 검토할 때 참고하되, 오행 명칭만으로 사용자를 집단에 가입시키거나 모집·주문·결제·계약·배차 상태를 확정하지 않는다.
...
| 차량 적재·혼적 엔진 | 화물과 차량의 적합성을 판단하고 상·하차에 맞는 적재 순서를 계획한다. | 화물 중량·부피·취급 조건, 차량 제원, 상·하차 순서 | 적합 차량 후보, 적재율, 적재·혼적 순서와 경고 | `차량적재추천Engine`, `차량혼적순서Engine` |
...
sequenceDiagram
    ...
    participant UseCase as 창고 작업 UseCase
    participant Engine as 피킹 배치 엔진
    participant Picker as 피킹 작업자 화면
    participant Packer as 포장 작업자 화면

    Manager->>UseCase: 창고별 피킹/포장 방식 저장
    UseCase->>Engine: 출고 라인·적재대·작업자 후보 전달
    Engine-->>UseCase: 피킹·포장 배치 계획 반환
    UseCase->>Picker: 검증·저장된 피킹 작업 목록 제공
    Picker->>Picker: 적재대 위치 바코드 스캔
    Picker->>Picker: 해당 적재대의 할당 상품 목록 조회
    Picker->>Picker: 상품 선택, 상품 바코드 스캔, 수량 입력
    Picker->>UseCase: 피킹 완료 Command
    alt 피킹/포장 통합
        UseCase->>Picker: 같은 작업자의 포장 작업 제공
    else 피킹/포장 분리
        UseCase->>Packer: 포장 작업 인계
    else 피킹만
        UseCase->>Manager: 피킹 완료 상태 반영
    end
```

## 4. 운송 의뢰 배차 엔진

운송 의뢰 배차 엔진은 실제 차량과 기사를 붙이는 살뜰 2.0 흐름이다. 문화교통 1.0~1.5는 수요와 공급·이동 준비 정보를 사람에게 인계할 뿐 이 엔진을 자동 실행하지 않는다. 같이수입, 창고 출고, 판매채널 출고, 알뜰살뜰 마트 같은 후속 워크플로우가 실제 운송이나 고객 전달을 필요로 하고 운영 조건을 충족할 때 이 엔진으로 합류한다.
...
sequenceDiagram
    ...
    participant Coordinator as 배차 Workflow Coordinator
    participant Engine as 운송 의뢰 배차 엔진
    participant UseCase as 배차 상태전이 UseCase
    participant Driver as DriverApp
    participant Admin as SsalddelAdmin

    Source->>Coordinator: 운송 의뢰 생성 또는 배차대기 진입
    Coordinator->>Engine: 원본의뢰유형·업무 유형·기사 후보 전달
    Engine-->>Coordinator: 추천 후보·점수·사유 반환
    Coordinator->>UseCase: 추천 상태전이 요청
    UseCase->>Driver: 검증·저장된 기사 추천 노출
    Driver->>UseCase: 수락, 거절, 보류
    UseCase->>Admin: 배차 상태 변경
...
| 판단 도구 | 현재 경계 | 다음 완성도 보완 |
| --- | --- | --- |
| 주문자 집단화 엔진 | Process Manager가 Mongo 원장 상태와 사람 승인 인계를 소유하고 엔진은 집단화 결과를 계산한다. | 여러 서버의 정기 점검이 같은 집단을 잡지 않도록 분산 lease를 추가하고 `ReadyToConfirm` 이후 확정 Command를 더 좁힌다. |
| 공동구매 경제성 엔진 | 수량별 비용·편익·위험 시나리오를 순수 계산한다. | KAMIS 원문 거래 단위와 사용자 환산 단위를 분리해 계산 근거 DTO까지 일관되게 전달한다. |
| 출고 배치 엔진 | 판매채널 동기화가 계획 결과를 검증한 뒤 재고를 원자적으로 예약한다. | 후보 재고 조회를 Planning Service로 분리해 Engine을 순수 계획 경계로 좁히고, 주문참조·판매자·SKU의 동시 생성 고유성을 보강한다. |
| 피킹 배치 엔진 | 적재대·작업자·피킹/포장 방식을 입력받아 작업 계획만 반환한다. | 계획 저장·작업 보드 조회·스캔 완료 Command를 Warehouse API로 연결한다. |
| 차량 적재·혼적 엔진 | 차량 후보와 상하차 순서를 계산하고 DB 조회는 별도 Service가 담당한다. | 출고 배치 결과에서 운송 의뢰 초안으로 넘어가는 명시적 인계 계약을 둔다. |
| 화물·음식 배차 엔진 | 후보·점수·준비 불가 사유를 반환하고 상태전이는 배차 Service가 담당한다. | 후보 조회 Port를 더 좁히고 화물·음식·마트 원천별 릴리즈 게이트와 추천 목록 표시를 일치시킨다. |
| 이미지·광고·배송권·수출 준비 Planner | 초안과 차단 사유만 만들고 게시·계약·외부 실행은 별도 계층에 둔다. | Planner 결과를 실행 Command로 승격할 때 사람 승인과 `Simulation`/`Operational` 경계를 공통 계약으로 검증한다. |

배차·통관·판매채널 자동 작업은 `SsalddelBackgroundJobActivationPolicy`에서 실행 모드, workflow 기능 플래그와 작업별 활성 설정을 함께 확인한다. Scheduler 등록 여부만으로 실행 권한을 판단하지 않는다.
...
- 각 엔진에서 발생한 이벤트를 커뮤니티에 공개 가능한 활동 신호로 변환하는 정책이 필요하다.
