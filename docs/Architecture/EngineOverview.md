...
이 문서는 현재 살뜰에서 핵심 실행 단위로 보는 HIOPS, 하위 OS, 워크플로우, 엔진을 한 곳에 정리한다.
...
**HIOPS**는 Ssalddel Integrated Operations & Policy System의 약자이며 살뜰의 최상위 운영 체제 이름이다. HIOPS는 특정 업무 목적을 달성하기 위해 국내 화물 운송 OS, 공동주문 수입 OS, 창고·커머스 이행 OS 같은 하위 OS와 워크플로우, 엔진을 조합하는 상위 실행 관점이다. 다만 HIOPS의 최종 목적은 기술 구성 자체가 아니라 여러 참여자의 입장을 조율하는 데 있다.
...
국내 화물 운송 OS는 살뜰 1.0의 기준 OS이므로 상세 운영 기준을 [국내 화물 운송 OS](DomesticCargoTransportOS.md)에 별도로 둔다.
...
운송 의뢰 배차 엔진은 실제 차량과 기사를 붙이는 흐름이다. 살뜰 1.0의 중심축이며, 공동주문 수입, 창고 출고, 판매채널 출고, 알뜰살뜰 마트 같은 확장 워크플로우가 실제 운송이나 고객 전달을 필요로 할 때 이 엔진으로 합류한다.
