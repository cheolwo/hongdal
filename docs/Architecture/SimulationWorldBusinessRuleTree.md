...
│  ├─ 주문: 개인 주문
│  ├─ 마트: 재고 진열
│  ├─ 창고: 용량 예약 / 입고 검수 / 재고 적재
│  ├─ 물류: 거점 간 이동
│  ├─ 화물: 배차 / 운송
│  ├─ 음식점: 식재료 주문
│  ├─ 팀 역할: 카드 장착 / 활동 시작 / 활동 종료
│  └─ 수집 보상: L2·L1 발견 / 직접 농사 완료 / 개인 뽑기 / 팀원 양도
├─ 5. 객체–업무 규칙 연결
...
├─ 8. Figma 근거 UI 기획 원장
│  ├─ 역할 + 업무 단계 + 정보 + 상태 + 행동 후보
│  └─ UI–규칙 연결
│     └─ 원본 객체–업무 규칙 연결 + 시설 기능 + 규칙 개정
└─ 9. 독립 Synty·URP 표현 파이프라인
...
| UI 기획 | 역할별 정보·상태·행동과 원본 시설 기능·업무 규칙 연결 | Canvas 좌표·Prefab·서버 상태 확정 |
...
`pyeongchang-farm-hub-town-business-rules.v4`는 시설 4개, 시설 기능 18개, 업무 Simulation 규칙 18개, 객체–규칙 연결 11개, Scenario 규칙 묶음 1개와 규칙 항목 18개를 구성한다. `v3`를 바꾸지 않고 L2 타일 발견 보상, 직접 농사 완료 보상, 개인 뽑기와 팀원 양도 규칙 정의를 추가한 불변 개정이다. 팀 역할과 수집 보상 규칙은 특정 건물 기능에 묶이지 않는 Session 규칙이므로 객체–규칙 연결 수는 늘리지 않는다.

역할 카드 규칙의 식별자·Engine·입출력 계약은 이 파생 DB에 저장하지만, 누가 어느 카드 사본을 장착했는지와 현재 진행 중인 활동은 저장하지 않는다. 가변 상태는 Simulation Session 원장과 Save/Replay가 소유한다.

수집 카드 정의와 확률 규칙의 개정·해시는 Session 상태 사본과 재생 해시에 봉인한다. 발견 타일, 행위자별 현재 L2, 실패 보정 수, 미개봉 기회, 카드 사본, 소유자와 양도 이력은 가변 상태이므로 공간 파생 DB가 아니라 Simulation Session 저장 자료가 소유한다. 수집 카드는 표현용이며 기존 팀 역할 카드나 생존 타로와 합치지 않는다.
...
UI는 업무 규칙 식별자를 임의로 직접 선택하지 않는다. 활성 `객체–업무 규칙 연결`을 입력으로 받아 같은 시설의 화면 영역에 연결하고, 원본 연결 식별자와 시설 기능 코드를 함께 보존한다. 따라서 규칙이 다른 시설로 옮겨지거나 기능 연결이 바뀌면 기존 UI 기획 해시를 그대로 재사용할 수 없다.
