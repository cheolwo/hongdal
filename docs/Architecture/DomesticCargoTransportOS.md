...
| 입금대기 | 운송완료후정산 건이 하차 완료되어 화주 입금이 필요함 | `정산상태=입금대기`, 토스페이먼츠 가상계좌 결제대기 건, 1일/3일/7일 입금 요청 알림 |
...
| `POST` | `/api/v1/payments/prepare` | 하차 완료 후 입금 요청 | 토스페이먼츠 가상계좌 결제 흐름으로 확장 가능한 결제대기 건 생성 기준 |
