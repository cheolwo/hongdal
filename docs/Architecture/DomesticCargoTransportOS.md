...
국내 화물 운송 OS는 하나의 정책만 쓰지 않는다. 필수 조건은 `Fit First`로 걸러내고, 시간창은 `EDF`, 지리 조건은 `Geo Nearest`, 큐 승격은 `MLFQ`, 장기 대기는 `Aging`으로 보정한다.
...
| Aging | 배차대기 | 오래 기다린 의뢰의 추천 점수와 노출 우선순위 보정 | 임계값 초과 시 운영자 확인 또는 공개배차 전환 |
...
sequenceDiagram
    ...
    Policy-->>Engine: 차량 적합성 + 거리 + 시간창 + Aging 점수
