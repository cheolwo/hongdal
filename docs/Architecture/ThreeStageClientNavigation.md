...
역할 선택과 커뮤니티는 이 세 단계 바깥의 공통 셸 문맥이다. 역할을 정하지 않았을 때는 사방괘 대신 바깥 `방편·반야` 태극과 안쪽 `커뮤니티·상점` 태극을 겹친 공통 시작점을 보여준다. 역할을 바꾸면 세 단계에서 보이는 방향, 노드, 행동이 함께 바뀌며, 커뮤니티 글이나 원장 초안에서도 관련 다이어그램으로 들어갈 수 있다.
...
| 아래 | 북·감괘 `☵` | 운송 | 출고 이후 운송 연결 다이어그램을 연다. |
| 왼쪽 | 동·진괘 `☳` | 입고 | 공급처·입고·검수·보관 다이어그램을 연다. |

음식 배달 기사 역할은 같은 위치를 배달 원장의 참여자와 실행 흐름에 맞춰 사용한다.

| 화면 위치 | 방위·괘 | 배달 문맥 | 선택 결과 |
| --- | --- | --- | --- |
| 위 | 남·리괘 `☲` | 음식점 | 음식점 위치와 픽업 정보를 확인한다. |
| 오른쪽 | 서·태괘 `☱` | 배차 | 추천 배달권과 배차 수락 흐름을 확인한다. |
| 아래 | 북·감괘 `☵` | 운송·배달 | 픽업 이후 주문자 전달 완료까지 처리한다. |
| 왼쪽 | 동·진괘 `☳` | 주문자 | 주문자 전달 위치와 도착 정보를 확인한다. |

모든 역할의 공통 사방괘 가운데는 산을 뜻하는 간괘 `☶`다. 펼친 이동판은 외곽 사방괘 원과 내부 간괘 원의 두 동심원으로 구성한다. 내부 간괘는 태극 문양처럼 위쪽 빨간 양효 영역과 아래쪽 파란 음효 영역으로 나누며, 빨간 영역은 커뮤니티 글 목록, 파란 영역은 꾸미기 상점을 연다. 이 중심 디자인은 공통 컴포넌트의 기본값으로 유지하고, 역할별로는 외곽 사방괘의 목적지만 바꾼다.
...
| 집합/복합 원장 | 여러 기본 원장을 묶거나 후속 이행 원장을 조정 | 기본 원장은 독립 경계를 유지하고 바깥에 집합 테두리와 후속 관계선을 표시 | 공동주문 묶음 원장 |

연결 원장 묶음은 원장 데이터를 합치는 뜻이 아니다. 각각의 식별자, 상태, 참여자, 권한, 종료 조건은 유지하고, 사용자가 하나의 업무 여정으로 이해할 수 있도록 개요 다이어그램에서 함께 보여준다. 집합/복합 원장은 하위 원장을 숨기는 큰 레코드가 아니라 기본 원장들의 참조와 진행 상태를 요약하는 상위 문맥이다.
...
`공동주문 묶음 원장`은 별도의 거대한 주문이 아니라 상품과 공동 조건이 맞는 여러 `주문 원장`을 참조로 묶은 집합이다. 각 주문 원장은 자신의 판매·입출고·배송·운송 원장을 계속 독립적으로 관리한다. 수입 결정, 선적/통관과 입고/분배는 주문 묶음이 확정된 뒤 열리는 후속 이행 원장이다.

```mermaid
flowchart TD
    subgraph GP["공동주문 묶음 원장"]
        O1["개별 주문 원장 A"] --> A["주문 묶음·수량 합계"]
        O2["개별 주문 원장 B"] --> A
        O3["개별 주문 원장 C"] --> A
        A --> B["공동주문 수입 결정 원장"]
        B --> C["공동주문 선적/통관 원장"]
...
개별 주문에서 묶음으로 들어오는 실선은 `포함(1:N)` 관계이고, 묶음 이후 실선은 후속 이행 순서다. 점선은 조건에 따라 열리는 외부 업무 원장 인계다. 공동주문 개요에서 개별 주문이나 후속 원장을 선택하면 같은 2단계의 세부 원장 다이어그램을 열고, 해당 원장의 노드 행동을 선택할 때만 3단계 데이터 페이지로 이동한다.
