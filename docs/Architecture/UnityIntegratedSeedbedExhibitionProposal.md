...
에셋 모판의 S0~S6와 턴 카드 모판의 C0~C6을 일반화해 전시 후보에는 `E0~E7` 승격 조건을 적용한다.

| 단계 | 이름 | 통과 조건 |
| --- | --- | --- |
| E0 | 후보 등록 | 목적, 소유 업무 흐름, 금지 효과를 기록 |
| E1 | 출처·계약 | 출처와 기준 원장 고유 식별자, 계약 상태 버전 확인 |
| E2 | 권한·개인정보 | 관점과 공개·권한 필드를 테스트로 확인 |
| E3 | 읽기 관점별 조회 결과 | 서버·시뮬레이션 상태 사본과 실패 상태를 보존 |
| E4 | 전시 표현 | 한국어 카드·VisualKey·계보·mode가 Game View에서 읽힘 |
| E5 | 체험 폐루프 | 필요한 경우 미리보기→확정→Tick/Command→재조회 검증 |
| E6 | 운영 인계 | 인증·상태 버전·오류·Web/MAUI 인계를 실제 환경에서 확인 |
| E7 | World 이식 | 대표 Game View, 성능, 변경 기록과 회귀 test를 남기고 정식 Scene catalog에 승격 |

`E4`까지만 통과한 항목은 훌륭한 전시 후보일 수 있지만 운영 기능은 아니다. `E5` 시뮬레이션 통과도 `E6` 실운영 통과를 대신하지 않는다.
