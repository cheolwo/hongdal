...
- `SsalddelAdmin`(Blazor Web)과 `RestaurantDeskApp`(MAUI Blazor Hybrid)에서 동일한 루트 Provider/테마 구성을 사용한다.
...
- 이 기준을 따르면 `Ssalddel.Ui.Common`의 공통 컴포넌트를 SsalddelApp, WarehouseManagerApp, RestaurantDeskApp, OrdererApp, SsalddelAdmin에서 재사용하기 쉽다.

## 이번 단계에서 적용한 항목

### 1) 공통 프로젝트 추가
- `Ssalddel.Ui.Common/Areas/BackOffice` (Ssalddel.Ui.Common 내부 BackOffice 공통 UI 영역)
...
- `Ssalddel.BackOffice.Client` (Class Library)
  - 공통 API 옵션 뼈대
...
- `SsalddelAdmin`은 기존처럼 Blazor Web 유지.
- 두 앱의 루트 MudBlazor Provider/테마 구성이 공통 컴포넌트로 통합됨.
- 공통 클라이언트/진단 계층은 뼈대 수준으로 준비됨.

## 다음(2차) 권장 작업
1. `SsalddelAdmin`와 `RestaurantDeskApp` 페이지 중 중복 UI를 `Ssalddel.Ui.Common/Areas/BackOffice`로 단계적 이관
2. 공통 메뉴/레이아웃(네비, 헤더, 상태 배지) 통합
3. `Ssalddel.BackOffice.Client`에 공통 HttpClient 팩토리/인증 핸들러 추가
