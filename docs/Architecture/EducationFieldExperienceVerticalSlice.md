...
학생이 홍달 안에서 수행한 활동을 학교에 제출할 수 있는 증빙 묶음으로 정리한다. 플랫폼은 활동 기록과 제출 과정을 지원하지만 출석 인정 자체를 확정하지 않는다. 최종 출석 인정 여부는 원장 학교 범위와 일치하는 선생님의 결정으로만 기록한다.
...
- 자동 전송은 기본적으로 꺼져 있다.
- RDB 블록 관계 투영에는 교육 원장의 블록 내용과 개인 식별 서술을 복제하지 않고 구조·상태 인덱스만 남긴다.
