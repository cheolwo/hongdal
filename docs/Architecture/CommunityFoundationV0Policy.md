...
살뜰은 예수의 이웃 사랑과 『대학』의 `수신 -> 제가 -> 치국`을 같은 실천 원리로 봅니다. 자신을 바로 세우고 가까운 이웃을 사랑하며, 자신이 책임질 수 있는 약속부터 실천해 작은 관계에서 쌓은 신뢰를 더 넓은 공동체로 확장합니다. 세부 판단 질문과 피해야 할 구현은 [이웃에서 시작하는 공동행동 개발 철학](NeighborCenteredDevelopmentPhilosophy.md)을 따릅니다.

업무에서 만난 사람과 관계를 이어가는 기능은 업무 로그를 자동 친구 관계로 바꾸지 않습니다. 친구 후보 기록, 친구 요청, 친구 수락과 연락처 공개를 분리하는 상세 기준은 [업무 경험에서 친구 요청으로 이어지는 커뮤니티 설계 기준](FriendRequestCommunityDesignStandard.md)을 따릅니다.
...
2. 관심, 참여, 친구 요청·수락, 연락처 공개, 가원장과 실원장 전환은 서로 분리된 동의입니다.
...
문화교통 0.0의 실제 사용자 흐름은 플랫폼 배차가 아니라 다음과 같은 커뮤니티 대화로 시작합니다.
...
2. 살뜰 2.0 국내 화물/용달은 문화교통 0.0 기반이 비활성인 환경에서 운영하지 않습니다.
...
서버 실행 모드는 `SsalddelExecution:Mode` 한 곳에서 관리하며 값은 두 개만 사용합니다.
