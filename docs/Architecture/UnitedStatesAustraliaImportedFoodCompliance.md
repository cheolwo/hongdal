...
검토 기준일은 **2026-07-19**다. 이 문서는 살뜰에서 같이 수입 여정을 설계하거나 글쓰기 다이어그램에 규정 확인 노드를 붙일 때 사용하는 개발 기준이며, 개별 거래에 대한 법률·통관 자문이 아니다.
...
- 살뜰은 확인 항목과 공식 근거를 보여 주되 신고, 통관 승인, importer 지정, broker 자동 선택을 수행하지 않는다.
...
호주는 아직 살뜰의 실제 운영시장으로 활성화하지 않는다. 두 국가 profile 모두 다음 값을 유지한다.
