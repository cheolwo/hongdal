...
5. 생성된 음성을 Google Cloud Storage에 올리고 구간별 객체 정보를 DB에 저장한다.
6. 사용자가 음성 정보 또는 파일을 요청하면 접근 로그를 남긴다.

이벤트 신호가 유실되더라도 DB 작업이 남으므로 Worker의 주기 조회에서 복구된다. 게시글 등록 요청은 Typecast와 Google Cloud Storage의 응답을 기다리지 않는다.

## 비용 제한

정규화된 `제목 + 본문`이 100자 이상 500자 미만일 때만 Typecast TTS를 호출한다. 100자 미만 또는 500자 이상이면 작업을 `길이제외`로 종료하므로 API 호출, 재시도, GCS 업로드 비용이 발생하지 않는다. 경계값은 `CommunityPostAudio:MinCharacters`와 `MaxCharactersExclusive`로 관리한다.
...
Google Cloud Storage의 실제 객체 주소는 공개 응답에 포함하지 않는다. 다운로드가 서버를 통과하므로 로그인 사용자는 사용자 ID, 익명 사용자는 요청 Trace ID를 기준으로 접근 기록을 남긴다.
...
```json
{
  ...
  "GoogleCloudStorage": {
    "BucketName": "private-audio-bucket"
  }
}
```
...
최초 게시글 등록 시 음성을 생성한다. 게시글 수정에 따른 음성 재생성, 오래된 GCS 객체 정리, 관리자 수동 재시도 화면은 후속 범위다.
