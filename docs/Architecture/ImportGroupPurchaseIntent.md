...
> **문서 책임:** 이 문서는 `group-purchase`와 `group-import`의 업무·원장 경계를 정의하는 기준 문서다. 주문자 화면에서 수입 과정을 설명하는 구체적인 화면 구성과 구현 순서는 [주문자 수입 여정 다이어그램 활용 계획](OrdererImportJourneyDiagramUtilizationPlan.md)에서 다룬다.

## 원장 경계

- `group-purchase`는 개별 주문 집계, 공동 조건, 구매 확정, 수령 거점과 참여자 분배를 담는 공동구매 원장이다.
- `group-import`는 확정된 `group-purchase`를 원천 원장으로 연결한 뒤 해외 발주, 선적, 통관, 국내 반출과 3PL 입고를 담는 같이 수입 원장이다.
...
- Entry point: 공통 홈의 업무 모드에서 `같이수입` 선택
...
- 상품 옵션에 HS 코드가 있으면 같이 수입 선택안, 없으면 국내 공동구매로 구분한다.
...
- `같이 수입 후보`: 선택한 HS 코드별 실제 구매 후보 상품과 공급·가격·Incoterms 참고 조건 입력
...
- 같이 수입 선택 결과가 확정되면 기존 같이 수입 원장의 해외 선적·통관·입고 단계로 넘긴다.
...
- `group-import` 같이 수입 원장이 최초 등록될 때 승인·활성·수임 가능·수입 전문 조건을 모두 만족하는 플랫폼 관세사에게 알림을 적재한다.
...
같이 수입 원장은 확정된 공동구매 원장을 원천 근거로 연결한 뒤 아래 노드로 풀어 다이어그램에 배치한다.
...
- `1.5 준비 블록`: 여러 재료의 합산 수요, 공급·가격·품목분류·규제 근거와 최소 전달 자료를 기존 같이 수입 원장 안에서 준비한다.
...
- `SsalddelApp`의 공동구매·같이 수입 화면에서 여러 재료의 수요 확인 캠페인을 생성한다.
...
- 포워더 회신의 견적 유효기간·물류 조건과 같이 수입 결정 사이의 검증 규칙을 보강한다.
