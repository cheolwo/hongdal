...
| 구분 | 예시 | 책임 | 서버 재시작 시 |
| --- | --- | --- | --- |
| DB 업무 원장 | `배차대기`, `화주운송의뢰`, `배송_운송` | 추천중, 확정, 운송 진행, 완료 같은 업무 상태를 확정한다 | 그대로 남아야 한다 |
...
1. `배차대기.배차업무유형 == 용달운송`
2. `배차대기.상태 == 대기`
3. `배차대기.배차큐단계`가 `확정` 또는 `종료`가 아님
4. `배차대기.배차노출상태`가 `추천중`이 아니거나, 추천 만료 시각이 지남
...
| 배달권 후보 묶음 | `배차대기`, `용달기사`, `기사근무`, 위치 heartbeat | `I배달권실행공간Store` 사전/해시셋 실행 공간 |
| 미처리 운송 의뢰 | `배차대기` 대기·미확정·미종료 행 | DB에 그대로 두고 배차 스캔과 조율 입력 Factory가 읽음 |
...
| `국내화물배차조율입력Factory.LoadQueuesAsync` | DB `배차대기`에서 미처리 운송 의뢰를 읽는 원장 조회 지점 |
| `국내화물배차조율입력Factory.LoadDriverStatesAsync` | 메모리 기사 상태 인덱스에서 후보 기사를 읽는 실행 조회 지점 |
| `배차대기원장전환Service` | `배차대기` DB 원장을 추천, 공개배차, 만료, 확정, 수락취소 상태로 전환하고 결과 코드를 반환하는 지점 |
| `국내화물배차조율적용Service.추천잠금적용Async` | 추천 잠금을 DB `배차대기`에 트랜잭션으로 반영하는 지점 |
