...
이 문서는 음식배달과 국내 화물 운송에서 `큐`와 `DB`를 어떻게 나누고, 서로의
실행공간을 어떻게 격리할지 정리한다.
...
핵심 원칙은 단순하다. 업무 상태는 DB가 원장이고, 메모리의 사전, 해시셋,
sorted set, geo-index는 배차를 빠르게 하기 위한 실행 자료다. 서버가 내려가서
실행 인덱스가 사라져도, DB에 남아 있는 미처리 운송 의뢰와 기사 운행 상태를 다시
읽어 실행 인덱스를 재구성할 수 있어야 한다.

음식배달과 국내화물은 배달권이라는 비슷한 개념을 사용하지만 하나의 물리 실행공간을
공유하지 않는다. 공통 지역 Catalog와 `원장배달권투영`은 지역 사실과 원장 검색을
위한 참고 투영일 뿐이며, 실시간 기사·의뢰 후보 집합의 부모 Store가 아니다.
...
| 공통 기사 실행 인덱스 | `I국내화물운송기사상태Store`, `IDriverWorkQueueStore`, `IDriverLocationStore` | 운행중 기사, 최신 위치, Aging 점수와 반경 검색을 빠르게 처리한다 | DB와 heartbeat로 다시 만든다 |
| 음식배달 실행공간 | `I음식배달권실행공간Store`, 물리공간 `dispatch:food:v1` | Food 위치 셀별 FDriver와 음식배달 의뢰만 보관한다 | 음식 주문 원장과 FDriver heartbeat로 다시 만든다 |
| 국내화물 실행공간 | `I국내화물배달권실행공간Store`, 물리공간 `dispatch:cargo:v1` | 국내화물 권역별 화물기사와 화물의뢰만 보관한다 | 화물 운송 원장과 화물기사 heartbeat로 다시 만든다 |
...
1. `운송원장.배차업무유형`이 `음식배달` 또는 `용달운송`
2. `운송원장.상태 == 배차대기`
3. `운송원장.배차큐단계`가 `확정` 또는 `종료`가 아님
4. `운송원장.배차노출상태`가 `추천중`이 아니거나, 추천 만료 시각이 지남
5. 기사 후보는 `용달기사.운행상태`, 최근 `기사근무`, 위치 heartbeat 또는
   `기사위치기록`을 기준으로 복구하되, 근무의 실행유형에 맞는 한 공간에만 올림
...
| 음식배달 후보 묶음 | 음식 주문·음식배달 `운송원장`, FDriver 실행유형, 위치 heartbeat | `I음식배달권실행공간Store` 전용 사전/해시셋 |
| 국내화물 후보 묶음 | 국내화물 `운송원장`, 화물기사 실행유형, 위치 heartbeat | `I국내화물배달권실행공간Store` 전용 사전/해시셋 |
...
```mermaid
flowchart TD
    DB["DB 원장<br/>음식 주문·운송원장·화주운송의뢰"] --> Classify["배차업무유형 분류"]
    Classify --> FoodSpace["Food 물리공간<br/>dispatch:food:v1"]
    Classify --> CargoSpace["Cargo 물리공간<br/>dispatch:cargo:v1"]
    FoodHeartbeat["FDriver 음식배달 heartbeat"] --> FoodSpace
    CargoHeartbeat["화물기사 heartbeat"] --> CargoSpace
    FoodSpace --> FoodEngine["음식배달 엔진"]
    CargoSpace --> CargoEngine["국내화물 엔진"]
    FoodEngine --> Lock["DB 추천 잠금<br/>운송원장 추천중"]
    CargoEngine --> Lock
...
| `I음식배달권실행공간Store` | 음식배달 셀의 기사·의뢰만 보관하는 독립 실행공간 계약 |
| `I국내화물배달권실행공간Store` | 국내화물 권역의 기사·의뢰만 보관하는 독립 실행공간 계약 |
...
6. Food와 Cargo 실행공간은 공통 인터페이스로 합치지 않고, 서로의 Key 쓰기를
   거부한다. 공통화는 내부 자료구조 구현에만 한정한다.
