...
살뜰 서버는 Quartz 작업으로 KAMIS 국내 농수축산물 가격과 USDA NASS 미국 농작물·축산물·조사 대상 양식 수산물의 생산자 가격을 주기적으로 수집한다. 수집 서비스는 기존 관측값을 키 기준으로 다시 사용하거나 갱신하므로 같은 기간을 재실행해도 중복 행이 늘어나지 않는다. 각 실행 결과와 실패 내용은 농수산물 수집 실행 이력 테이블에 남는다.
...
| USDA NASS 월별 가격 | 매월 10일 07:30 (한국 시간) | 현재 연도와 직전 1년의 전국 생산자 수취가격 |
