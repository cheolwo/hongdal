...
살뜰 서버는 Quartz 작업으로 KAMIS 국내 농수산물 가격과 USDA NASS 미국 농산물 가격을 주기적으로 수집한다. 수집 서비스는 기존 관측값을 키 기준으로 다시 사용하거나 갱신하므로 같은 기간을 재실행해도 중복 행이 늘어나지 않는다. 각 실행 결과와 실패 내용은 농수산물 수집 실행 이력 테이블에 남는다.
