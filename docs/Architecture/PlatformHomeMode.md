...
| 모드 | 주 책임 | 종료·이동 |
| --- | --- | --- |
| 공통 홈 요약 | 실시간 베스트·커뮤니티·상점 핵심 현황 | 게시판, 글쓰기, 상점 또는 역할별 내비게이션으로 이동 |
| 커뮤니티 | 게시판·피드·글쓰기·원장 초안 | 업무 또는 다이어그램으로 전환 |
| 업무 | 역할별 상태 요약과 업무 진입 | 목적별 라우트로 이동 |
| 다이어그램 | 원장 노드와 연결선 편집 | 커뮤니티로 돌아가 공유 |

통합 앱의 `/` 첫 본문은 공통 홈 요약으로 시작한다. 다이어그램, 원장 초안, 게시판 개설과 업무 허브는 첫 화면에 섞지 않고 사용자가 게시판·업무·다이어그램 모드로 들어간 뒤 표시한다. 커뮤니티 전용 웹 페이지처럼 `UseCompactHomeSummary`를 사용하지 않는 화면은 기존 상세 커뮤니티를 바로 렌더링한다.
