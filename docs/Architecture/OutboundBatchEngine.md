...
출고 배치 엔진은 주문이나 운송 요청을 받아 어떤 창고의 어떤 적재 위치에서 어떤 수량을 꺼낼지 결정하고, 그 결과를 출고 예약, 피킹 작업, 포장 작업으로 나누는 계획 계층이다.

단순히 피킹 작업을 만드는 모듈이 아니라, 단일 상품 주문과 복수 상품 주문, 단일 창고 출고와 복수 창고 분할 출고를 모두 판단하는 상위 흐름으로 둔다.
...
flowchart TD
    ...
    K --> L
    L --> M["출고 예약"]
    M --> N["창고별 OutboundBatch 생성"]
    N --> O["WarehousePickWave 생성"]
    O --> P["피킹 작업 생성"]
    P --> Q["포장 작업 생성"]
    Q --> R["배차 / 배송 / 재위탁 운송 연결"]
```
...
| Service | `WarehousePickingPlanner` | 확정된 창고별 계획을 피킹 wave/task로 변환 |
| Domain | `OutboundReservation` | 출고 예약 상태 |
| Domain | `OutboundBatch` | 창고별 작업 묶음 |
| Domain | `SplitShipment` | 복수 창고 또는 복수 배송 단위 정보 |

`WarehousePickingPlanner`는 출고 배치 엔진의 하위 컴포넌트로 본다. 즉, 엔진이 “어느 창고에서 무엇을 꺼낼지”를 정하고, 피킹 플래너가 “현장에서 어떤 순서와 단위로 꺼낼지”를 정한다.
