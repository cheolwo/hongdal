...
이 문서는 한국과 미국의 공식 행정구역 코드와 경계 자료를 농수산물 산지·가격 자료에 연결하고, 지도 마커를 선택해 지역 문화와 가격 근거를 살펴보게 하는 방법을 제안한다. 구현은 이 제안의 코드 체계, 개인정보 경계와 원천 연결 방식을 검토한 뒤 별도 기능 slice로 진행한다.
