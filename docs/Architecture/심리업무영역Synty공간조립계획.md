...
> 심리 영역·업무 영역, 광복기·암흑기, 카드·사건·WI의 공통 게임 의미는 [Ssalddel 게임 기획 통합 기준](게임기획통합기준.md)이 소유한다. 이 문서는 두 발전소와 업무 영역을 보유 Synty 자산으로 조립하는 공간·표현 세부안만 소유한다.
...
두 발전소의 첫 시각 검토는 회복·위협 × A/B/C × 기본·강화 상태의 12개 카드로 준비한다. Unity는 대표 3/4, 플레이어 눈높이, 출입구·동선, 탑다운 촬영과 입력·계획·Rendering Profile·이미지 hash를 묶어 서버 검토 대장으로 보낸다. 휴대폰의 `좋음`은 유망 후보 표시일 뿐 `ApprovedForSceneApply`가 아니며, PC Unity의 최종 hash 대조와 승인 영수증을 대신하지 않는다.

관리자 API, 주차 확인 잠금, 오프라인 대기열과 Unity 촬영 인계 계약은 [Synty 모바일 조합물 검토 계획](Synty모바일조합물검토계획.md)을 따른다. 신호 대기나 정체 중에는 사용하지 않고 합법적인 장소에 완전히 주차한 뒤에만 검토한다.
