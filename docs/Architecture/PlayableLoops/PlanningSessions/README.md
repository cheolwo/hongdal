...
- 현행 기획 계보: [기획 목차](../../../AI/PLANNING.md), [기존 기획 현행화 전수 목록](../../../AI/기존기획-현행화-전수목록-2026-09-02.md)
- 현행화 판정: `NonPlanningExcluded / RoutingIndex`; 이 문서는 저장 위치와 검색 경로를 소유하고 개별 플레이 약속은 각 `PLAN-*` 기획이 소유

이 폴더는 하나의 긴 대화 기록에 서로 다른 게임 구성요소를 계속 추가하지 않고, 질문의 주 상태 소유자에 따라 문답을 나누어 저장한다.

## 파일 기반 문답 검색

[문답 파일 데이터베이스 사용법](../../../../eng/planning-inquiries/README.md)을 먼저 따른다. 기존 원문은 그대로 두고 [생성 JSON 색인](../../../AI/generated/planning-inquiry-search.json)에서 질문 번호·의미 ID·주제·깊이·원문 위치를 검색한다. Q001~339 구현 원장, Q340~347 후속 기록과 번호 없는 체력/마나·건설 후속 문답을 함께 찾되 구현 승인은 합치지 않는다. 새 질문 전에 키워드 조회와 현재 결정 대조를 수행하고, 답변 반영 후 `Write`·`Validate`한다. 원문 소실과 실제 미답변은 구분한다.
...
- 현재 확정: [첫 플레이 체감 Q397](첫플레이체감/first-play-experience.inquiry.r1.md)의 첫 농사 목표 10~15분. [명상 Q398](플레이어내면명상/player-mind-meditation.inquiry.r1.md#q-398-선택형-집중-타이밍과-명상-성장)은 선택형 타이밍 성공→명상 성장 방향이며 세부 적용·실패 조건은 미정이다. 기존 승인 개발을 막지 않는다.

- 현재 확정 묶음: [LS-01 경관 구성](건물공간배치/landscape-composition.inquiry.r1.md) r2 — Q387~396 전체 추천안 채택(D362). 기존 강변 Farm 패턴을 유지하며 시야·재료·조작 판독·날씨/성장 표현을 구체화한다. 실제 후보·화면 승인은 별도다.
- 현재 검토 묶음: [FB-01 농사 생활·위임](검토묶음/FB-01-농사생활위임.md) r2 — Q378~386 및 Q268은 D357에서 수정·승인됐다. 초기 농사/위임과 후속 트럭 판매를 구분하며 기획 승인은 실제 구현·E 완료가 아니다.
...
| 약초·Recipe·조합 제작 | [약초 Recipe 제작 문답](약초Recipe제작/herbal-recipe-crafting.inquiry.r1.md) | Q-045~050, Q-061~064, Q-068~071, Q-131, Q-133, Q-142, Q-150, Q-157, Q-269~296, Q-340~347 | Q-347 확인 완료, 같은 파일 |
...
- 최신 확인 기획 문답 `Q-340~Q-347`은 약초 회복 폐루프의 1인칭 탐색·채집, 자산 준비와 첫 물 확보·냄비 운반/컵 음용 역할을 정밀화한 범위다. Q-347은 D-326으로 확인했다. 현재 Q-001~Q-339 구현 원장에는 아직 이관하지 않았으며, 승인 기획 revision과 원장 범위 확장 전에는 신규 WI 구현이나 E 승격 근거로 사용하지 않는다.
