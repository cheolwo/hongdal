...
가격 수집은 [농수산물 가격 수집 배치](AgriculturalFisheriesBatchJobs.md)가 담당한다. `PublishCommunityPriceBriefs`를 켜면 KAMIS 일별·USDA 월별 수집 성공 직후 게시까지 같은 파이프라인에서 수행한다. 이 가격 근거 작업은 [공동구매 수요·모집 OS](GroupPurchaseDemandOS.md)의 1.0 지원 배치 카탈로그에도 등록된다. 수집 직후 게시가 활성일 때는 같은 원천의 독립 Quartz 일정을 등록하지 않고, 비활성일 때만 아래 독립 일정을 조정 작업으로 사용한다.
