...
2. 인연 기록 기능이 켜진 Command만 익명화된 업무 접점을 `WorkRelationshipSnapshots`에 저장한다.
...
업무 로그는 공개 게시글, 자동 친구 관계, 추천 점수나 검색 순위가 아니다. `WorkRelationshipSnapshots.Enabled` 또는 Command별 부가 기능 설정이 꺼져 있거나 API가 실패하면 임의 sample 관계를 만들지 않고 빈 상태나 오류를 표시한다.

현재 실제 자동 기록 예시는 기사 배차 수락이다. 이 한 기록은 기사와 화주 양쪽에서 확인할 수 있다. 주문자 집단화와 창고 입고·검수·포장 등 나머지 Command는 각 상태 전이 후처리로 옮긴 뒤 같은 장에 순차적으로 포함해야 하며, 아직 기록되지 않은 업무를 화면에서 성공한 인연처럼 표현하지 않는다.
