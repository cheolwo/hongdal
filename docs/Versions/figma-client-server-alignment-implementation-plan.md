...
- 작성일: 2026-07-28
- 상태: 진행 중 · `05 Warehouse` 1C와 `07 Restaurant` 복구 핵심 구현
- Figma 기준:
...
| `04 Driver` | `04P1.01~06` 만료·재연결·세션 복구 | 서버 만료시각, 1초 countdown, 수락 차단, SignalR 자동 재연결, 30초 조회, 토큰 갱신이 있음 | 재연결 완료 즉시 원장 재조회와 연결 상태의 typed contract·통합 검증 필요 |
...
| `07 Restaurant` | `07P1.01~06` 재시작·복구·소유권·업무 재개 | 미처리 의미·최근 변경시각·paging, `OrderNo` upsert, typed 복구 출처, 재연결 즉시 조회와 30초 오류 복구까지 연결 | 인증된 실제 수신함 렌더, 앱 종료 중 주문·재연결·401·403을 포함한 통합 검증 필요 |
