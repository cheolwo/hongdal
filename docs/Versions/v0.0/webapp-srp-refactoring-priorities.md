...
`P1-2` 주문자 마트 주문 요청은 54줄 조립 shell과 접근·선택·상품·인증·작성·저장 영수증 component로 분리했다. 저장 뒤 URL의 같은 요청 ID를 재조회하고, 실패 재시도는 같은 멱등 ID를 유지하며 재고 예약·결제·이행 효과를 만들지 않는다. `P0-4` 공동구매 workspace는 현재 다른 변경과 겹치므로 해당 변경이 먼저 검증·commit될 때까지 보류한다. 다음 깨끗한 세로 단위는 `P1-3` 판매 `OrderFulfillment.razor`이며, 주문 목록·필터·정확한 선택 상세·수동 이행 Command를 운영 경계 안에서 분리한다.
