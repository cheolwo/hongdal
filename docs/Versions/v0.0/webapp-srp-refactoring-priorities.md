...
`P1-3` 판매 주문 이행은 61줄 조립 shell과 조회·Simulation 샘플·재고·입고·피킹·포장·알림 정책 component 및 독립 ViewModel로 분리했다. 첫 주문과 피킹 작업을 자동 선택하지 않고, 수동 Command 뒤 같은 로컬 Simulation 원장을 다시 읽으며 외부 주문·실재고·메시지·운송·결제·정산 효과를 만들지 않는다. `P0-4` 공동구매 workspace는 현재 다른 변경과 겹치므로 해당 변경이 먼저 검증·commit될 때까지 보류한다. 다음 깨끗한 세로 단위는 `P1-4` 판매 `ProductListings.razor`이며, 판매상품 선택·정확한 채널 계정·출품 초안·외부 실행 경계를 분리한다.
