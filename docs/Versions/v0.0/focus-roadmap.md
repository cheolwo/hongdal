...
- 공개 진입점은 **밀도 높은 전통 게시판**을 기본형으로 사용합니다. 큰 소개 카드보다 게시판 이름, 분류 탭, 글 목록과 글쓰기를 첫 화면에서 먼저 보여 줍니다.
- 기본 탐색 흐름은 `게시판 전체보기 -> 게시판별 글 목록 -> 글쓰기 -> 게시글 본문`입니다.
- 업무 연결, 공동 원장, 다이어그램과 게시판 개설은 공개 글 목록 아래에 이어 붙이지 않고 별도의 업무·원장 공간으로 이동합니다.
- 카드형 보기는 보조 표현으로 둘 수 있지만, 0.0 공개 커뮤니티의 최초 진입과 게시판별 기본 보기는 목록형을 유지합니다.
