...
| 기능 | 포함 여부 | 관련 앱/모듈 | 메모 |
| --- | --- | --- | --- |
| 커뮤니티 홈·게시판·글쓰기 | 선행 의존 | `Ssalddel.Ui.Common`, 역할별 앱 홈 | 0.0 범위이며 1.0에서 재구현하지 않음 |
| 공동 원장·다이어그램 | 선행 의존 | 원장 API, 다이어그램 작업공간 | 0.0 원장에서 운송 의뢰로 인계 |
| 참여 동의·신고·신뢰 기록 | 선행 의존 | 커뮤니티 API, 관리자 화면 | 0.0 게이트 통과가 1.0의 전제 |
| 국내 화물/용달 운송 의뢰 | 제한 포함 | `SsalddelApp`, `Ssalddel` | 샘플 원장을 이용한 기술 검증 |
...
| 통관/HS 데이터 | 보류 | `Ssalddel.WebApp`, `SsalddelAdmin` | 2.0 |
