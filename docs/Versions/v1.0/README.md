...
사람들이 커뮤니티에서 만나 필요한 일을 이야기하고, 공동 원장과 다이어그램으로 정리해 참여자가 직접 합의한 업무를 안전하게 기록하도록 돕습니다. 국내 화물/용달 운송은 이 구조를 검증하는 첫 번째 기술 흐름입니다.

## 현재 집중 원칙

1.0은 커뮤니티와 공동 원장을 먼저 안정화하는 최소 제품 버전입니다. 운송·창고·주문 기능은 커뮤니티에서 생긴 일을 구조화하는 업무 도구로 붙이며, 규제 대상이 될 수 있는 유상 배차·주선·정산은 실운영과 기술 검증을 분리합니다.

| 판단 질문 | 1.0 처리 |
| --- | --- |
| 게시글, 댓글, 모집, 신고, 신뢰 기록처럼 커뮤니티 형성에 직접 필요한가 | 1.0 최우선 작업 |
| 공동 원장, 다이어그램, 참여자, 상태, 선택적 증빙을 더 명확하게 남기는가 | 1.0 우선 작업 |
| 운송 상태·증빙 API를 샘플 데이터와 모의 흐름으로 검증하는가 | 기술 검증 범위로 포함 |
...
| 영역 | 내용 |
| --- | --- |
| 커뮤니티 | 게시글, 댓글, 모집, 신고, 익명 활동, 후기와 신뢰 기록 |
| 공동 원장 | 대화에서 원장 생성, 참여자·상태·증빙 공유, 다이어그램 구성 |
| 업무 도구 | 운송 의뢰, 상차·하차, 인수, 결제 상태를 샘플·모의 흐름으로 검증 |
| 관리자 | 커뮤니티 신고·노출, 원장 상태와 개발용 업무 흐름 확인 |
...
1.0의 첫 필수 화면은 통합 커뮤니티 홈, 게시판, 글쓰기, 공동 원장과 다이어그램입니다. [홍달 1.0 필수 페이지 기준](../../ProjectOverview/hongdal-v1-required-pages.md)의 화주·기사·관리자 화면은 원장 상태와 증빙이 끝까지 이어지는지 확인하는 운송 기술 검증 경로로 봅니다.

## 안정화 기준

- 커뮤니티 대화가 공동 원장과 다이어그램으로 이어진다.
- 참여자가 같은 원장의 상태와 선택적 증빙을 함께 확인할 수 있다.
- 운송 기술 검증에서는 의뢰·상차·하차·인수 상태가 샘플 데이터로 끝까지 이어진다.
- 실운영 유상 배차·주선·정산 기능은 명시적으로 비활성 상태를 유지한다.

실운영 경계는 [홍달 1.0 커뮤니티 중심 제품 원칙](../../Architecture/CommunityFirstV1Policy.md)을 따릅니다.
...
커뮤니티 중심 1.0에서는 실제 화물 배차 이용료와 주선료를 받지 않습니다. 기존 이용료 계산 코드는 정산 모델과 FakePG를 검증하는 샘플로만 유지하며 운영 청구에 사용하지 않습니다. 향후 요금 도입은 허가·제휴 구조, 계약 당사자, 세금계산과 환불 책임을 별도로 검토한 뒤 새 정책으로 결정합니다.
