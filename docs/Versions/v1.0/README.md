...
1.0에서는 국내 화물/용달 운송의 상태명을 먼저 고정합니다. 이후 버전의 창고, 통관, 공동 주문, 알뜰살뜰 마트 기능은 이 흐름을 보조하거나 참조할 수 있지만, 1.0 흐름을 필수 의존으로 깨면 안 됩니다.
...
차량별 기준운임은 `차량단가`의 `기본운임`, `Km당단가`, `최소운임`을 서버 기준으로 적용합니다. 거리 기준은 상차/하차 좌표가 있으면 `Directions5` 실제 운행거리를 우선 사용하고, 실제 경로를 확보하지 못하면 좌표 기반 도로거리 보정, 화주 입력 거리, 직선거리 순서로 내려갑니다. 결제예정금액이 이 기준운임보다 낮으면 의뢰 생성을 차단합니다.
