...
`/community/home`의 공개 마커에서 물류대행·운송대행·개별 주문으로 이동하는 Web 경로에는 개인정보 입력 전 수집·이용 동의 관문이 있습니다. 업무별 목적·항목·보유 기간·거부권을 구분해 표시하고, 필수 동의를 마치기 전에는 신청 양식을 렌더링하지 않습니다.

이 관문은 `application-privacy-consent-draft-2026-08-04` 화면 버전이며 운영 증적은 아닙니다. 서버 원장에 동의 버전·시각·범위를 저장하고 Command에서 검증하는 기능, 철회·파기 이력, 실제 위탁자·제3자 제공받는 자와 국외 이전 조건은 아직 연결되지 않았습니다. 이 항목들이 완료되기 전에는 UI 체크 상태만으로 운영 승인이나 법률 준수를 주장하지 않습니다.

신청 시작 시점에는 상대가 정해지지 않으므로 플랫폼 내부 신청 처리용 수집·이용과 제3자 제공·국외 이전을 포괄 동의로 합치지 않습니다. 실제 창고·운송사·판매자 또는 해외 처리자가 정해진 뒤 제공받는 자·목적·항목·기간이나 이전 국가·방법을 표시하고 별도 적법 근거를 확인해야 합니다.

English 화면은 FTC Act 제5조, COPPA, 적용 요건을 충족하는 사업자의 CCPA/CPRA 수집 시점 고지를 참고한 영문 검토안을 제공합니다. 화면 언어는 이용자 위치, 사업자 적용 요건, 업종, 주별 법률 또는 준거법을 판정하지 않습니다. 특히 미국 운영 전에는 실제 주별 적용 범위, 서비스 제공자 계약, 판매·공유 및 맞춤형 광고 정의, 소비자 권리 요청 절차와 보안 통제를 별도로 검토합니다. 현재 14세 이상 확인은 COPPA의 13세 미만 기준을 그대로 바꾼 법적 판정이 아니라, 부모 동의 workflow가 없는 상태에서 유지하는 더 엄격한 제품 제한입니다.
