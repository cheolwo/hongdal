...
/// <summary>출고예정 운송 전 검토 페이지의 기능 노출과 창고 업무 인증만 관리합니다.</summary>
public sealed class 창고출고예정검토PageViewModel : 창고PageViewModelBase
{
    private readonly WarehousePageAvailabilityService _availability;
    private bool _초기화됨;
    private bool _초기화중;
    private bool _기능사용가능;
    private string _기능안내 = "출고예정 검토 기능 상태를 확인하고 있습니다.";
    private string? _페이지오류메시지;

    public 창고출고예정검토PageViewModel(
        창고작업세션상태ViewModel 세션,
        창고로그인ViewModel 인증,
        WarehousePageAvailabilityService availability)
        : base(세션, 창고PageCodes.일반출고예정검토, "출고예정 운송 전 검토", 창고운영ProfileCodes.일반입출고)
    {
    ...
    public async Task<bool> 초기화Async(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var available = await _availability.GetOutboundPlanReviewAsync(cancellationToken);
            기능사용가능 = available.IsEnabled;
            기능안내 = available.Notice;
        ...
        {
            기능사용가능 = false;
            페이지오류메시지 = "출고예정 검토 기능 응답을 처리하지 못했습니다.";
        }
        finally
        {
            초기화중 = false;
