...
/// <summary>입고 검수 페이지의 기능 노출과 창고 업무 인증만 관리합니다.</summary>
public sealed class 창고입고검수PageViewModel : 창고PageViewModelBase
{
    private readonly WarehousePageAvailabilityService _페이지사용가능성;
    private bool _초기화됨;
    private bool _초기화중;
    private bool _기능사용가능;
    private string _기능안내 = "입고 검수 기능 상태를 확인하고 있습니다.";
    private string? _페이지오류메시지;

    public 창고입고검수PageViewModel(
        창고작업세션상태ViewModel 세션,
        창고로그인ViewModel 인증,
        WarehousePageAvailabilityService 페이지사용가능성)
        : base(세션, 창고PageCodes.일반입고, "입고 검수")
    {
        _페이지사용가능성 = 페이지사용가능성;
        this.인증 = 구성요소등록(인증);
    }
    ...
    public async Task<bool> 초기화Async(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var availability = await _페이지사용가능성.GetInboundInspectionAsync(cancellationToken);
        ...
        {
            기능사용가능 = false;
            페이지오류메시지 = "입고 검수 기능 응답을 처리하지 못했습니다.";
        }
        finally
        {
            초기화중 = false;
