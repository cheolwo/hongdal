...
public sealed class SampleWarehouseWorkEntryGateService : IWarehouseWorkEntryGateService
{
    ...
    [
        ...
        new("01077771111", "도심입고", HrDetailedRoleCodes.WarehouseInboundOperator, WarehouseWorkProcessCodes.MartInbound),
        new("01077772222", "도심보충", HrDetailedRoleCodes.WarehouseInventoryOperator, WarehouseWorkProcessCodes.MartReplenishment),
        new("01077773333", "도심피킹", HrDetailedRoleCodes.WarehouseDispatchOperator, WarehouseWorkProcessCodes.MartPicking),
        new("01077774444", "배달픽업", HrDetailedRoleCodes.WarehouseDispatchOperator, WarehouseWorkProcessCodes.MartDeliveryPickup),
        new(
            "01099998888",
            "최관리",
            ...
            WarehouseWorkProcessCodes.MartInbound,
            WarehouseWorkProcessCodes.MartReplenishment,
            WarehouseWorkProcessCodes.MartPicking,
            WarehouseWorkProcessCodes.MartDeliveryPickup)
    ];
    ...
    public Task<WarehouseWorkOperatorVerificationResult> VerifyAsync(string processCode, string phoneLastEightDigits, CancellationToken cancellationToken = default)
    {
        ...
        if (suffix.Length != 8)
        {
            return Task.FromResult(Deny("휴대폰 번호 뒤 8자리를 숫자로 입력해 주세요."));
        }
        ...
        if (!worker.ProcessCodes.Contains(normalizedProcess, StringComparer.OrdinalIgnoreCase))
        {
            return Task.FromResult(new WarehouseWorkOperatorVerificationResult(
                ...
                $"{worker.Name} 작업자는 이 물류 공정 역할을 부여받지 않았습니다."));
        }
    ...
    private static string NormalizeProcess(string processCode)
        => processCode switch
        {
            ...
            WarehouseWorkProcessCodes.MartInbound => WarehouseWorkProcessCodes.MartInbound,
            WarehouseWorkProcessCodes.MartReplenishment => WarehouseWorkProcessCodes.MartReplenishment,
            WarehouseWorkProcessCodes.MartPicking => WarehouseWorkProcessCodes.MartPicking,
            WarehouseWorkProcessCodes.MartDeliveryPickup => WarehouseWorkProcessCodes.MartDeliveryPickup,
