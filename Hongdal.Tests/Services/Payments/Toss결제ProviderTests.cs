...
namespace Hongdal.Tests.Services.Payments;
...
public sealed class Toss결제ProviderTests
{
    ...
    public async Task Simulation에서는_Toss_승인을_호출하지_않는다()
    {
        ...
        var provider = new Toss결제Provider(toss, CreatePolicy(HongdalExecutionMode.Simulation));
    ...
    {
        var toss = new 기록용TossPaymentsService();
        var provider = new Toss결제Provider(toss, CreatePolicy(HongdalExecutionMode.Operational));
    ...
    private static IHongdalExecutionModePolicy CreatePolicy(HongdalExecutionMode mode)
        => new HongdalExecutionModePolicy(Options.Create(new HongdalExecutionOptions { Mode = mode }));
