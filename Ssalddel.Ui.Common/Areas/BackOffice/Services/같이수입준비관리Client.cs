...
public interface I같이수입준비관리Client
{
    ...
    Task<같이수입준비Os상태응답?> 준비Os상태조회Async(
        string 자동집단Id,
        CancellationToken cancellationToken = default);

    Task<같이수입준비Os상태응답> 준비Os작업실행Async(
        string 자동집단Id,
        같이수입준비Os작업실행요청 요청,
        CancellationToken cancellationToken = default);
...
[SsalddelCodeMetadata(
    ...
    "두 관리자 앱에서 1.0 인계 상태와 기존 같이 수입 원장의 1.5 공급·가격·무역 준비 블록을 같은 API 계약으로 조회하고 저장합니다.",
...
public sealed class 같이수입준비관리Client(ISsalddelJsonApiClient client) : I같이수입준비관리Client
{
    ...
    public Task<같이수입준비원장응답?> 준비원장조회Async(
        string 자동집단Id,
        CancellationToken cancellationToken = default)
        => client.GetAsync<같이수입준비원장응답>(
            ReadinessPath(자동집단Id),
            "같이 수입 원장 1.5 준비 블록 조회",
            allowNotFound: true,
            cancellationToken);
    ...
    public async Task<같이수입준비원장응답> 미리보기Async(
        ...
        CancellationToken cancellationToken = default)
    {
        var result = await client.SendAsync<같이수입준비원장저장요청, 같이수입준비원장응답>(
            HttpMethod.Post,
            $"{ReadinessPath(자동집단Id)}/preview",
            요청,
            "같이 수입 원장 1.5 준비 블록 미리보기",
            cancellationToken: cancellationToken);
        return result ?? throw new InvalidOperationException("같이 수입 원장 1.5 준비 블록 미리보기 응답이 비어 있습니다.");
    }
    ...
    {
        ArgumentNullException.ThrowIfNull(요청);
        var result = await client.SendWithHeadersAsync<같이수입준비원장저장요청, 같이수입준비원장응답>(
            HttpMethod.Put,
            ReadinessPath(자동집단Id),
            요청,
            IdempotencyHeader(요청.요청멱등키),
            "같이 수입 원장 1.5 준비 블록 저장",
            cancellationToken: cancellationToken);
        return result ?? throw new InvalidOperationException("같이 수입 원장 1.5 준비 블록 저장 응답이 비어 있습니다.");
    }

    public Task<같이수입준비Os상태응답?> 준비Os상태조회Async(
        string 자동집단Id,
        CancellationToken cancellationToken = default)
        => client.GetAsync<같이수입준비Os상태응답>(
            $"{ReadinessPath(자동집단Id)}/os",
            "1.5 같이 수입 준비 OS 상태 조회",
            allowNotFound: true,
            cancellationToken);

    public async Task<같이수입준비Os상태응답> 준비Os작업실행Async(
        string 자동집단Id,
        같이수입준비Os작업실행요청 요청,
        CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(요청);
        var result = await client.SendWithHeadersAsync<같이수입준비Os작업실행요청, 같이수입준비Os상태응답>(
            HttpMethod.Post,
            $"{ReadinessPath(자동집단Id)}/os/workloads/run",
            요청,
            IdempotencyHeader(요청.요청멱등키),
            "1.5 같이 수입 준비 OS 작업 실행",
            cancellationToken: cancellationToken);
    ...
    {
        ArgumentNullException.ThrowIfNull(요청);
        var result = await client.SendWithHeadersAsync<같이수입준비Os전문검토인계요청, 같이수입준비Os상태응답>(
            HttpMethod.Post,
