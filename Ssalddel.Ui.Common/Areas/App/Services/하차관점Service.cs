...
public interface I하차관점Service
{
    Task<하차관점페이지응답> 목록조회Async(
...
public sealed class 하차관점Service(ISsalddelJsonApiClient client) : I하차관점Service
{
    private const string BasePath = "api/v1/unloading-perspectives";

    public async Task<하차관점페이지응답> 목록조회Async(
        string perspectiveCode,
        string? communityLedgerId,
        하차관점목록조회요청 request,
        CancellationToken cancellationToken = default)
    {
        ...
        {
            하차업무관점코드.주문자 => $"{BasePath}/orderer",
            하차업무관점코드.판매자 => $"{BasePath}/seller",
            하차업무관점코드.창고관리자 => $"{BasePath}/warehouse",
            하차업무관점코드.운송담당자 => $"{BasePath}/transport",
            하차업무관점코드.공동원장 when !string.IsNullOrWhiteSpace(communityLedgerId)
                => $"{BasePath}/community-ledgers/{Uri.EscapeDataString(communityLedgerId.Trim())}",
            하차업무관점코드.공동원장 => throw new InvalidOperationException("조회할 공동 원장을 선택해 주세요."),
            _ => throw new InvalidOperationException($"지원하지 않는 하차 관점입니다: {perspectiveCode}")
        };
        ...
        return await client.GetAsync<하차관점페이지응답>(
