...
public sealed class SsalddelDocumentOutputService(ISsalddelIdentifierCodeGenerator identifierCodeGenerator) : ISsalddelDocumentOutputService
{
    public SsalddelDocumentOutput CreateWaybill(SsalddelWaybillDocumentDraft draft)
    {
        var title = $"운송장 {draft.DocumentNo}";
        var html = BuildWaybillHtml(draft, title);
        var plainText = BuildPlainText(draft);

        return new SsalddelDocumentOutput(
            $"ssalddel-waybill-{SanitizeFileName(draft.DocumentNo)}.html",
            title,
    ...
    public SsalddelDocumentOutput CreateInboundExpectedItems(SsalddelExpectedItemDocumentDraft draft)
    {
        return CreateExpectedItems(
            draft,
            SsalddelExpectedItemDocumentKindCode.InboundExpectedItems,
            "입고예정 품목표",
            "inbound-expected-items",
            SsalddelIdentifierKindCode.InboundRequest);
    }

    public SsalddelDocumentOutput CreateOutboundExpectedItems(SsalddelExpectedItemDocumentDraft draft)
    {
        return CreateExpectedItems(
            draft,
            ...
            SsalddelIdentifierKindCode.OutboundPlan);
    }

    private SsalddelDocumentOutput CreateExpectedItems(
        SsalddelExpectedItemDocumentDraft draft,
        string documentKind,
        ...
        string fallbackBarcodeKind)
    {
        ...
        var title = string.IsNullOrWhiteSpace(draft.Title)
            ...
            : draft.Title;
        var normalizedDraft = new SsalddelExpectedItemDocumentDraft
        {
            ...
            DocumentBarcodePayload = draft.DocumentBarcodePayload
                ?? SsalddelIdentifierCodePayloads.Create(fallbackBarcodeKind, draft.DocumentNo),
            Lines = draft.Lines
        };
        ...
        return new SsalddelDocumentOutput(
            $"ssalddel-{fileKind}-{SanitizeFileName(draft.DocumentNo)}.html",
            title,
            "text/html;charset=utf-8",
            html,
            plainText);
    ...
    private static string BuildWaybillHtml(SsalddelWaybillDocumentDraft draft, string title)
    {
        ...
        html.AppendLine("<div><div class=\"title\">살뜰 운송장</div><div>운송 접수 및 배차 확인용</div></div>");
    ...
    private string BuildExpectedItemsHtml(SsalddelExpectedItemDocumentDraft draft, string fallbackTitle)
    {
        var layout = ResolvePrintLayout(draft.PrintLayout);
        var documentPayload = draft.DocumentBarcodePayload
            ?? SsalddelIdentifierCodePayloads.Create(SsalddelIdentifierKindCode.Unknown, draft.DocumentNo);
        ...
        html.AppendLine("<div>Ssalddel WebAssembly document output</div>");
        html.AppendLine("</section>");
        html.AppendLine("</main>");
        html.AppendLine("</body>");
        html.AppendLine("</html>");
    ...
    private string BuildExpectedItemsReceiptSlipHtml(
        SsalddelExpectedItemDocumentDraft draft,
        string fallbackTitle,
        SsalddelIdentifierCodePayload documentPayload,
        SsalddelPrintLayoutProfile layout)
    {
        ...
        html.AppendLine("<div>Ssalddel order slip</div>");
        html.AppendLine("</section>");
        html.AppendLine("</main>");
        html.AppendLine("</body>");
        html.AppendLine("</html>");
    ...
    private void AppendReceiptDocumentCode(StringBuilder html, SsalddelIdentifierCodePayload payload, SsalddelPrintLayoutProfile layout)
    {
        ...
        {
            var qr = identifierCodeGenerator.Generate(new SsalddelIdentifierCodeImageRequest(
                payload,
                SsalddelMachineReadableCodeFormatCode.QrCode,
        ...
        var barcode = identifierCodeGenerator.Generate(new SsalddelIdentifierCodeImageRequest(
            payload,
            SsalddelMachineReadableCodeFormatCode.Code128,
    ...
    private void AppendReceiptItemLine(StringBuilder html, SsalddelExpectedItemDocumentLine line, SsalddelPrintLayoutProfile layout)
    {
    ...
    private void AppendDocumentCode(StringBuilder html, SsalddelIdentifierCodePayload payload, SsalddelPrintLayoutProfile layout)
    {
        SsalddelIdentifierCodeImage? qr = null;
        if (layout.IncludeDocumentQrCode)
        {
            qr = identifierCodeGenerator.Generate(new SsalddelIdentifierCodeImageRequest(
                payload,
                SsalddelMachineReadableCodeFormatCode.QrCode,
                layout.DocumentQrSizePx,
                layout.DocumentQrSizePx,
    ...
    private void AppendExpectedItemTable(StringBuilder html, IReadOnlyList<SsalddelExpectedItemDocumentLine> lines, SsalddelPrintLayoutProfile layout)
    {
        ...
        foreach (var line in lines)
        {
            ...
            var barcode = layout.IncludeLineBarcodes
                ? identifierCodeGenerator.Generate(new SsalddelIdentifierCodeImageRequest(
                    payload,
                    SsalddelMachineReadableCodeFormatCode.Code128,
                    layout.LineBarcodeWidthPx,
                    layout.LineBarcodeHeightPx,
    ...
    private static SsalddelIdentifierCodePayload ResolveLineBarcodePayload(SsalddelExpectedItemDocumentLine line)
    {
        ...
        if (!string.IsNullOrWhiteSpace(line.ProductBarcode))
        {
            return line.ProductBarcode.Contains(':', StringComparison.Ordinal)
                ? SsalddelIdentifierCodePayloads.Parse(line.ProductBarcode)
                : SsalddelIdentifierCodePayloads.Create(SsalddelIdentifierKindCode.Product, line.ProductBarcode);
        }

        if (!string.IsNullOrWhiteSpace(line.BundleBarcode))
        {
            return line.BundleBarcode.Contains(':', StringComparison.Ordinal)
                ? SsalddelIdentifierCodePayloads.Parse(line.BundleBarcode)
                : SsalddelIdentifierCodePayloads.Create(SsalddelIdentifierKindCode.Bundle, line.BundleBarcode);
        }

        return SsalddelIdentifierCodePayloads.Create(
            SsalddelIdentifierKindCode.Product,
            string.IsNullOrWhiteSpace(line.Sku) ? line.LineNo : line.Sku);
    }

    private static SsalddelPrintLayoutProfile ResolvePrintLayout(SsalddelDocumentPrintLayoutOptions? options)
    {
        ...
        var orientation = IsThermalPaperSize(paperSize)
            ? SsalddelDocumentOrientationCode.Portrait
        ...
        return new SsalddelPrintLayoutProfile(
            paperSize,
            density,
            orientation,
            printStyle,
    ...
    private static string NormalizePaperSize(string? value)
    {
        ...
        {
            "a5" => SsalddelDocumentPaperSizeCode.A5,
            "a6" => SsalddelDocumentPaperSizeCode.A6,
            "thermal-80" or "receipt-80" or "80mm" => SsalddelDocumentPaperSizeCode.Thermal80,
            "thermal-58" or "receipt-58" or "58mm" => SsalddelDocumentPaperSizeCode.Thermal58,
            _ => SsalddelDocumentPaperSizeCode.A4
        };
    }
    ...
    private static string NormalizeDensity(string? value)
    {
        return (value ?? string.Empty).Trim().ToLowerInvariant() switch
        {
            "comfortable" or "normal" => SsalddelDocumentDensityCode.Comfortable,
            "dense" => SsalddelDocumentDensityCode.Dense,
            _ => SsalddelDocumentDensityCode.Compact
        };
    }

    private static string NormalizeOrientation(string? value)
    {
        return string.Equals(value, SsalddelDocumentOrientationCode.Landscape, StringComparison.OrdinalIgnoreCase)
            ? SsalddelDocumentOrientationCode.Landscape
            : SsalddelDocumentOrientationCode.Portrait;
    }
    ...
    private static string NormalizePrintStyle(string? value, string paperSize)
    {
        ...
        {
            return SsalddelDocumentPrintStyleCode.ReceiptSlip;
        }

        return string.Equals(value, SsalddelDocumentPrintStyleCode.ReceiptSlip, StringComparison.OrdinalIgnoreCase)
            ? SsalddelDocumentPrintStyleCode.ReceiptSlip
            : SsalddelDocumentPrintStyleCode.Sheet;
    }

    private static bool IsThermalPaperSize(string paperSize)
    {
        return paperSize is SsalddelDocumentPaperSizeCode.Thermal80 or SsalddelDocumentPaperSizeCode.Thermal58;
    }
    ...
    private static string ResolvePageSizeCss(string paperSize, string orientation)
    {
        ...
        {
            SsalddelDocumentPaperSizeCode.A5 => $"A5 {orientation}",
            SsalddelDocumentPaperSizeCode.A6 => $"A6 {orientation}",
            SsalddelDocumentPaperSizeCode.Thermal80 => "80mm 200mm",
            SsalddelDocumentPaperSizeCode.Thermal58 => "58mm 200mm",
            _ => $"A4 {orientation}"
        };
    }
    ...
    private static (string SheetWidth, string SheetMinHeight, int MetaColumns) ResolveSheetSize(string paperSize, string orientation)
    {
        ...
        {
            (SsalddelDocumentPaperSizeCode.A4, SsalddelDocumentOrientationCode.Landscape) => ("277mm", "180mm", 6),
            (SsalddelDocumentPaperSizeCode.A5, SsalddelDocumentOrientationCode.Landscape) => ("190mm", "128mm", 4),
            (SsalddelDocumentPaperSizeCode.A5, _) => ("128mm", "190mm", 3),
            (SsalddelDocumentPaperSizeCode.A6, SsalddelDocumentOrientationCode.Landscape) => ("128mm", "88mm", 3),
            (SsalddelDocumentPaperSizeCode.A6, _) => ("88mm", "128mm", 2),
            (SsalddelDocumentPaperSizeCode.Thermal80, _) => ("72mm", "auto", 1),
            (SsalddelDocumentPaperSizeCode.Thermal58, _) => ("50mm", "auto", 1),
            _ => ("190mm", "267mm", 4)
        };
    }

    private static SsalddelDensityCss ResolveDensityCss(string density, string paperSize)
    {
        if (paperSize == SsalddelDocumentPaperSizeCode.Thermal58)
        {
            return new(
                "3mm",
                "2mm",
        ...
        if (paperSize == SsalddelDocumentPaperSizeCode.Thermal80)
        {
            return new(
                "4mm",
                "2mm",
        ...
        {
            SsalddelDocumentDensityCode.Comfortable => new(
                "10mm",
                "12mm",
                "8px",
                "14px",
                ...
                240,
                70),
            SsalddelDocumentDensityCode.Dense => new(
                "5mm",
                "5mm",
                "4px",
                "6px",
    ...
    private static string BuildPlainText(SsalddelWaybillDocumentDraft draft)
    {
    ...
    private static string BuildExpectedItemsPlainText(SsalddelExpectedItemDocumentDraft draft)
    {
...
internal sealed record SsalddelPrintLayoutProfile(
    string PaperSize,
    string Density,
    string Orientation,
    string PrintStyle,
...
internal sealed record SsalddelDensityCss(
    string PageMargin,
    string SheetPadding,
    string Gap,
    string HeadGap,
...
public static class SsalddelDocumentOutputServiceCollectionExtensions
{
    public static IServiceCollection AddSsalddelDocumentOutputServices(this IServiceCollection services)
    {
        services.TryAddSingleton<ISsalddelIdentifierCodeGenerator, ZxingSsalddelIdentifierCodeGenerator>();
        services.AddSingleton<ISsalddelDocumentOutputService, SsalddelDocumentOutputService>();
        return services;
    }
}
