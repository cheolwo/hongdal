...
{
    public const string ConfigurationKey = "SsalddelApiBaseAddress";
    ...
    public static Uri ResolveBaseAddress(string? configuredBaseAddress, Uri? fallback = null)
    {
        ...
        if (!Uri.TryCreate(configuredBaseAddress.Trim(), UriKind.Absolute, out var configuredUri))
        {
            throw new ArgumentException(
                "The Ssalddel API base address must be an absolute HTTP(S) URI.",
    ...
    public static Uri NormalizeBaseAddress(Uri baseAddress)
    {
        ...
        {
            throw new ArgumentException(
                "The Ssalddel API base address must be an absolute HTTP(S) URI without a query or fragment.",
                nameof(baseAddress));
        }
