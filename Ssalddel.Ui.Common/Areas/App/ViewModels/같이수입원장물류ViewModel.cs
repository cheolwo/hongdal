...
public sealed record 같이수입물류경로옵션(
    string 코드,
    string 이름,
    string 설명);

/// <summary>
/// 같이 수입 원장과 그 아래 국제운송·국내운송·입고·출고 원장을
/// 사용자가 선택한 물류 경로에 맞춰 계획하고 생성합니다.
/// </summary>
public sealed class 같이수입원장물류ViewModel : 공동구매작업ViewModelBase, IDisposable
{
    private readonly I같이수입원장전환Client _client;
    ...
    public 같이수입원장물류ViewModel(
        I같이수입원장전환Client client,
    ...
    public static IReadOnlyList<같이수입물류경로옵션> 경로옵션 { get; } =
    [
        new(
    ...
    public bool 전환가능 => _분기.같이수입활성 && 계획?.Ready == true;
    ...
    public string 선택한경로안내
        => 경로옵션.FirstOrDefault(option =>
           ...
           ?? "같이 수입 물류 경로를 선택해 주세요.";

    public bool 물류경로선택(string routeCode)
    {
        if (!_분기.같이수입활성)
        {
            return 유효성실패("같이 수입 분기가 활성화된 경우에만 수입 물류 경로를 선택할 수 있습니다.");
        }

        if (!CommunityGroupImportLogisticsRouteCodes.All.Contains(routeCode))
        {
            return 유효성실패("지원되는 같이 수입 물류 경로를 선택해 주세요.");
        }
    ...
    public async Task<bool> 서버미리보기Async(CancellationToken cancellationToken = default)
    {
        ...
        {
            return 유효성실패("같이 수입 물류 경로를 계획할 공동구매를 선택해 주세요.");
        }
        if (!_분기.같이수입활성)
        {
            return 유효성실패("해외 출발·국내 반입 같이 수입에서만 이 물류 계획을 사용할 수 있습니다.");
        }
        ...
        return await 작업실행Async(
            async token =>
            {
                계획 = await _client.미리보기Async(campaignId.Value, 초안, token)
                    ?? throw new InvalidOperationException("같이 수입 물류 경로 미리보기 응답이 비어 있습니다.");
            },
            "선택한 같이 수입 물류 경로를 확인했습니다.",
            cancellationToken);
    }
    ...
    public async Task<bool> 기존원장조회Async(CancellationToken cancellationToken = default)
    {
        ...
        if (campaignId is null)
        {
            return 유효성실패("같이 수입 원장을 조회할 공동구매를 선택해 주세요.");
        }
    ...
    public async Task<bool> 같이수입원장전환Async(CancellationToken cancellationToken = default)
    {
        var campaignId = _화면상태.선택된공동구매Id;
        if (campaignId is null)
        {
            return 유효성실패("같이 수입 원장으로 전환할 공동구매를 선택해 주세요.");
        }
        if (!_분기.같이수입활성)
        {
            return 유효성실패("해외 출발·국내 반입으로 판정된 공동구매만 같이 수입 원장으로 전환할 수 있습니다.");
        }
        ...
        return await 작업실행Async(
            async token =>
            {
                저장된원장 = await _client.전환Async(campaignId.Value, 초안, token)
                    ?? throw new InvalidOperationException("같이 수입 원장 전환 응답이 비어 있습니다.");
                계획 = 저장된원장;
                await _화면상태.단계도달Async(
                    공동구매절차코드.실행,
                    $"같이 수입 원장 {저장된원장.GroupImportLedgerId}으로 인계했습니다.",
                    token);
            },
            "같이 수입 원장과 선택한 하위 물류 원장을 생성했습니다.",
            cancellationToken);
    }
