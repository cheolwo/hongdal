...
public sealed record 같이수입선적단계옵션(string 코드, string 표시명);

public sealed class 같이수입선적초안ViewModel : 업무입력ViewModelBase
{
...
public sealed class 같이수입선적이벤트초안ViewModel : 업무입력ViewModelBase
{
...
public sealed class 같이수입통관동기화초안ViewModel : 업무입력ViewModelBase
{
    private string _문서관리번호 = string.Empty;
...
/// <summary>
/// 같이 수입 원장 뒤의 해외 선적, 이벤트, 통관 동기화를 한 흐름으로 관리합니다.
/// 공개 조회는 일반 사용자용이며 저장·동기화 메서드는 서버 관리자 권한 API가 최종 통제합니다.
/// </summary>
public sealed class 같이수입선적통관ViewModel : 공동구매작업ViewModelBase, IDisposable
{
    private readonly I같이수입선적통관Client _client;
    private readonly 공동구매화면상태ViewModel _화면상태;
    private readonly 공동구매거래경로분기ViewModel _분기;
    private 같이수입원장물류ViewModel? _원장물류;
    ...
    public 같이수입선적통관ViewModel(
        I같이수입선적통관Client client,
        ...
        공동구매거래경로분기ViewModel 분기)
    {
        ...
        선적초안 = new 같이수입선적초안ViewModel();
        이벤트초안 = new 같이수입선적이벤트초안ViewModel();
        통관초안 = new 같이수입통관동기화초안ViewModel();
    ...
    public static IReadOnlyList<같이수입선적단계옵션> 선적단계 { get; } =
    [
    ...
    public 같이수입선적초안ViewModel 선적초안 { get; }
    public 같이수입선적이벤트초안ViewModel 이벤트초안 { get; }
    public 같이수입통관동기화초안ViewModel 통관초안 { get; }
    ...
    public string 다음작업안내
        => !활성
            ? "같이 수입으로 판정된 공동구매에서 선적·통관 흐름을 사용할 수 있습니다."
            : !원장준비완료
                ? "같이 수입 원장을 먼저 생성한 뒤 해외 선적 문서를 등록해 주세요."
                : 현재선적 is null
    ...
    public void 원장물류연결(같이수입원장물류ViewModel 원장물류)
    {
    ...
    public async Task<bool> 관리자선적저장Async(CancellationToken cancellationToken = default)
    {
        if (!원장준비완료)
        {
            return 유효성실패("같이 수입 원장을 생성한 뒤 선적 문서를 저장할 수 있습니다.");
        }
        if (!선적초안.전체검증())
        {
        ...
        return await 작업실행Async(
            async token =>
            {
                현재선적 = await _client.관리자저장Async(선적초안.요청생성(), token)
                    ?? throw new InvalidOperationException("같이 수입 선적 저장 응답이 비어 있습니다.");
                선적초안.초기화(_화면상태.선택된공동구매, 현재선적);
                통관초안.초기화(현재선적.문서관리번호);
            },
            "해외 선적 문서를 같이 수입 원장 흐름에 연결했습니다.",
            cancellationToken);
    }
    ...
    public async Task<bool> 관리자이벤트추가Async(CancellationToken cancellationToken = default)
    {
        ...
        {
            return 유효성실패("같이 수입 원장과 선적 문서를 먼저 준비해 주세요.");
        }
        if (!이벤트초안.전체검증())
        {
        ...
        return await 작업실행Async(
            async token =>
            {
                현재선적 = await _client.관리자이벤트추가Async(
                    현재선적.문서관리번호,
                    이벤트초안.요청생성(),
                    token) ?? throw new InvalidOperationException("같이 수입 선적 이벤트 응답이 비어 있습니다.");
                선적초안.초기화(_화면상태.선택된공동구매, 현재선적);
                이벤트초안.초기화();
            },
        ...
        return await 작업실행Async(
            async token =>
            {
                통관결과 = await _client.관리자통관동기화Async(통관초안.요청생성(), token)
                    ?? throw new InvalidOperationException("같이 수입 통관 동기화 응답이 비어 있습니다.");
