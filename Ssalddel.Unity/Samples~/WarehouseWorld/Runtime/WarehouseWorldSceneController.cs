...
namespace Ssalddel.Unity.Samples.WarehouseWorld
{
    public sealed class WarehouseWorldSceneController : MonoBehaviour
    {
        ...
        private WarehouseWorldLoadCoordinator coordinator = null!; private WarehousePresenter presenter = null!; private CancellationTokenSource lifetime = null!; private Task? inFlight;
        [Inject] public void Construct(WarehouseWorldLoadCoordinator loadCoordinator, WarehousePresenter warehousePresenter, WarehouseWorldView view) { coordinator = loadCoordinator; presenter = warehousePresenter; warehouseView = view; }
        ...
        private async Task LoadAsync()
        {
            ...
            warehouseView.Render(presenter.Present(await coordinator.LoadAsync(warehouseId, lifetime.Token)));
        }
