...
- `도심마트ManagerRuntime`의 shelf·task·source-plan·detail change set을 적용하는 관리자 View
- 관리자 shelf 선택을 World selection으로 되돌리고 주기적 refresh에서 last-success를 유지하는 Controller
...
관리자 surface sample은 Unity 메뉴 `Ssalddel/Samples/Create Urban Market Manager Primitive Scene`에서 별도 Scene으로 만든다. 이 Scene은 manager role이 승인된 `AuthorizedUserWorld` Simulation context와 두 진열대의 상태·task·SourcePlan·detail surface를 사용한다. 관리자 우선순위 점수나 업무 queue는 만들지 않으며 기존 공개 상품 Scene을 덮어쓰지 않는다.
