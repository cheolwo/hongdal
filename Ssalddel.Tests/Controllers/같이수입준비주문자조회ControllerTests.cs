...
public sealed class 같이수입준비주문자조회ControllerTests
{
    [Fact]
    public void Controller는_인증과1_5기능플래그로보호된_Get조회만제공한다()
    {
        var type = typeof(같이수입준비주문자조회Controller);
    ...
    public async Task 로그인주문자의식별자를_조회UseCase에전달한다()
    {
        ...
        var response = new 같이수입준비주문자조회응답 { 상품명 = "쌀" };
        var useCase = new RecordingUseCase(response);
        var controller = new 같이수입준비주문자조회Controller(useCase)
        {
    ...
    private sealed class RecordingUseCase(같이수입준비주문자조회응답? response)
        : I같이수입준비주문자조회UseCase
    {
        ...
        {
            LastRequest = (같이수입원장Id, 자동집단Id, 주문자UserId);
