...
public sealed class 커뮤니티게시글식재료가격참고ControllerTests
{
    [Fact]
    public void 가격힌트API는_익명읽기전용_커뮤니티0점0경계를명시한다()
    {
        var type = typeof(커뮤니티게시글식재료가격참고Controller);
    ...
    {
        var controller = new 커뮤니티게시글식재료가격참고Controller(
            new ThrowingService());

        var result = await controller.가격참고조회(
