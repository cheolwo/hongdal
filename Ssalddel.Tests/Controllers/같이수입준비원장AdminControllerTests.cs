...
public sealed class 같이수입준비원장AdminControllerTests
{
    [Fact]
    public void Controller는_관리자와1_5기능플래그로보호된다()
    {
        var type = typeof(같이수입준비원장AdminController);
    ...
    public void 원장과Os_계약_Api_Application은_같은1_5기능메타데이터를가진다()
    {
        var metadata = new[]
        {
            typeof(같이수입준비원장저장요청),
            typeof(같이수입준비Os상태응답),
            typeof(같이수입준비원장AdminController),
            typeof(같이수입준비원장Service),
            typeof(같이수입준비ProcessManager)
        }
    ...
    {
        var methods = typeof(같이수입준비원장AdminController).GetMethods();

        Assert.Equal(
            "os",
            methods.Single(method => method.Name == nameof(같이수입준비원장AdminController.Os상태조회))
                .GetCustomAttribute<HttpGetAttribute>()?.Template);
        Assert.Equal(
            "os/workloads/run",
            methods.Single(method => method.Name == nameof(같이수입준비원장AdminController.Os작업실행))
                .GetCustomAttribute<HttpPostAttribute>()?.Template);
        Assert.Equal(
            "os/qualified-review-handoff",
            methods.Single(method => method.Name == nameof(같이수입준비원장AdminController.전문검토인계))
                .GetCustomAttribute<HttpPostAttribute>()?.Template);
    }
}
