...
{
    [Fact]
    public void 커뮤니티01은_업무앱의인연기록을확인하는독립Route를제공한다()
    {
        var routes = Read(
            "Ssalddel.Contracts",
            "Common",
        ...
        Assert.Contains("업무에서 이어진 인연", layout);
    }

    [Fact]
    public void 업무인연장은_익명기록과별도동의를표현하고_상대식별자를렌더하지않는다()
    {
        var component = Read(
            "Ssalddel.Ui.Common",
            "Areas",
    ...
    [Fact]
    public async Task 연결가능한업무인연은_명시적목적과메시지로만요청하고_완료상태를남긴다()
    {
        ...
        var viewModel = new 업무인연커뮤니티ViewModel(service);

        Assert.True(await viewModel.조회Async());
        Assert.True(viewModel.연결요청가능(snapshot));
        Assert.True(await viewModel.연결요청Async(snapshot, "다음 운송 제안", "함께 일해서 감사했습니다."));
        ...
        Assert.True(viewModel.연결요청완료(snapshot.Id));
        Assert.False(viewModel.연결요청가능(snapshot));
    }
    ...
    private sealed class FakeWorkRelationshipCommunityService(
        WorkRelationshipSnapshotResponse snapshot) : I업무인연커뮤니티Service
    {
        ...
        public Task<WorkRelationshipSnapshotListResponse> 내업무인연조회Async(
            int take = 50,
