...
public sealed class 같이수입준비관리ViewModelTests
{
    [Fact]
    public async Task 초기화는_인계대상을선택하고_1_5체크리스트초안을구성한다()
    {
        var client = new FakeClient();
        var viewModel = new 같이수입준비관리ViewModel(client);
        ...
        Assert.Equal(
            [같이수입준비국제운송방식코드.Lcl, 같이수입준비국제운송방식코드.Fcl],
    ...
    public async Task 승인된수요재료를_같은준비묶음에추가하고_포워더인계와회신을별도로기록한다()
    {
        ...
        viewModel.포워더인계상태변경(같이수입준비포워더인계상태코드.인계기록됨);
        viewModel.포워더회신방식변경(같이수입준비국제운송방식코드.Fcl);
        ...
        Assert.Equal(같이수입준비국제운송방식코드.Fcl, viewModel.초안.국제운송검토.포워더제안방식코드);
        Assert.Equal(같이수입준비국제운송검토상태코드.포워더회신완료, viewModel.초안.국제운송검토.검토상태코드);
        Assert.Equal("회신 포워더", viewModel.초안.국제운송검토.회신업체표시명);
        Assert.Equal("운영 관리자", viewModel.초안.국제운송검토.회신기록자표시명);
        Assert.Equal(같이수입준비포워더인계상태코드.회신기록됨, viewModel.초안.포워더인계.인계상태코드);
    }
    ...
    public async Task 저장된원장만_Os점검과전문검토인계에사용하고_Revision을동기화한다()
    {
        ...
        await viewModel.Os작업실행Async(같이수입준비Os작업코드.전체준비점검, 재시도여부: false);
    ...
    private sealed class FakeClient(bool featureEnabled = true) : I같이수입준비관리Client
    {
        ...
        public Task<같이수입준비원장응답?> 준비원장조회Async(string 자동집단Id, CancellationToken cancellationToken = default)
        {
            ReadinessReadCount++;
            return Task.FromResult<같이수입준비원장응답?>(null);
        }

        public Task<같이수입준비원장응답> 미리보기Async(string 자동집단Id, 같이수입준비원장저장요청 요청, CancellationToken cancellationToken = default)
            => Task.FromResult(Response(요청, revision: 0));

        public Task<같이수입준비원장응답> 저장Async(string 자동집단Id, 같이수입준비원장저장요청 요청, CancellationToken cancellationToken = default)
        {
        ...
        public Task<같이수입준비Os상태응답?> 준비Os상태조회Async(string 자동집단Id, CancellationToken cancellationToken = default)
            => Task.FromResult<같이수입준비Os상태응답?>(OsResponse(SaveKeys.Count));

        public Task<같이수입준비Os상태응답> 준비Os작업실행Async(string 자동집단Id, 같이수입준비Os작업실행요청 요청, CancellationToken cancellationToken = default)
        {
            OsRunCount++;
            return Task.FromResult(OsResponse((요청.기대Revision ?? 0) + 1));
        }

        public Task<같이수입준비Os상태응답> 전문검토인계Async(string 자동집단Id, 같이수입준비Os전문검토인계요청 요청, CancellationToken cancellationToken = default)
        {
            QualifiedHandoffCount++;
            var response = OsResponse((요청.기대Revision ?? 0) + 1);
            response.전문검토인계기록 = new 같이수입준비Os전문검토인계기록
            {
        ...
        private static 같이수입준비Os상태응답 OsResponse(long revision)
            => new()
            {
                자동집단Id = "group-1",
                ...
                실행모드 = "Simulation",
                시뮬레이션여부 = true,
                상태코드 = 같이수입준비Os상태코드.전문검토인계준비,
                전문검토인계가능 = true
            };

        private static 같이수입준비원장응답 Response(같이수입준비원장저장요청 request, long revision)
            => new()
            {
                원장Id = "trade-readiness-1",
                Revision = revision,
