...
public sealed class 같이수입준비원장ServiceTests
{
    ...
    public async Task 승인된수요집단은_근거를한원장에저장하고_후속실행을열지않는다()
    {
        ...
        Assert.Equal(같이수입준비원장상태코드.전문검토자료준비, created.상태코드);
    ...
    [Fact]
    public async Task 기존같이수입원장이있으면_별도준비원장을만들지않고_준비블록만병합한다()
    {
        var fixture = CreateFixture();
    ...
    public async Task 미완성자료도_초안으로보존하지만_전문검토준비로표시하지않는다()
    {
        ...
        Assert.Equal(같이수입준비원장상태코드.초안, result.상태코드);
    ...
    public async Task 여러승인수요집단의재료를_한준비원장에연결하고_품목별근거를검증한다()
    {
        ...
        request.재료품목목록 =
        [
            new 같이수입준비재료품목 { 원천자동집단Id = fixture.Group.자동집단Id },
            new 같이수입준비재료품목 { 원천자동집단Id = secondGroup.자동집단Id }
        ];
        request.견적목록[0].재료키 = fixture.Group.상품키;
        request.견적목록.Add(new 같이수입견적근거
        {
            견적키 = "quote-2",
            재료키 = secondGroup.상품키,
            공급자후보키 = "supplier-1",
        ...
        request.품목분류후보목록.Add(new 같이수입품목분류후보
        {
    ...
    [Fact]
    public async Task 거래문맥이다른수요집단은_같은같이수입원장에합치지않는다()
    {
        ...
        [
            new 같이수입준비재료품목 { 원천자동집단Id = fixture.Group.자동집단Id },
            new 같이수입준비재료품목 { 원천자동집단Id = businessGroup.자동집단Id }
        ];
    ...
    {
        var group = Group();
        var korean = 같이수입준비원장정책.평가(CompleteRequest("KR"), group, Now);
        var unitedStates = 같이수입준비원장정책.평가(CompleteRequest("US"), group, Now);
        ...
        IReadOnlyList<공동구매자동집단응답>? additionalGroups = null)
    {
        ...
        var service = new 같이수입준비원장Service(
            groupStore,
    ...
    private static 같이수입준비원장저장요청 CompleteRequest(string destinationCountry = "KR")
    {
        var classificationSystem = destinationCountry == "US"
            ? 같이수입준비품목분류체계코드.미국Hts
            : 같이수입준비품목분류체계코드.한국HsK;
        ...
        return new 같이수입준비원장저장요청
        {
            ...
            책임초안목록 =
            [
                Responsibility(같이수입준비책임역할코드.판매자수출자, "해외 공급자 후보", "상품 규격과 공급·포장 근거 제공"),
                Responsibility(같이수입준비책임역할코드.수입자, "수입자 미지정", "수입 적격성과 신고 주체 확인"),
                Responsibility(같이수입준비책임역할코드.관세사, "관세사 미지정", "품목분류와 신고 자료 전문 검토"),
                Responsibility(같이수입준비책임역할코드.플랫폼, "살뜰 플랫폼", "수요와 근거 자료 연결, 거래 당사자 자동 선정 금지")
            ],
            미확인항목목록 = ["관세사 품목분류 최종 검토"]
        };
    }

    private static 같이수입책임초안 Responsibility(string role, string party, string summary)
        => new()
        {
            역할코드 = role,
            당사자표시명 = party,
