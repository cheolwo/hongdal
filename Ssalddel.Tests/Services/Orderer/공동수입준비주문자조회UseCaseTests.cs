...
public sealed class 공동수입준비주문자조회UseCaseTests
{
    [Fact]
    public async Task 참여주문자는_경로원장과자동집단이일치할때만_준비자료를조회한다()
    {
        var group = Group();
        var readiness = new 공동수입준비원장응답
        {
        ...
        var useCase = new 공동수입준비주문자조회UseCase(
        ...
        var response = Assert.IsType<공동수입준비주문자조회응답>(result);
    ...
    {
        var group = Group();
        var readinessService = new FakeReadinessService(new 공동수입준비원장응답
        {
            원장Id = "group-import-ledger-1"
        });
        var useCase = new 공동수입준비주문자조회UseCase(
    ...
    private sealed class FakeReadinessService(공동수입준비원장응답? readiness) : I공동수입준비원장Service
    {
        public int ReadCount { get; private set; }

        public Task<공동수입준비원장응답?> 조회Async(
            string 자동집단Id,
        ...
        public Task<공동수입준비원장응답> 미리보기Async(
            string 자동집단Id,
            공동수입준비원장저장요청 request,
            CancellationToken cancellationToken = default)
            => throw new NotSupportedException();

        public Task<공동수입준비원장응답> 저장Async(
            string 자동집단Id,
            공동수입준비원장저장요청 request,
            string actorUserId,
