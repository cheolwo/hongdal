...
public sealed class 같이수입준비ProcessManagerTests
{
    ...
    {
        var fixture = CreateFixture();
        var request = new 같이수입준비Os작업실행요청
        {
            요청멱등키 = "os-run-1",
        ...
        Assert.Equal(같이수입준비Os트리거코드.수동점검, first.마지막트리거코드);
    ...
    public async Task 전문검토인계는_사람수신자와범위를기록하지만_외부실행을열지않는다()
    {
        ...
        Assert.Equal(같이수입준비Os상태코드.전문검토진행중, result.상태코드);
    ...
    public async Task 전문검토와당사자확인이원장에완료되면_다음단계인계후보까지만전이한다()
    {
        ...
        request.품목분류후보목록[0].검토상태코드 = 같이수입준비검토상태코드.전문가검토완료;
        request.품목분류후보목록[0].전문가검토필요 = false;
        request.품목분류후보목록[0].검토자표시명 = "검토 관세사";
        request.국가별검토항목목록[0].검토상태코드 = 같이수입준비검토상태코드.전문가검토완료;
        request.책임초안목록.ForEach(item => item.당사자확인여부 = true);
        request.미확인항목목록.Clear();
        request.국제운송검토 = new 같이수입준비국제운송검토
        {
            검토상태코드 = 같이수입준비국제운송검토상태코드.포워더회신완료,
            방식후보목록 = [같이수입준비국제운송방식코드.Lcl, 같이수입준비국제운송방식코드.Fcl],
            포워더제안방식코드 = 같이수입준비국제운송방식코드.Lcl,
        ...
        {
            인계상태코드 = 같이수입준비포워더인계상태코드.회신기록됨,
            전달대상업체명 = "회신 포워더",
            전달정보범위코드 = 같이수입준비포워더전달정보범위코드.집계수요전용,
            전달항목코드목록 = [.. 같이수입준비포워더전달항목코드.기본집계목록],
        ...
        Assert.Equal(같이수입준비Os상태코드.다음단계인계후보, result.상태코드);
        Assert.False(result.계약서명가능);
        Assert.False(result.운송지시가능);
    }
    ...
    public async Task 최신성기준을지난공급자근거는_전문검토인계를차단한다()
    {
        ...
        Assert.NotNull(result);
        Assert.Equal(같이수입준비Os상태코드.근거재확인필요, result.상태코드);
    ...
    private static Fixture CreateFixture(같이수입준비원장저장요청? request = null)
    {
        var group = Group();
        var ledger = Ledger(group, request ?? CompleteRequest());
        var store = new FakeLedgerStore(ledger);
        var service = new 같이수입준비ProcessManager(
            ...
            new StaticOptionsMonitor<GroupImportReadinessProcessManagerOptions>(
                new GroupImportReadinessProcessManagerOptions
            {
                Enabled = true,
    ...
    private static 같이수입준비원장저장요청 CompleteRequest()
        => new()
        {
            요청멱등키 = "readiness-1",
            ...
            예상비용목록 = 같이수입준비비용범주코드.필수목록
                .Select((category, index) => new 같이수입예상비용근거
                {
                    비용키 = $"cost-{index + 1}",
                    범주코드 = category,
                    표시명 = category,
            ...
            책임초안목록 =
            [
                Responsibility(같이수입준비책임역할코드.판매자수출자),
                Responsibility(같이수입준비책임역할코드.수입자),
                Responsibility(같이수입준비책임역할코드.관세사),
                Responsibility(같이수입준비책임역할코드.플랫폼)
            ],
            미확인항목목록 = ["관세사 품목분류 최종 검토"]
        };

    private static 같이수입책임초안 Responsibility(string role)
        => new()
        {
            역할코드 = role,
            당사자표시명 = $"{role} 담당 후보",
    ...
    private sealed record Fixture(
        같이수입준비ProcessManager Service,
        공동구매자동집단응답 Group,
        커뮤니티원장Dto Ledger,
        FakeLedgerStore Store);

    private sealed class FakeSourceGroupReader(
        공동구매자동집단응답 group) : I같이수입준비SourceGroupReader
    {
    ...
    private sealed class FakeLedgerStore(
        커뮤니티원장Dto ledger) : I같이수입준비BusinessCaseStore
    {
    ...
    private sealed class FakeEvidenceBatchReader : I같이수입준비EvidenceBatchReader
    {
