...
public sealed class FoodMartLedgerMongoSyncBuilderTests
{
    ...
    [Fact]
    public void Outbound_builder_classifies_mart_reference_as_ssalddel_mart_ledger()
    {
        ...
        Assert.Equal("ssalddel-mart:MART-ORDER-1", request.원장Id);
        Assert.Equal(CommunityLedgerTemplateKeys.SsalddelMart, request.원장템플릿Key);
        Assert.Equal(CommunityLedgerOperatingSystemCodes.SsalddelMartUrbanLogistics, request.대상OsCode);
        Assert.Equal("피킹 시작", request.현재단계Key);
        Assert.Equal("MART-ORDER-1", request.외부참조["주문참조번호"]);
        Assert.Equal("SsalddelMartOrder", request.외부참조["원천유형"]);
