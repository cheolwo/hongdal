...
public sealed class 창고작업경험치EventHandlerTests
{
    ...
    public async Task Handle_창고상태변경Events_창고경험치코드를기록한다()
    {
        ...
        await handler.Handle(
            new 창고입고완료됨Event("warehouse-user", 역할명.창고관리자, 10, 2, "/inbounds/10/complete", "trace-1", occurredAt, App식별자.SsalddelApp),
            CancellationToken.None);
        await handler.Handle(
            new 창고입고검수완료됨Event("warehouse-user", 역할명.창고관리자, 20, 8, 1, "/inventory/20/inspect", "trace-2", occurredAt, App식별자.SsalddelApp),
            CancellationToken.None);
        await handler.Handle(
            new 창고적재위치배정됨Event("warehouse-user", 역할명.창고관리자, 30, "A-1", "/inventory/30/put-away", "trace-3", occurredAt, App식별자.SsalddelApp),
            CancellationToken.None);
        await handler.Handle(
            new 창고포장완료됨Event("warehouse-user", 역할명.창고관리자, 40, 3, "/inventory/40/pack", "trace-4", occurredAt, App식별자.SsalddelApp),
            CancellationToken.None);
        await handler.Handle(
            ...
            CancellationToken.None);
        await handler.Handle(
            new 창고재위탁운송생성됨Event("warehouse-user", 역할명.창고관리자, 50, 1, "HD-50", "/inventory/reconsignment", "trace-5", occurredAt, App식별자.SsalddelApp),
            CancellationToken.None);
    ...
    private static void AssertRequest(
        ...
        string route,
        string appKey = App식별자.SsalddelApp)
    {
