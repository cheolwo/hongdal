...
public sealed class 업무인연연결요청작성CommandHandlerTests
{
    [Fact]
    public async Task 본인의연결가능업무인연은_상대식별자를노출하지않고_대기요청을만든다()
    {
        ...
        var handler = new 업무인연연결요청작성CommandHandler(
            context,
        ...
        var request = await context.인연연결요청.SingleAsync();
        ...
        Assert.Equal(인연연결요청상태.대기, request.상태);
        Assert.Contains(
            snapshot.Id.ToString(),
            await context.Command알림Outbox.Select(x => x.PayloadJson).SingleAsync(),
    ...
    public async Task 익명확인전용기록은_자동관계나연결요청으로바꾸지않는다()
    {
        ...
        var result = await handler.Handle(
            new 업무인연연결요청작성Command(snapshot.Id, "다시 만나기", "안녕하세요."),
            CancellationToken.None);

        Assert.True(result.IsFailed);
        Assert.Contains("연결 요청에 사용할 수 없습니다", result.Errors.Single().Message);
        Assert.Empty(context.인연연결요청);
    }

    [Fact]
    public async Task 업무상대도_같은스냅샷에서_기록작성자에게인연요청을보낼수있다()
    {
        await using var context = CreateContext();
        var snapshot = CreateSnapshot(
            actorUserId: "driver-1",
        ...
        var result = await handler.Handle(
            new 업무인연연결요청작성Command(snapshot.Id, "다음 운송 제안", "다시 함께하고 싶습니다."),
            CancellationToken.None);
    ...
    [Fact]
    public async Task 다른사용자의업무인연기록은_존재여부를숨기고_요청을거부한다()
    {
        await using var context = CreateContext();
        var snapshot = CreateSnapshot(
            actorUserId: "driver-1",
        ...
        Assert.True(result.IsFailed);
        Assert.Contains("현재 사용자의 업무 인연 기록을 찾을 수 없습니다", result.Errors.Single().Message);
        Assert.Empty(context.인연연결요청);
    }
