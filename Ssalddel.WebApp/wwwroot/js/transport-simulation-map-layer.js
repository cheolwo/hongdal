...
class GoogleOverlayCanvasSimulationRenderer extends google.maps.OverlayView {
    ...
    resizeCanvas() {
        ...
        const paneBounds = this.canvas.parentElement?.getBoundingClientRect();
        const width = Math.max(1, mapElement.clientWidth);
        const height = Math.max(1, mapElement.clientHeight);
        this.canvasOffsetX = paneBounds ? paneBounds.left - mapBounds.left : 0;
        this.canvasOffsetY = paneBounds ? paneBounds.top - mapBounds.top : 0;
        ...
        for (const scenario of visibleScenarios) {
            const points = scenario.route
                .map(point => {
                    ...
                    return pixel
                        ? {
                            x: pixel.x + this.canvasOffsetX,
                            y: pixel.y + this.canvasOffsetY
                        }
                        : null;
                })
                .filter(Boolean);
