...
[SsalddelCommunityV0Module(
    ...
    "독립된 커뮤니티 Route Page가 공통으로 사용하는 제목·복귀 링크·화면 문맥을 구성",
...
public sealed class CommunityWorkspaceRouteContext
{
        ...
        string? diagramMode)
    {
        ...
        WorkspaceKey = $"{(IsWriteRoute ? "write" : IsWorkspaceLandingRoute ? "workspace" : IsRecommendedListRoute ? "recommended-list" : "detail")}-{EffectivePostId?.ToString() ?? "none"}-{seedPostTitle}-{boardName}-{boardKey}-{diagramMode}";
