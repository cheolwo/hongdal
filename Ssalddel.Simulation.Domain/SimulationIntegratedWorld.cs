    ...
    public sealed partial class 경영SimulationSessionAggregate
    {
            ...
            SimulationIntegratedWorldPreviewSnapshot preview)
        {
            ...
            integratedConstructionProjects.Add(projectId, new SimulationConstructionProjectSnapshot
            {
                ...
                ConstructionStartsAtTick = CurrentTick + 1,
                ConstructionCompletesAtTick = CurrentTick + blueprint.ConstructionTicks,
        ...
        private void AddCommitment(string actorId, string code, string sourceId)
        {
            ...
            if (!CommitmentCompatible(actorId, code))
        ...
        {
            var active = integratedCommitments.Values.Where(value => value.Active
                && value.ActorStableId == actorId).Select(value => value.CommitmentCode).ToArray();
        ...
        internal static string BuildIntegratedCommandFingerprint(
            SimulationIntegratedWorldCommandRequest request)
            => HashIntegrated(string.Join("|", request.ActionCode,
                request.Manufacturing?.RecipeStableId, request.Manufacturing?.PreferredManufacturingFacilityStableId,
                request.Construction?.BlueprintStableId, request.Construction?.BuildSiteH1StableId,
