...
public static class 공동수입준비국제운송방식코드
{
...
public static class 공동수입준비국제운송검토상태코드
{
...
public static class 공동수입준비포워더인계상태코드
{
    public const string 초안 = "Draft";
...
public static class 공동수입준비포워더전달정보범위코드
{
...
public static class 공동수입준비포워더전달항목코드
{
...
public static class 공동수입준비Incoterms코드
{
...
[SsalddelCodeMetadata(
    ...
    "승인된 공동구매 수요를 기존 공동수입 원장의 공급·가격·무역 준비 블록과 포워더 인계 자료로 연결하는 1.5 계약입니다.",
    FlowOrder = 10,
...
public sealed class 공동수입준비재료품목
{
    public string 재료키 { get; set; } = string.Empty;
    public string 재료명 { get; set; } = string.Empty;
...
public sealed class 공동수입준비국제운송검토
{
    public string 검토상태코드 { get; set; } = 공동수입준비국제운송검토상태코드.검토필요;
    public List<string> 방식후보목록 { get; set; } = [.. 공동수입준비국제운송방식코드.비교후보목록];
...
public sealed class 공동수입준비포워더인계
{
    public string 인계상태코드 { get; set; } = 공동수입준비포워더인계상태코드.초안;
    public string 전달대상업체키 { get; set; } = string.Empty;
    public string 전달대상업체명 { get; set; } = string.Empty;
    public string 전달정보범위코드 { get; set; } = 공동수입준비포워더전달정보범위코드.집계수요전용;
    public List<string> 전달항목코드목록 { get; set; } = [.. 공동수입준비포워더전달항목코드.기본집계목록];
...
public static class 공동수입준비원장정책
{
        ...
        DateTimeOffset evaluatedAtUtc)
    {
        ...
        var destinationReady = 공동수입준비국가코드.지원목록.Contains(destinationCountry);
        if (!destinationReady)
        {
        ...
        공동구매자동집단응답 group)
    {
        ...
        request.포워더인계.전달항목코드목록 ??= [.. 공동수입준비포워더전달항목코드.기본집계목록];
        request.국제운송검토 ??= new 공동수입준비국제운송검토();
        request.국제운송검토.방식후보목록 ??= [.. 공동수입준비국제운송방식코드.비교후보목록];
        if (request.재료품목목록.Count == 0)
        {
            request.재료품목목록.Add(new 공동수입준비재료품목
            {
    ...
    public static IReadOnlyList<공동수입준비재료품목> 유효재료품목목록(
        공동수입준비원장저장요청 request,
        공동구매자동집단응답 group)
    {
    ...
    private static bool EvaluateMaterials(
        IReadOnlyList<공동수입준비재료품목> materials,
        공동구매자동집단응답 anchorGroup,
        ICollection<string> blockers)
    {
        if (materials.Count == 0)
        {
            blockers.Add("공동수입 준비 묶음에는 재료 품목이 하나 이상 필요합니다.");
            return false;
        }
    ...
    private static bool EvaluateInternationalTransport(
        ...
        ICollection<string> unresolved)
    {
        var review = request.국제운송검토 ?? new 공동수입준비국제운송검토();
        var candidates = (review.방식후보목록 ?? [])
            ...
            .ToArray();
        var ready = true;
        if (!공동수입준비국제운송검토상태코드.지원목록.Contains(review.검토상태코드)
            || candidates.Length == 0
            || candidates.Any(item => !공동수입준비국제운송방식코드.지원여부(item)))
        {
    ...
    private static (bool StructureReady, bool HandoffRecorded) EvaluateForwarderHandoff(
        ...
        ICollection<string> unresolved)
    {
        var handoff = request.포워더인계 ?? new 공동수입준비포워더인계();
        var allowedItems = 공동수입준비포워더전달항목코드.기본집계목록
            .ToHashSet(StringComparer.OrdinalIgnoreCase);
        var items = (handoff.전달항목코드목록 ?? [])
            ...
            .ToArray();
        var ready = true;
        if (!공동수입준비포워더인계상태코드.지원목록.Contains(handoff.인계상태코드)
            || !공동수입준비포워더전달정보범위코드.지원목록.Contains(handoff.전달정보범위코드)
            || items.Length == 0
    ...
    public static bool 포워더회신완료여부(공동수입준비원장저장요청 request)
    {
        ArgumentNullException.ThrowIfNull(request);
        var review = request.국제운송검토 ?? new 공동수입준비국제운송검토();
        return string.Equals(
                   ...
                   공동수입준비국제운송검토상태코드.포워더회신완료,
                   StringComparison.OrdinalIgnoreCase)
               && 공동수입준비국제운송방식코드.지원여부(ForwarderProposal(review))
               && !string.IsNullOrWhiteSpace(ForwarderResponseSummary(review))
               && !string.IsNullOrWhiteSpace(ForwarderResponseCompany(review))
               && !string.IsNullOrWhiteSpace(ForwarderResponseRecorder(review))
               && ForwarderResponseTime(review).HasValue;
    ...
    private static string ForwarderProposal(공동수입준비국제운송검토 review)
        => FirstValue(review.포워더제안방식코드, review.선택방식코드).ToUpperInvariant();

    private static string ForwarderResponseSummary(공동수입준비국제운송검토 review)
        => FirstValue(review.포워더회신요약, review.판단근거);

    private static string ForwarderResponseCompany(공동수입준비국제운송검토 review)
        => FirstValue(review.회신업체표시명, review.검토자표시명);

    private static string ForwarderResponseRecorder(공동수입준비국제운송검토 review)
        => FirstValue(review.회신기록자표시명, review.검토자표시명);

    private static DateTimeOffset? ForwarderResponseTime(공동수입준비국제운송검토 review)
