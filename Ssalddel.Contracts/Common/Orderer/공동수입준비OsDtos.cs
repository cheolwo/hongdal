...
public static class 공동수입준비Os상태코드
{
...
public static class 공동수입준비Os작업코드
{
...
public static class 공동수입준비Os작업상태코드
{
...
public static class 공동수입준비Os트리거코드
{
...
[SsalddelCodeMetadata(
    ...
    "기존 공동수입 원장의 1.5 준비 블록을 읽어 내부 점검, 공유 공공데이터 배치, 포워더 인계·회신과 사람 전문검토 handoff를 조율합니다.",
    FlowOrder = 14,
...
public sealed class 공동수입준비Os상태응답
{
    ...
    public string 상태코드 { get; set; } = 공동수입준비Os상태코드.자료수집중;
    ...
    public IReadOnlyList<공동수입준비Os작업응답> 작업목록 { get; set; } = [];
...
public sealed class 공동수입준비Os작업실행요청
{
    public string 요청멱등키 { get; set; } = string.Empty;
    public long? 기대Revision { get; set; }
    public string 작업코드 { get; set; } = 공동수입준비Os작업코드.전체준비점검;
    public bool 재시도여부 { get; set; }
}

public sealed class 공동수입준비Os전문검토인계요청
{
    public string 요청멱등키 { get; set; } = string.Empty;
    public long? 기대Revision { get; set; }
...
public sealed class 공동수입준비Os전문검토인계기록
{
    public string 검토수신자표시명 { get; set; } = string.Empty;
    public string 검토범위 { get; set; } = string.Empty;
    public string 인계메모 { get; set; } = string.Empty;
...
public sealed class 공동수입준비Os정기점검응답
{
