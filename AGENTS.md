...
- PlayableLoop 기획 스레드는 [PlayableLoop 문답 정밀화 체계](docs/Architecture/PlayableLoop문답정밀화체계.md)의 D-353 검토 묶음을 기본으로 사용한다. 기존 결정의 중복을 확인한 뒤 약 10개 질문과 각 추천 답안을 함께 제시하고 사용자의 전체 또는 부분 수정·승인을 받는다. 추천안은 승인 전 `Asked`이며 미래 묶음까지 승인한 것으로 해석하지 않는다. 승인 뒤 기획 revision·hash·작업 명세·담당/경로·검증 상한을 결속해 기존 개발 통합 스레드로 인계한다. 질문은 `주제 + D1~D5 탐구 깊이 + 주제 내 연번 + 전체 Q 호환 번호`로 식별하고 예상 E 준비와 실제 Logic·Presentation·통합 E를 구분한다. 사용자가 원하면 단일 질문이나 작은 묶음으로 돌아갈 수 있으며, 정차·대기 중 검토 원칙을 유지한다.
