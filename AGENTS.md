...
- WI가 여러 엔진을 거쳐 표현·귀환되는 폐루프는 `eng/execution-ledgers/playable-loop-engine-interaction-validation.json`의 순서 프로필과 `manage-playable-loop-engine-interaction-validation.ps1 -Mode Validate`를 사용한다. 이 검사는 `Logic`·`Presentation`을 대체하는 세 번째 성숙도 축이 아니라 같은 WI·Command·Revision 인계를 확인하는 통합 관문이며, 표현 엔진은 권위 Revision을 변경할 수 없다.
