...
- PlayableLoop 기획 스레드는 [PlayableLoop 문답 정밀화 체계](docs/Architecture/PlayableLoop문답정밀화체계.md)에 따라 기본적으로 핵심 질문 하나를 제시하되 사용자가 요청하면 같은 주제·탐구 깊이의 독립 질문 3~5개를 묶어 순서형 숫자 답변을 받는다. 질문은 `주제 + D1~D5 탐구 깊이 + 주제 내 연번 + 전체 Q 호환 번호`로 식별하고, 깊이별 예상 E 준비 범위와 저장소에서 실제 검증된 Logic·Presentation·통합 E 및 다음 차단을 분리해 보여 준다. 답변은 의도·확정 후보·미정·영향·다음 질문으로 정리한다. 사용자가 안전하게 정차하거나 대기할 때만 답변하도록 하며 이동 중 즉답을 요구하지 않는다. 확인된 결정은 판본화된 문답 기록과 주제 기획서에 반영하고 대화 기억만 개발 명세로 넘기지 않는다.
