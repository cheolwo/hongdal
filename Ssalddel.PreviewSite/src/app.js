const roles = [
  ...
  },
  {
    ...
    imageAlt: "Ssalddel Global 상품 탐색 화면",
    accent: "teal",
    startPath: "/global",
    screens: [
...
function renderHome() {
  ...
  document.title = "Ssalddel 역할별 화면 체험";
  app.innerHTML = `
    <section class="experience experience--${role.accent}">
      <header class="experience__intro">
        <div>
          <span class="eyebrow">SSALDDEL PRODUCT PREVIEW</span>
          <h1>역할별 화면 체험</h1>
...
function renderScreen(entry) {
  ...
  document.title = `${title} | Ssalddel 체험`;
  app.innerHTML = `
...
function renderUnknown(path) {
  document.title = "준비 중인 화면 | Ssalddel 체험";
  app.innerHTML = `
    <section class="unknown-page">
      ...
      <div class="unknown-page__actions">
        ...
        <a class="secondary-button" href="https://github.com/cheolwo/ssalddel" target="_blank" rel="noreferrer">GitHub 보기</a>
      </div>
    </section>`;
}

function renderSalesPageComposer() {
  document.title = "판매 페이지 만들기 | Ssalddel 체험";
  app.innerHTML = `
    <section class="sales-composer-page">
      ...
      <header class="sales-composer-hero">
        <div>
          <span class="eyebrow">SSALDDEL SELLER PAGE</span>
        ...
        <section class="seller-editor" aria-label="판매 페이지 입력">
          ...
          <div class="editor-section editor-section--source">
            ...
            <div class="source-result" id="source-result" aria-live="polite">
              ...
              <div><strong>외부 자료 호출 전</strong><small>불러온 가격과 재고는 Ssalddel 판매 조건으로 자동 확정되지 않습니다.</small></div>
          ...
          <div class="editor-section">
            ...
            <div class="price-row">
              <label class="field-label">Ssalddel 판매가<input id="sales-price" inputmode="numeric" value="24,900"></label>
...
function bindSalesPageComposerInteractions() {
  ...
  document.querySelector("[data-import-amazon]")?.addEventListener("click", (event) => {
    ...
    document.querySelector("#preview-source").innerHTML = `<strong>외부 상세 참고 · Amazon</strong><span>ASIN B0CLWNBWVT · 평점 4.3</span><small>관측 가격 없음 · 재고 없음. Ssalddel 판매가와 재고로 자동 적용하지 않았습니다.</small>`;
