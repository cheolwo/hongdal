...
## 커뮤니티를 중심으로 통합되는 제품 구조

살뜰의 중심 제품은 커뮤니티입니다. 게시글과 대화에서 발견한 필요를 공동 원장과 다이어그램으로 구체화하고, 운송·창고·통관·음식·마트·공동주문 같은 관리 기능은 그 여정에 필요한 실행 도구로 연결합니다. 업무 결과와 경험은 다시 커뮤니티에 돌아와 다음 공동행동의 정보와 신뢰가 됩니다.

```mermaid
flowchart TB
    A["커뮤니티<br/>글 · 대화 · 모집 · 참여 동의"]
    B["공동 원장과 다이어그램<br/>역할 · 조건 · 관계 · 진행 상태"]
    C["운송 관리"]
    D["창고 · 통관 관리"]
    E["음식 · 마트 관리"]
    F["공동주문과 이후 업무 도구"]
...
코드에서는 역할과 업무에 따라 여러 앱과 프로젝트로 나뉘어 있어도, 사용자에게는 커뮤니티의 같은 문맥에서 필요한 다이어그램과 구체적인 관리 화면으로 이어지는 하나의 제품을 지향합니다. 화면 이동의 목표 구조는 [통합 클라이언트 3단계 내비게이션](docs/Architecture/ThreeStageClientNavigation.md), 제품 경계는 [커뮤니티 0.0 기반 제품 원칙](docs/Architecture/CommunityFoundationV0Policy.md)에 정리되어 있습니다.
