...
홍달의 서버 판단 로직은 현재 네 엔진을 중심으로 본다. 집단화 엔진은 같은 배송권의 주문자 수요를 묶고, 출고 배치 엔진은 어느 창고와 재고로 출고할지 계획하며, 피킹 배치 엔진은 그 출고 물량을 창고 현장 작업자에게 피킹/포장 단위로 나눈다. 운송 의뢰 배차 엔진은 출고 완료 뒤 실제 기사 추천과 상하차 흐름을 책임진다.

```mermaid
flowchart LR
    Grouping["집단화 엔진<br/>같은 상품·배송권 수요 묶음"]
    Outbound["출고 배치 엔진<br/>창고·재고 출고 계획"]
    Picking["피킹 배치 엔진<br/>적재대·피킹·포장 작업자 배정"]
    Dispatch["운송 의뢰 배차 엔진<br/>기사 추천·수락·상하차·증빙"]

    Grouping -->|공동주문 확정 후보| Outbound
    Grouping -->|직접 세대 배송 또는 3PL 입고 운송| Dispatch
    Outbound -->|창고별 출고 라인| Picking
    Picking -->|포장 완료 화물| Dispatch
```

네 엔진의 역할, 입력/출력, 경계, 인계 방식은 [홍달 주요 엔진 정리](docs/Architecture/EngineOverview.md)에 둔다. 출고 배치와 피킹 배치의 상세 판단 기준은 [출고 배치 엔진](docs/Architecture/OutboundBatchEngine.md)을 따른다.

WarehouseManagerApp에는 `피킹 배치` 화면을 두어 창고별 피킹/포장 방식, 상품 바코드 검증 여부, 작업자별 피킹 항목, 적재대 위치 바코드 스캔, 상품 바코드 확인, 피킹 수량 입력 흐름을 확인할 수 있게 했다.

홍달마트 3.5의 도심 창고는 장소가 협소할 수 있으므로 기본 피킹 배치 옵션을 `피킹포장통합`으로 두고, 피킹 완료 물량은 작업자 휴대폰 뒤 8자리 바코드 아래에 묶어 포장과 기사 픽업 인계 단위로 추적한다.

DriverApp 상차 화면은 1.0 운송 중심축 안에서 수입/통관 연계 운송의 하위 개념인 LCL 혼재화물과 FCL 컨테이너를 구분해 보여준다. LCL은 BL/반출 문서, 박스 라벨, 수량, 타 화주 화물 혼입 여부를 확인하고, FCL은 컨테이너 번호, 봉인번호, CY 반출 승인, 봉인 사진 확보를 확인한 뒤 상차 완료로 넘어간다. 세대별 또는 하차지별 상품 구성, 수량, 중량이 다를 때만 바코드 조회 결과에 적재순번을 표시하고, 동일 상품·동일 수량·동일 중량이면 적재순번 없이 수량 확인 중심으로 처리한다.
...
- POD: Proof of Delivery의 약자로, 배송 또는 하차가 실제로 완료되었음을 증명하는 기록입니다. 하차 완료 사진, 수령자 서명, 인수자 이름/소속, 완료 시각, 위치, 바코드 스캔 기록, 인수증 사진이 POD에 포함될 수 있습니다.
