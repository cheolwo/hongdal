...
현재 공개 배포 범위는 문화교통 `0.0`의 정보 공개형 커뮤니티입니다. 개발 범위는 문화교통 `1.5`까지 이어지지만, 플랫폼은 거래 상대를 추천·배정하거나 계약과 결제를 대신하지 않습니다. 후속 업무 모듈은 기능 플래그와 `SsalddelExecution:Mode` 경계 안에서만 열립니다.
