...
public sealed class Ssalddel음식주문Client(
    ...
    ILogger<Ssalddel음식주문Client> logger) : I음식주문ApiClient
{
    public async Task<IReadOnlyList<음식주문응답>> 주문목록조회Async(CancellationToken cancellationToken = default)
    {
        ...
        {
            logger.LogDebug(ex, "Ssalddel 서버 주문 목록 조회에 실패해 데스크 샘플 주문으로 대체합니다.");
        }
    ...
    public async Task<음식주문응답?> 주문상세조회Async(string 주문번호, CancellationToken cancellationToken = default)
    {
        ...
        {
            logger.LogDebug(ex, "Ssalddel 서버 주문 상세 조회에 실패해 목록 조회 fallback을 사용합니다. 주문번호={OrderNo}", 주문번호);
        }
