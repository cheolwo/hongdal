...
public sealed class 입고조회ViewModel : 입출고업무조각ViewModelBase, I목록조회ViewModel<입고요청항목응답>
{
    ...
    public Task<bool> 조회Async(CancellationToken cancellationToken = default)
        => 작업실행Async(
            async token => 화면상태.입고목록적용(await Service.입고목록조회Async(token)),
            "입고 목록을 조회했습니다.",
            cancellationToken);
...
public sealed class 입고등록ViewModel : 입출고업무조각ViewModelBase, I명령ViewModel<입고요청저장요청>
{
