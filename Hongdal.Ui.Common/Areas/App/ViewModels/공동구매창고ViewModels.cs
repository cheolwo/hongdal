...
/// <summary>창고 기준정보와 담당자를 관리합니다.</summary>
public sealed partial class 공동구매창고기준정보ViewModel : 공동구매작업ViewModelBase, IDisposable
{
    private readonly I공동구매창고Service _service;
    private readonly 공동구매창고상태ViewModel _창고상태;

    public 공동구매창고기준정보ViewModel(
...
/// <summary>입고 요청, 입고 완료, 검수와 적재를 입고원장 관점에서 관리합니다.</summary>
public sealed partial class 공동구매입고원장ViewModel : 공동구매작업ViewModelBase, IDisposable
{
    private readonly I공동구매창고Service _service;
    private readonly 공동구매창고상태ViewModel _창고상태;

    public 공동구매입고원장ViewModel(
        I공동구매창고Service service,
        공동구매창고상태ViewModel 창고상태)
    {
        _service = service;
...
/// <summary>가용 재고의 포장과 운송 인계를 출고원장 관점에서 관리합니다.</summary>
public sealed partial class 공동구매출고원장ViewModel : 공동구매작업ViewModelBase, IDisposable
{
    private readonly I공동구매창고Service _service;
    private readonly 공동구매창고상태ViewModel _창고상태;
    ...
    public IReadOnlyList<재고항목응답> 출고가능재고목록 => _창고상태.선택창고재고목록;
    public 재고항목응답? 선택된재고 => _창고상태.선택된재고;
    public 창고작업결과응답? 최근포장결과 => _창고상태.최근출고작업결과;
    public 화주운송의뢰응답? 최근운송의뢰 => _창고상태.최근운송의뢰;
    ...
    public string 현재지원범위
        => "공동주문별 서버 재고 배분 결과를 받아 재고 조회 → 포장 → 운송 인계까지 처리합니다. 출고예정 목록과 출고완료 확정은 전용 API가 추가된 뒤 연결합니다.";
    ...
    public bool 배분출고선택(string individualOrderLedgerId, long inboundProductId)
    {
        ...
        if (allocation is not null
            && _창고상태.창고목록.Any(warehouse => warehouse.Id == allocation.WarehouseId))
        {
            _창고상태.창고선택(allocation.WarehouseId);
        }

        if (allocation is null || draft is null || !_창고상태.재고선택(inboundProductId))
        {
