...
/// </summary>
public sealed partial class Api작업ViewModel<TResult> : ObservableObject
{
    ...
    public async Task 실행Async(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            결과 = await _실행(cancellationToken);
            상태 = Api작업상태.성공;
        }
...
/// </summary>
public sealed partial class Api작업ViewModel<TParameter, TResult> : ObservableObject
{
    ...
    public async Task 실행Async(TParameter parameter, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            결과 = await _실행(parameter, cancellationToken);
            상태 = Api작업상태.성공;
        }
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
        {
