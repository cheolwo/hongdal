...
public sealed class 콘텐츠혜택계산Service : I콘텐츠혜택계산Service
{
    ...
    public 콘텐츠혜택계산Service(SsalddelContext db)
    {
        _db = db;
    }
    ...
    {
        var 사용가능보상 = await _db.살뜰콘텐츠보상지급
        ...
        {
            if (보상.보상유형 == 살뜰보상유형.포인트)
            {
                포인트사용가능액 += 보상.지급포인트;
            }

            if (보상.보상유형 == 살뜰보상유형.할인금액)
            {
                콘텐츠할인금액 += 보상.할인금액;
            }

            if (보상.보상유형 == 살뜰보상유형.할인율)
            {
    ...
    {
        var rewards = await _db.살뜰콘텐츠보상지급
            .Where(x => x.사용자Id == 사용자Id)
            .Where(x => !x.결제사용여부)
            .ToListAsync(cancellationToken);
