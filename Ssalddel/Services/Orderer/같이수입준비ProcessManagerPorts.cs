...
/// <summary>
/// 같이 수입 준비 업무가 앞 단계의 공동구매 집단을 읽기 위한 Port입니다.
/// 같은 서버에서는 Mongo 저장소 Adapter를 사용하고, 서버가 분리되면 HTTP나 메시지 기반 Adapter로 교체할 수 있습니다.
/// </summary>
public interface I같이수입준비SourceGroupReader
{
...
/// <summary>
/// 같이 수입 준비 Business Case를 조회하고 저장하기 위한 전용 Port입니다.
/// 같이 수입 Process Manager가 커뮤니티 저장소 구현을 직접 알지 않게 합니다.
/// </summary>
public interface I같이수입준비BusinessCaseStore
{
...
/// <summary>
/// 같이 수입 준비 판단에 참고할 공공데이터 배치 상태를 읽는 Port입니다.
/// 배치 실행이나 설정 변경은 하지 않고 현재 근거 상태만 반환합니다.
/// </summary>
public interface I같이수입준비EvidenceBatchReader
{
    IReadOnlyList<공동구매수요모집Os배치작업응답> 조회();
}

internal sealed class 같이수입준비LocalSourceGroupReader(
    I공동구매자동집단화저장소 저장소) : I같이수입준비SourceGroupReader
{
...
internal sealed class 같이수입준비LocalBusinessCaseStore(
    I커뮤니티원장저장소 저장소) : I같이수입준비BusinessCaseStore
{
...
internal sealed class 같이수입준비LocalEvidenceBatchReader(
    I공동구매수요모집BatchCatalog catalog) : I같이수입준비EvidenceBatchReader
{
