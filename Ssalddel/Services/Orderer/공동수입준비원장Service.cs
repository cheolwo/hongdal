...
[SsalddelCodeMetadata(
    ...
    "사람이 승인한 1.0 자동집단의 공급·가격·무역 준비 자료를 기존 공동수입 원장의 블록으로 저장하고 다시 조회합니다.",
...
public sealed class 공동수입준비원장Service : I공동수입준비원장Service
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var sources = await 원천수요목록조회Async(
            ...
            대상원장Id: null,
            cancellationToken);
        var existing = await 기존공동수입원장조회Async(sources, cancellationToken);
        var primary = PrimarySource(sources, normalizedGroupId);
        var evaluation = 공동수입준비원장정책.평가(request, primary.Group, _timeProvider.GetUtcNow());
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InvalidOperationException("기존 공동수입 원장의 준비 블록을 갱신하려면 기대 Revision이 필요합니다.");
        }

        var now = _timeProvider.GetUtcNow();
        var evaluation = 공동수입준비원장정책.평가(request, primary.Group, now);
        ...
        커뮤니티원장Dto? existing)
    {
        ...
        var title = materialItems.Count > 1
            ? $"{materialItems[0].재료명} 외 {materialItems.Count - 1}개 재료 공동수입 원장"
            : $"{materialItems[0].재료명} 공동수입 원장";
        ...
        return new 커뮤니티원장저장요청
        {
            ...
            원함 = string.IsNullOrWhiteSpace(existing?.원함)
                ? $"여러 재료의 확인된 수요({totalDemandText})를 한 공동수입 준비 묶음으로 두고 포워더 인계·공급자·가격·품목분류·국가별 수입 자료와 연결합니다."
                : existing.원함,
        ...
        공동수입준비원장평가응답 evaluation)
    {
        ...
        return new DiagramSnapshotDto
        {
            ...
            DiagramName = "1.0 수요 → 공동수입 원장 준비·포워더 인계",
        ...
        bool alreadyProcessed)
    {
        ...
        var evaluation = 공동수입준비원장정책.평가(request, primary.Group, evaluatedAt);
        ...
        CancellationToken cancellationToken)
    {
        ...
        공동수입준비원장정책.단일재료호환정규화(request, anchorGroup);
        ...
        foreach (var groupId in orderedGroupIds)
        {
            ...
            {
                throw new InvalidOperationException($"재료 '{group.상품명}'의 수요 집단은 이미 다른 공동수입 원장에 연결되어 있습니다.");
            }
    ...
    private async Task<커뮤니티원장Dto?> 기존공동수입원장조회Async(
        IReadOnlyList<원천수요Context> sources,
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("선택한 수요 집단들이 서로 다른 공동수입 원장에 이미 연결되어 있어 한 묶음으로 합칠 수 없습니다.");
        }
        if (linkedLedgerIds.Length == 1)
        {
            var linked = await _ledgerStore.원장조회Async(linkedLedgerIds[0], cancellationToken)
                ?? throw new InvalidOperationException("1.0 수요 OS에 연결된 공동수입 원장을 찾을 수 없습니다.");
        ...
        if (sourceLedgerIds.Length > 0)
        {
            ...
            {
                throw new InvalidOperationException("원천 공동구매 원장을 포함한 공동수입 원장이 둘 이상입니다. 중복 원장을 먼저 정리해야 합니다.");
            }
            if (matching.Length == 1)
            {
                return matching[0];
        ...
        string ledgerId)
    {
        ...
        {
            throw new InvalidOperationException($"재료 '{conflict.Group.상품명}'의 수요 집단은 이미 다른 공동수입 원장에 연결되어 있습니다.");
        }
    }
    ...
    private static void ValidateGroupImportLedger(커뮤니티원장Dto ledger)
    {
        ...
        {
            throw new InvalidOperationException("1.0 수요 OS의 후속 대상은 공동수입 원장이어야 합니다.");
        }
    }
    ...
    private static IReadOnlyList<공동수입준비원천수요응답> SourceResponses(
        IReadOnlyList<원천수요Context> sources)
        => sources.Select(source => new 공동수입준비원천수요응답
        {
    ...
    private static void ValidateTransactionContextCompatibility(IReadOnlyList<원천수요Context> sources)
    {
        ...
        if (incompatible is not null)
        {
            throw new InvalidOperationException(
                $"'{incompatible.Group.상품명}' 수요는 B2B/B2C 또는 부가세 표시 기준이 달라 같은 공동수입 원장에 합칠 수 없습니다. 거래 문맥별로 원장을 나눠 주세요.");
        }
    }

    private static 공동수입준비원장저장요청 ReadRequest(커뮤니티원장Dto ledger)
    {
    ...
    private static void ValidateIdempotencyKey(string? value)
    {
        ...
        {
            throw new InvalidOperationException("공동수입 원장의 1.5 준비 블록 저장에는 요청 멱등 키가 필요합니다.");
        }
