...
public interface I공동구매수요모집BatchCatalog
{
    공동구매수요모집Os배치Catalog응답 조회();
}

internal sealed record 공동구매수요모집BatchRegistrationItem(
    string 작업코드,
    bool 등록여부,
    bool Quartz등록여부,
    string 실행방식,
...
internal sealed class 공동구매수요모집BatchRegistrationPlan
{
    private readonly IReadOnlyDictionary<string, 공동구매수요모집BatchRegistrationItem> _항목;

    private 공동구매수요모집BatchRegistrationPlan(
        IReadOnlyDictionary<string, 공동구매수요모집BatchRegistrationItem> 항목)
    {
        _항목 = 항목;
    }

    public 공동구매수요모집BatchRegistrationItem 조회(string 작업코드)
        => _항목.TryGetValue(작업코드, out var 항목)
            ? 항목
            : throw new InvalidOperationException($"등록되지 않은 실행 조율 배치 코드입니다. WorkloadCode={작업코드}");

    public bool Quartz등록여부(string 작업코드)
        => 조회(작업코드).Quartz등록여부;

    public static 공동구매수요모집BatchRegistrationPlan 생성(
        AgriculturalFisheriesBatchOptions 농수축산배치)
    {
        ...
        return new 공동구매수요모집BatchRegistrationPlan(항목);
    }

    public static 공동구매수요모집BatchRegistrationPlan 빈계획()
        => 생성(new AgriculturalFisheriesBatchOptions());

    private static 공동구매수요모집BatchRegistrationItem Quartz항목(
        string 작업코드,
        bool 등록여부,
        string 스케줄,
        string 시간대)
...
[SsalddelCodeMetadata(
    ...
    "1.0 내부 점검과 공동구매 판단에 필요한 공유 공공데이터 수집 상태·출처·실행 경계를 배치 카탈로그로 조립합니다.",
    ContractType = typeof(I공동구매수요모집BatchCatalog),
    FlowOrder = 35,
    Effects = SsalddelCodeEffect.None,
    Boundary = "카탈로그 조회는 작업을 즉시 실행하거나 설정을 변경하지 않으며, 외부 API key와 개인정보를 반환하지 않습니다.")]
internal sealed class 공동구매수요모집BatchCatalog : I공동구매수요모집BatchCatalog
{
    private const string QuartzCron안내 = "Quartz cron";

    private readonly 공동구매수요모집BatchRegistrationPlan _등록계획;
    private readonly IOptionsMonitor<GroupPurchaseDemandProcessManagerOptions> _processManagerOptions;
    private readonly IVersionFeatureFlagService _기능플래그;
    private readonly ISsalddelExecutionModePolicy _실행모드;

    public 공동구매수요모집BatchCatalog(
        공동구매수요모집BatchRegistrationPlan 등록계획,
        IOptionsMonitor<GroupPurchaseDemandProcessManagerOptions> processManagerOptions,
        IVersionFeatureFlagService 기능플래그,
        ISsalddelExecutionModePolicy 실행모드)
    {
        _등록계획 = 등록계획;
        _processManagerOptions = processManagerOptions;
        _기능플래그 = 기능플래그;
        _실행모드 = 실행모드;
    }
    ...
    {
        var processManagerOptions = _processManagerOptions.CurrentValue;
        var 기능활성 = _기능플래그.IsEnabled(
            VersionFeatureFlagKeys.GroupPurchaseDemandWorkflow);
        var processManager활성 = 기능활성 && processManagerOptions.Enabled;

        return new 공동구매수요모집Os배치Catalog응답
        {
            기능활성여부 = 기능활성,
            OsWorker활성여부 = processManager활성,
            실행모드 = _실행모드.Mode.ToString(),
            시뮬레이션여부 = _실행모드.IsSimulation,
            작업목록 =
            [
                모집마감점검(processManagerOptions, processManager활성),
                공유작업(
                    공동구매수요모집Os배치작업코드.Kamis일별가격수집,
                    "KAMIS 일별 가격 근거 수집",
                    ...
                    "공공가격수집",
                    "KAMIS 농산물유통정보",
                    processManager활성: processManager활성,
                    필요설정목록:
                    [
        ...
        bool processManager활성)
    {
        ...
        return new 공동구매수요모집Os배치작업응답
        {
            ...
            시간대 = "UTC 원장 시각",
            등록여부 = true,
            Os사용활성여부 = processManager활성,
            공유인프라여부 = false,
            게시글작성여부 = false,
            상태코드 = 상태코드(등록여부: true, processManager활성: processManager활성),
        ...
        IReadOnlyList<string>? 선행작업코드목록 = null)
    {
        ...
        var 사용활성 = processManager활성 && 등록.등록여부;
        return new 공동구매수요모집Os배치작업응답
        {
            작업코드 = 작업코드,
            작업명 = 작업명,
            ...
            공유인프라여부 = true,
            게시글작성여부 = false,
            상태코드 = 상태코드(등록.등록여부, 사용활성),
            데이터출처 = 데이터출처,
            선행작업코드목록 = 선행작업코드목록 ?? [],
            필요설정목록 = 필요설정목록,
            상태안내 = 상태안내(등록.등록여부, 사용활성, 공유인프라여부: true),
            실행경계 = 실행경계
        };
    }

    private static string 상태코드(bool 등록여부, bool processManager활성)
        => !등록여부
        ...
        bool 공유인프라여부)
    {
        ...
        if (processManager활성)
        {
            return "1.0 공동구매 수요·모집 Process Manager가 이 작업을 사용할 수 있는 상태입니다.";
        }

        return 공유인프라여부
            ? "공유 배치에는 등록되어 있지만 1.0 기능 플래그 또는 background worker가 꺼져 있어 실행 조율 사용 상태는 아닙니다."
            : "background worker는 등록되어 있지만 1.0 기능 플래그 또는 Process Manager 설정이 꺼져 있습니다.";
    }
}
