...
public interface I같이수입준비ProcessManager
{
    Task<같이수입준비Os상태응답?> 운영상태조회Async(
        string 자동집단Id,
        CancellationToken cancellationToken = default);

    Task<같이수입준비Os상태응답> 작업실행Async(
        string 자동집단Id,
        같이수입준비Os작업실행요청 요청,
        string actorUserId,
    ...
    Task<같이수입준비Os정기점검응답> 정기점검Async(
        int? 최대건수 = null,
        CancellationToken cancellationToken = default);
}

internal static class 같이수입준비ProcessManager상태저장정책
{
    ...
    public static 같이수입준비ProcessManager영속상태 읽기(커뮤니티원장Dto ledger)
    {
        ...
        {
            return new 같이수입준비ProcessManager영속상태();
        }

        try
        {
            return JsonSerializer.Deserialize<같이수입준비ProcessManager영속상태>(json, JsonOptions)
                   ?? new 같이수입준비ProcessManager영속상태();
        }
        catch (JsonException)
        {
            return new 같이수입준비ProcessManager영속상태
    ...
    public static 같이수입준비원장저장요청 준비자료읽기(커뮤니티원장Dto ledger)
    {
        var json = ledger.블록목록
        ...
        if (string.IsNullOrWhiteSpace(json))
        {
            throw new InvalidOperationException("같이 수입 원장에 1.5 준비 자료 원본 블록이 없습니다.");
        }

        return JsonSerializer.Deserialize<같이수입준비원장저장요청>(json, JsonOptions)
               ?? throw new InvalidOperationException("같이 수입 원장의 1.5 준비 자료를 읽을 수 없습니다.");
    }

    public static 커뮤니티원장저장요청 저장요청(
        커뮤니티원장Dto ledger,
        같이수입준비ProcessManager영속상태 state,
        string 상태코드)
    {
...
internal sealed class 같이수입준비ProcessManager영속상태
{
    ...
    public Dictionary<string, 같이수입준비ProcessManager작업실행기록> 작업실행기록 { get; set; } = new(StringComparer.Ordinal);
    public List<같이수입준비ProcessManager명령기록> 최근명령목록 { get; set; } = [];
}

internal sealed class 같이수입준비ProcessManager작업실행기록
{
...
internal sealed class 같이수입준비ProcessManager명령기록
{
...
[SsalddelCodeMetadata(
    ...
    "기존 같이 수입 원장의 1.5 준비 블록에서 완성도·최신성·포워더 인계·회신·전문검토 상태를 작업별로 조율합니다.",
    ContractType = typeof(I같이수입준비ProcessManager),
    FlowOrder = 32,
    Effects = SsalddelCodeEffect.PersistentRead | SsalddelCodeEffect.PersistentWrite,
    Boundary = "공유 공공데이터 배치는 등록 상태만 참조하며 포워더 자동 선정, 외부 자동 전송, 계약·결제·신고·운송·창고 실행 또는 별도 같이 수입 원장 생성을 수행하지 않습니다.")]
public sealed class 같이수입준비ProcessManager : I같이수입준비ProcessManager
{
    private const string 수동명령유형 = "WorkloadRun";
    private const string 전문검토인계명령유형 = "QualifiedReviewHandoff";

    private readonly I같이수입준비SourceGroupReader _sourceGroupReader;
    private readonly I같이수입준비BusinessCaseStore _businessCaseStore;
    private readonly I같이수입준비EvidenceBatchReader _evidenceBatchReader;
    private readonly IVersionFeatureFlagService _featureFlags;
    private readonly ISsalddelExecutionModePolicy _executionMode;
    private readonly IOptionsMonitor<GroupImportReadinessProcessManagerOptions> _options;
    private readonly TimeProvider _timeProvider;

    public 같이수입준비ProcessManager(
        I같이수입준비SourceGroupReader sourceGroupReader,
        I같이수입준비BusinessCaseStore businessCaseStore,
        I같이수입준비EvidenceBatchReader evidenceBatchReader,
        IVersionFeatureFlagService featureFlags,
        ISsalddelExecutionModePolicy executionMode,
        IOptionsMonitor<GroupImportReadinessProcessManagerOptions> options,
    ...
    public async Task<같이수입준비Os상태응답?> 운영상태조회Async(
        string 자동집단Id,
        CancellationToken cancellationToken = default)
    {
    ...
    public async Task<같이수입준비Os상태응답> 작업실행Async(
        string 자동집단Id,
        같이수입준비Os작업실행요청 요청,
        string actorUserId,
        string actorDisplayName,
        CancellationToken cancellationToken = default)
    {
        ...
        var workloadCode = string.IsNullOrWhiteSpace(요청.작업코드)
            ? 같이수입준비Os작업코드.전체준비점검
            : 요청.작업코드.Trim();
        if (!같이수입준비Os작업코드.수동실행지원목록.Contains(workloadCode))
        {
            throw new ArgumentException($"수동 실행을 지원하지 않는 1.5 준비 작업입니다. WorkloadCode={workloadCode}");
        }

        var context = await Context조회Async(자동집단Id.Trim(), cancellationToken)
            ?? throw new KeyNotFoundException("점검할 1.5 준비 블록이 있는 같이 수입 원장을 찾을 수 없습니다.");
        ...
        var targetCodes = workloadCode == 같이수입준비Os작업코드.전체준비점검
            ? 같이수입준비Os작업코드.수동실행지원목록
                .Where(code => code != 같이수입준비Os작업코드.전체준비점검)
                .ToArray()
            : [workloadCode];
        foreach (var targetCode in targetCodes)
        {
            var execution = context.State.작업실행기록.GetValueOrDefault(targetCode)
                            ?? new 같이수입준비ProcessManager작업실행기록();
            execution.시도횟수++;
    ...
    public async Task<같이수입준비Os상태응답> 전문검토인계Async(
        string 자동집단Id,
        같이수입준비Os전문검토인계요청 요청,
        string actorUserId,
        string actorDisplayName,
        CancellationToken cancellationToken = default)
    {
        ...
        var context = await Context조회Async(자동집단Id.Trim(), cancellationToken)
            ?? throw new KeyNotFoundException("전문 검토로 인계할 1.5 준비 블록이 있는 같이 수입 원장을 찾을 수 없습니다.");
        ...
        var now = _timeProvider.GetUtcNow();
        context.State.전문검토인계기록 = new 같이수입준비Os전문검토인계기록
        {
        ...
        var execution = context.State.작업실행기록.GetValueOrDefault(같이수입준비Os작업코드.전문검토인계)
                        ?? new 같이수입준비ProcessManager작업실행기록();
        execution.시도횟수++;
        execution.마지막실행시각Utc = now;
        execution.마지막오류 = string.Empty;
        context.State.작업실행기록[같이수입준비Os작업코드.전문검토인계] = execution;
        context.State.마지막트리거코드 = 같이수입준비Os트리거코드.전문검토인계;
        context.State.마지막조율자표시명 = actorDisplayName.Trim();
        context.State.마지막조율시각Utc = now;
        context.State.다음점검시각Utc = 다음점검시각(now);
    ...
    public async Task<같이수입준비Os정기점검응답> 정기점검Async(
        int? 최대건수 = null,
        CancellationToken cancellationToken = default)
    {
        var now = _timeProvider.GetUtcNow();
        var result = new 같이수입준비Os정기점검응답 { 기준시각Utc = now };
        ...
        foreach (var ledger in ledgers)
        {
            ...
            var hasReadinessSource = ledger.블록목록.Any(block => string.Equals(
                block.BlockId,
                같이수입준비ProcessManager상태저장정책.준비자료BlockId,
            ...
            var state = 같이수입준비ProcessManager상태저장정책.읽기(ledger);
            ...
            try
            {
                await 작업실행Async(
                    ...
                    new 같이수입준비Os작업실행요청
                    {
                        ...
                        기대Revision = ledger.Revision,
                        작업코드 = 같이수입준비Os작업코드.전체준비점검
                    },
                    "system:group-import-readiness-os",
                    "1.5 같이 수입 준비 Process Manager",
                    cancellationToken);
                result.조율건수++;
            }
    ...
    private async Task<같이수입준비ProcessManagerContext?> Context조회Async(
        string 자동집단Id,
        CancellationToken cancellationToken)
    {
        ...
        return new 같이수입준비ProcessManagerContext(
            ledger,
            group,
            같이수입준비ProcessManager상태저장정책.준비자료읽기(ledger),
            같이수입준비ProcessManager상태저장정책.읽기(ledger));
    }

    private 같이수입준비Os상태응답 상태구성(
        같이수입준비ProcessManagerContext context,
        bool 이미처리됨)
    {
        var now = _timeProvider.GetUtcNow();
        var options = _options.CurrentValue;
        var freshness = TimeSpan.FromDays(Math.Clamp(options.EvidenceFreshnessDays, 1, 365));
        var evaluation = 같이수입준비원장정책.평가(context.Request, context.Group, now);
        ...
        var stateCode = nextStageCandidate
            ? 같이수입준비Os상태코드.다음단계인계후보
            : context.State.전문검토인계기록 is not null && handoffReady
                ? 같이수입준비Os상태코드.전문검토진행중
                : handoffReady
                    ? 같이수입준비Os상태코드.전문검토인계준비
                    : freshnessBlockers.Length > 0 && evaluation.차단사유목록.Count == 0
                        ? 같이수입준비Os상태코드.근거재확인필요
                        : 같이수입준비Os상태코드.자료수집중;

        return new 같이수입준비Os상태응답
        {
        ...
        같이수입준비ProcessManager영속상태 state)
    {
        ...
        var status = blockers.Count > 0
            ...
            : responseRecorded
                ...
                : handoffRecorded
                    ? 같이수입준비Os작업상태코드.진행중
                    : 같이수입준비Os작업상태코드.사람검토대기;

        return 작업(
            같이수입준비Os작업코드.재료묶음운송검토,
            "다중 재료·포워더 인계·회신 점검",
            "인계조율",
            status,
            responseRecorded
    ...
    private async Task<같이수입준비Os상태응답> 상태저장Async(
        같이수입준비ProcessManagerContext context,
        string actorUserId,
        CancellationToken cancellationToken)
    {
        ...
        var saved = await _businessCaseStore.저장Async(
            같이수입준비ProcessManager상태저장정책.저장요청(context.Ledger, context.State, response.상태코드),
            actorUserId,
            cancellationToken);
        ...
        TimeSpan freshness)
    {
        ...
        return 작업(
            같이수입준비Os작업코드.공급자근거점검,
            "공급자 근거·최신성 점검",
            "원장근거점검",
            blockers.Count == 0 ? 같이수입준비Os작업상태코드.준비 : 같이수입준비Os작업상태코드.차단,
    ...
    private 같이수입준비Os작업응답 공유공공데이터작업(
        ...
        같이수입준비ProcessManager영속상태 state)
    {
        ...
        return 작업(
            같이수입준비Os작업코드.공유공공데이터점검,
            "KAMIS·USDA·기업 근거 공유 배치 점검",
            "공유배치카탈로그",
            activeCount > 0 ? 같이수입준비Os작업상태코드.준비 : 같이수입준비Os작업상태코드.설정비활성,
            activeCount > 0
        ...
        bool reviewCompleted)
    {
        ...
        return 작업(
            ...
            status switch
            {
                같이수입준비Os작업상태코드.완료 => "전문 검토 인계와 원장상 검토 완료 근거가 모두 기록되었습니다.",
                같이수입준비Os작업상태코드.진행중 => "검토 수신자에게 자료가 인계되었으며 결과 기록을 기다립니다.",
                같이수입준비Os작업상태코드.사람검토대기 => "관리자가 검토 수신자·범위·메모를 확인해 인계해야 합니다.",
                _ => "앞선 구조·최신성 차단 사유를 해소해야 합니다."
            },
            "같이 수입 원장의 1.5 준비 블록과 사람 인계 기록",
            state,
        ...
        IReadOnlyList<string>? 선행작업코드목록 = null)
    {
        ...
        return new 같이수입준비Os작업응답
        {
            ...
            재시도가능여부 = 수동실행가능여부 && status is
                같이수입준비Os작업상태코드.차단 or
                같이수입준비Os작업상태코드.실패 or
                같이수입준비Os작업상태코드.설정비활성,
    ...
    private bool 전문검토완료(같이수입준비원장저장요청 request)
        => 전문분류규제검토완료(request)
           && 국제운송검토완료(request)
    ...
    private static bool 국제운송검토완료(같이수입준비원장저장요청 request)
        => 같이수입준비원장정책.포워더회신완료여부(request)
           && request.포워더인계 is not null
           && request.포워더인계.인계상태코드 is
               같이수입준비포워더인계상태코드.인계기록됨 or
               같이수입준비포워더인계상태코드.회신기록됨
           && request.포워더인계.인계시각Utc.HasValue;

    private static bool 전문분류규제검토완료(같이수입준비원장저장요청 request)
    {
        var destinationCountry = 같이수입준비국가코드.정규화(request.도착국가코드);
    ...
    private void 명령기록(
        ...
        DateTimeOffset now)
    {
        state.최근명령목록.Add(new 같이수입준비ProcessManager명령기록
        {
    ...
    private static void 동일멱등요청검증(같이수입준비ProcessManager명령기록 previous, string fingerprint)
    {
    ...
    private static void 기대Revision검증(long? expected, long actual)
    {
        ...
        {
            throw new InvalidOperationException($"같이 수입 원장 Revision이 변경되었습니다. Expected={expected.Value}, Actual={actual}");
        }
    }
    ...
    private sealed record 같이수입준비ProcessManagerContext(
        커뮤니티원장Dto Ledger,
        공동구매자동집단응답 Group,
        같이수입준비원장저장요청 Request,
        같이수입준비ProcessManager영속상태 State);
...
[SsalddelCodeMetadata(
    ...
    "기존 같이 수입 원장의 1.5 준비 블록을 주기적으로 읽어 최신성·포워더·전문검토 인계 상태를 재조율합니다.",
    FlowOrder = 36,
    Effects = SsalddelCodeEffect.PersistentRead | SsalddelCodeEffect.PersistentWrite,
    Boundary = "원장 근거 점검만 실행하며 외부 공공데이터 호출, 게시, 계약, 결제, 신고, 운송 또는 창고 지시를 수행하지 않습니다.")]
public sealed class 같이수입준비정기점검BackgroundService : BackgroundService
{
    private readonly I같이수입준비ProcessManager _processManager;
    private readonly IOptionsMonitor<GroupImportReadinessProcessManagerOptions> _options;
    private readonly ILogger<같이수입준비정기점검BackgroundService> _logger;

    public 같이수입준비정기점검BackgroundService(
        I같이수입준비ProcessManager processManager,
        IOptionsMonitor<GroupImportReadinessProcessManagerOptions> options,
        ILogger<같이수입준비정기점검BackgroundService> logger)
    {
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
                ...
                if (options.Enabled)
                {
                    ...
                    if (result.조율건수 > 0 || result.실패건수 > 0)
                    {
                        _logger.LogInformation(
                            "1.5 같이 수입 준비 정기 점검 완료. Scanned={Scanned}, Coordinated={Coordinated}, Skipped={Skipped}, Failed={Failed}",
                            result.조회건수,
                            result.조율건수,
                            result.건너뜀건수,
                            result.실패건수);
            ...
            {
                _logger.LogError(exception, "1.5 같이 수입 준비 background 점검 중 예외가 발생했습니다.");
            }
