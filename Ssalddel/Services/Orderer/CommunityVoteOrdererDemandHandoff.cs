...
internal sealed class CommunityVoteOrdererDemandHandoff : ICommunityGroupPurchaseDemandHandoff
{
    ...
    public CommunityVoteOrdererDemandHandoff(
    ...
    {
        var result = await _processManager.수요등록조율Async(new 공동구매자동수요등록Command
        {
            ...
            거래유형 = request.TransactionTypeCode,
            가격표시기준 = request.PriceBasisCode,
            구매조직참조키 = request.PurchasingOrganizationReference ?? string.Empty,
            구매조직표시명 = request.PurchasingOrganizationName ?? string.Empty,
            세금계산서필요 = request.TaxInvoiceRequired,
