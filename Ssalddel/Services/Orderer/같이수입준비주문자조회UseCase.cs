...
public interface I같이수입준비주문자조회UseCase
{
...
[SsalddelCodeMetadata(
    ...
    "로그인 주문자가 참여한 1.0 수요 집단에 연결된 같이 수입 원장의 1.5 준비 자료만 조회합니다.",
...
public sealed class 같이수입준비주문자조회UseCase : I같이수입준비주문자조회UseCase
{
    private readonly I공동구매자동집단화저장소 _자동집단저장소;
    private readonly I같이수입준비원장Service _준비원장Service;

    public 같이수입준비주문자조회UseCase(
        I공동구매자동집단화저장소 자동집단저장소,
        I같이수입준비원장Service 준비원장Service)
    {
        _자동집단저장소 = 자동집단저장소;
        _준비원장Service = 준비원장Service;
    }

    public async Task<같이수입준비주문자조회응답?> 조회Async(
        string 같이수입원장Id,
        string 자동집단Id,
        string 주문자UserId,
        CancellationToken cancellationToken = default)
    {
        ...
        return 같이수입준비주문자Projection.생성(readiness);
    }
}
