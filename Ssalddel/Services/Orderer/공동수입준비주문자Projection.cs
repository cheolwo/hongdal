...
internal static class 공동수입준비주문자Projection
{
    public static 공동수입준비주문자조회응답 생성(공동수입준비원장응답 source)
    {
        ArgumentNullException.ThrowIfNull(source);

        var preparation = source.준비자료 ?? new 공동수입준비원장저장요청();
        var evaluation = source.평가 ?? new 공동수입준비원장평가응답();
        var materialNames = 재료명사전(source, preparation);
        var transactionType = 공동구매거래유형코드.정규화(source.거래문맥?.거래유형);

        return new 공동수입준비주문자조회응답
        {
            ...
            공급자근거목록 = (preparation.공급자근거목록 ?? [])
                .Select(item => new 공동수입준비주문자공급자근거응답
                {
            ...
            견적목록 = (preparation.견적목록 ?? [])
                .Select(item => new 공동수입준비주문자견적근거응답
                {
            ...
            예상비용목록 = (preparation.예상비용목록 ?? [])
                .Select(item => new 공동수입준비주문자예상비용응답
                {
                    재료명 = 재료명(materialNames, item.재료키, source.상품명),
                    범주코드 = 비용범주(item.범주코드),
            ...
            품목분류목록 = (preparation.품목분류후보목록 ?? [])
                .Select(item => new 공동수입준비주문자품목분류응답
                {
                    재료명 = 재료명(materialNames, item.재료키, source.상품명),
            ...
            국가별검토목록 = (preparation.국가별검토항목목록 ?? [])
                .Select(item => new 공동수입준비주문자국가별검토응답
                {
                    관할국가코드 = 국가코드(item.관할국가코드),
                    표시명 = 공개문자열(item.표시명, 200),
                    검토상태코드 = 검토상태(item.검토상태코드),
    ...
    private static IReadOnlyList<공동수입준비주문자재료집계응답> 재료집계(
        공동수입준비원장응답 source,
        공동수입준비원장저장요청 preparation,
        IReadOnlyDictionary<string, string> materialNames)
    {
        var sources = source.원천수요목록 ?? [];
        var items = sources.Count > 0
            ? sources.Select(item => new 공동수입준비주문자재료집계응답
            {
                재료명 = 재료명(materialNames, item.재료키, item.재료명),
                모인수요수량 = item.모인수요수량,
                수량단위 = 공개문자열(item.수량단위, 30)
            })
            : (preparation.재료품목목록 ?? []).Select(item => new 공동수입준비주문자재료집계응답
            {
                재료명 = 재료명(materialNames, item.재료키, item.재료명),
                모인수요수량 = item.모인수요수량,
                수량단위 = 공개문자열(item.수량단위, 30)
        ...
        var aggregated = items
            ...
            .Select(group => new 공동수입준비주문자재료집계응답
            {
                재료명 = group.First().재료명,
    ...
    private static 공동수입준비주문자포워더인계응답 포워더인계(공동수입준비포워더인계? source)
    {
        source ??= new 공동수입준비포워더인계();
        ...
        return new 공동수입준비주문자포워더인계응답
        {
            인계상태코드 = 공동수입준비포워더인계상태코드.지원목록.Contains(source.인계상태코드)
                ? source.인계상태코드.Trim()
                : 공동수입준비포워더인계상태코드.초안,
            전달대상업체명 = 공개문자열(source.전달대상업체명, 200),
            전달정보범위코드 = scope,
            전달항목코드목록 = (source.전달항목코드목록 ?? [])
                .Where(item => 공동수입준비포워더전달항목코드.기본집계목록.Contains(
                    item,
                    StringComparer.OrdinalIgnoreCase))
                .Select(item => item.Trim())
                .Distinct(StringComparer.OrdinalIgnoreCase)
                .ToArray(),
            전달범위요약 = scope == 공동수입준비포워더전달정보범위코드.집계수요전용
    ...
    private static 공동수입준비주문자국제운송응답 국제운송(공동수입준비국제운송검토? source)
    {
        source ??= new 공동수입준비국제운송검토();
        return new 공동수입준비주문자국제운송응답
        {
            검토상태코드 = 공동수입준비국제운송검토상태코드.지원목록.Contains(source.검토상태코드)
                ? source.검토상태코드.Trim()
                : 공동수입준비국제운송검토상태코드.검토필요,
            방식후보목록 = (source.방식후보목록 ?? [])
                .Where(공동수입준비국제운송방식코드.지원여부)
                .Select(item => item.Trim().ToUpperInvariant())
                .Distinct(StringComparer.Ordinal)
                .ToArray(),
            포워더제안방식코드 = 공동수입준비국제운송방식코드.지원여부(source.포워더제안방식코드)
    ...
    private static string 검토상태(string? value)
        => 공동수입준비검토상태코드.지원목록.Contains(value ?? string.Empty)
            ? value!.Trim()
            : 공동수입준비검토상태코드.미확인;

    private static string 비용범주(string? value)
        => 공동수입준비비용범주코드.필수목록.Contains(
            value ?? string.Empty,
            StringComparer.OrdinalIgnoreCase)
            ? value!.Trim()
            : string.Empty;
    ...
    private static string Incoterms(string? value)
        => 공동수입준비Incoterms코드.후보목록.Contains(
            value ?? string.Empty,
            StringComparer.OrdinalIgnoreCase)
