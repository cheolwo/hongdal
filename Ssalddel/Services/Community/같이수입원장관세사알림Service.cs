...
public interface I같이수입원장관세사알림Service
{
...
public sealed class 같이수입원장관세사알림Service : I같이수입원장관세사알림Service
{
    private const string TargetCustomsBroker = "CustomsBroker";

    private readonly SsalddelContext _db;
    private readonly ILogger<같이수입원장관세사알림Service> _logger;

    public 같이수입원장관세사알림Service(
        SsalddelContext db,
        ILogger<같이수입원장관세사알림Service> logger)
    {
        _db = db;
        _logger = logger;
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var traceId = 같이수입원장관세사알림Policy.BuildTraceId(
            eventId,
            $"{원장.원장Id}:{원장.Revision}");
        ...
        if (대상관세사참여자Ids.Count == 0)
        {
            _logger.LogInformation(
                "같이 수입 원장 등록 알림 대상 관세사가 없습니다. LedgerId={LedgerId}",
                원장.원장Id);
            return 0;
        }

        var hsCodes = 같이수입원장관세사알림Policy.ExtractHsCodes(원장);
        ...
        foreach (var participantId in 대상관세사참여자Ids)
        {
            ...
            {
                CommandName = "같이수입원장등록",
                EventName = "커뮤니티원장변경됨Event",
                FeatureName = Command알림FeatureNames.같이수입원장등록,
                Target = TargetCustomsBroker,
                PayloadJson = 같이수입원장관세사알림Policy.BuildPayload(원장, participantId, hsCodes),
                Status = "Pending",
                TraceId = traceId,
                CreatedAt = now,
                UpdatedAt = now
        ...
        _logger.LogInformation(
            "같이 수입 원장 등록 관세사 알림 적재 완료. LedgerId={LedgerId}, BrokerCount={BrokerCount}, HsCodeCount={HsCodeCount}",
            원장.원장Id,
            대상관세사참여자Ids.Count,
            hsCodes.Count);
...
public static class 같이수입원장관세사알림Policy
{
