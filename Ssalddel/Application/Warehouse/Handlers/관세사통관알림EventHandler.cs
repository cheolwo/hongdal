...
public sealed class 관세사통관알림EventHandler : INotificationHandler<통관절차생성됨Event>
{
    ...
    public 관세사통관알림EventHandler(SsalddelContext db, ILogger<관세사통관알림EventHandler> logger)
    {
        _db = db;
        _logger = logger;
    }
    ...
    public async Task Handle(통관절차생성됨Event notification, CancellationToken cancellationToken)
    {
        var 대상관세사 = await _db.관세사프로필
            ...
            .Where(x => notification.물류거래방향 != 살뜰.도메인.통관.물류거래방향.수입 || x.수입전문여부)
            .Where(x => notification.물류거래방향 != 살뜰.도메인.통관.물류거래방향.수출 || x.수출전문여부)
