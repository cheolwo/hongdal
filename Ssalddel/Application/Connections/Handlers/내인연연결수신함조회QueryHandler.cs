...
public sealed class 내인연연결수신함조회QueryHandler : IRequestHandler<내인연연결수신함조회Query, IReadOnlyList<인연연결요청항목응답>>
{
    private readonly SsalddelContext _db;
    private readonly ICurrentUserAccessor _currentUserAccessor;

    public 내인연연결수신함조회QueryHandler(SsalddelContext db, ICurrentUserAccessor currentUserAccessor)
    {
        _db = db;
        _currentUserAccessor = currentUserAccessor;
    }

    public async Task<IReadOnlyList<인연연결요청항목응답>> Handle(내인연연결수신함조회Query request, CancellationToken cancellationToken)
    {
        ...
        return await _db.인연연결요청
            .AsNoTracking()
            .Where(x => x.대상자참여자Id == _currentUserAccessor.UserId)
