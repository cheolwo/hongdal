...
public sealed class 업무인연연결요청작성CommandHandler
    : IRequestHandler<업무인연연결요청작성Command, Result<long>>
{
    private readonly SsalddelContext _db;
    private readonly ICurrentUserAccessor _currentUserAccessor;

    public 업무인연연결요청작성CommandHandler(
        SsalddelContext db,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return Result.Fail<long>("인연을 이어가려는 목적을 입력해 주세요.");
        }
        ...
        var snapshot = await _db.WorkRelationshipSnapshots
            .AsNoTracking()
            .FirstOrDefaultAsync(x => x.Id == request.업무인연스냅샷Id, cancellationToken);
        ...
        {
            return Result.Fail<long>("현재 사용자의 업무 인연 기록을 찾을 수 없습니다.");
        }
        ...
        if (!string.Equals(
                ...
                StringComparison.Ordinal))
        {
            return Result.Fail<long>("이 업무 인연 기록은 연결 요청에 사용할 수 없습니다.");
        }
        ...
        {
            return Result.Fail<long>("업무 인연에 기록된 역할을 연결 요청 역할로 변환할 수 없습니다.");
        }

        var duplicated = await _db.인연연결요청.AnyAsync(
        ...
        if (duplicated)
        {
            return Result.Fail<long>("이미 이 업무 상대에게 보낸 대기 중 인연 요청이 있습니다.");
        }
        ...
        {
            CommandName = nameof(업무인연연결요청작성CommandHandler),
