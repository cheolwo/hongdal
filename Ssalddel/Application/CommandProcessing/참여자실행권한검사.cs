...
{
    bool Try검증(string? 현재사용자Id, string? 현재보안역할, string 참여자Id, 살뜰역할유형 실행역할, out string 오류메시지);
}
...
public sealed class 참여자실행권한검사 : I참여자실행권한검사
{
    ...
    private static bool 역할검증불필요(살뜰역할유형 실행역할)
    {
        return 실행역할 is 살뜰역할유형.주문자
            or 살뜰역할유형.교육참여자
            or 살뜰역할유형.모임참여자;
    }

    private static string[] Get허용보안역할(살뜰역할유형 실행역할)
    {
        return 실행역할 switch
        {
            살뜰역할유형.기사 => [역할명.기사, 역할명.용달기사, 역할명.배달기사],
            살뜰역할유형.판매자 => [역할명.판매자, 역할명.화주],
            살뜰역할유형.창고관리자 => [역할명.창고관리자, 역할명.서버관리자],
            살뜰역할유형.관세사 => [역할명.관세사, 역할명.서버관리자],
            살뜰역할유형.선생님 => [역할명.선생님, 역할명.서버관리자],
            살뜰역할유형.현장체험지도자 => [역할명.현장체험지도자, 역할명.서버관리자],
            살뜰역할유형.운영자 => [역할명.서버관리자],
            _ => []
        };
    }
}
