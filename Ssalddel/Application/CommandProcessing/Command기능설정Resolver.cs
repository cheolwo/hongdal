...
public sealed class Command기능설정Resolver : ICommand기능설정Resolver
{
    ...
    {
        var rule = await ResolveGlobalRuleAsync(commandName, cancellationToken);
    ...
    public async Task<CommandProcessingRule> ResolveGlobalRuleAsync(string commandName, CancellationToken cancellationToken)
    {
        var rule = GetDefaultRule(commandName);
        var globalSettings = await GetUserSettingsAsync(GlobalUserId, commandName, cancellationToken);
    ...
    private async Task<IReadOnlyList<살뜰.도메인.설정.사용자Command기능설정>> GetUserSettingsAsync(string userId, string commandName, CancellationToken cancellationToken)
    {
        var cacheKey = CacheKey(userId, commandName);
        if (_cache.TryGetValue(cacheKey, out IReadOnlyList<살뜰.도메인.설정.사용자Command기능설정>? cached) && cached is not null)
        {
            return cached;
        }
    ...
    public static bool GetFeatureEnabled(CommandProcessingRule rule, string featureName)
    {
        return featureName switch
        {
            ...
            Command기능명.WorkRelationshipSnapshot => rule.WorkRelationshipSnapshotEnabled.GetValueOrDefault(),
    ...
    public static void ApplyFeature(CommandProcessingRule rule, string featureName, bool isEnabled)
    {
        switch (featureName)
        {
            case Command기능명.AuditLog:
                ...
                break;
            case Command기능명.WorkRelationshipSnapshot:
                rule.WorkRelationshipSnapshotEnabled = isEnabled;
                break;
            case Command기능명.Sms:
