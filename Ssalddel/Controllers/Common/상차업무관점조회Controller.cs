    ...
    ILoadingPerspectiveReadService 상차업무관점조회Service) : ControllerBase
{
    ...
    public Task<IActionResult> 공동원장상차목록(
        ...
        => 목록조회(상차업무관점코드.공동원장, communityLedgerId, request, cancellationToken);
    ...
    private async Task<IActionResult> 목록조회(
        ...
        상차관점목록조회요청 요청,
        CancellationToken cancellationToken)
        => this.ToActionResult(await 상차업무관점조회Service.QueryAsync(
            업무관점코드,
            공동원장Id,
            요청,
            cancellationToken));
