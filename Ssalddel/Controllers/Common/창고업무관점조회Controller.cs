...
[SsalddelApiContractName("WarehousePerspectiveReadController")]
public sealed class 창고업무관점조회Controller(
    IWarehousePerspectiveReadService 창고업무관점조회Service) : ControllerBase
{
    [HttpGet("inbounds/expected/orderer")]
    public async Task<IActionResult> 주문자입고예정(
        [FromQuery] 입고요청목록조회요청 request,
        CancellationToken cancellationToken)
        => this.ToActionResult(await 창고업무관점조회Service.QueryExpectedInboundsAsync(
            창고업무관점코드.주문자,
            null,
            request,
            cancellationToken));
    ...
    public async Task<IActionResult> 주문자출고예정(
        ...
        CancellationToken cancellationToken)
        => this.ToActionResult(await 창고업무관점조회Service.QueryExpectedOutboundsAsync(
            창고업무관점코드.주문자,
            null,
            request,
            cancellationToken));
