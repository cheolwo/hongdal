...
public sealed class 같이수입준비원장AdminController : ControllerBase
{
    private readonly I같이수입준비원장Service _service;
    private readonly I같이수입준비ProcessManager _processManager;

    public 같이수입준비원장AdminController(
        I같이수입준비원장Service service,
    ...
    [HttpGet]
    [ProducesResponseType(typeof(같이수입준비원장응답), StatusCodes.Status200OK)]
    ...
    [HttpPut]
    [ProducesResponseType(typeof(같이수입준비원장응답), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(같이수입준비원장응답), StatusCodes.Status201Created)]
    ...
    [HttpGet("os")]
    [ProducesResponseType(typeof(같이수입준비Os상태응답), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    ...
    public Task<IActionResult> Os작업실행(
        [FromRoute(Name = "autoGroupId")] string 자동집단Id,
        [FromBody] 같이수입준비Os작업실행요청 request,
        [FromHeader(Name = "Idempotency-Key")] string? idempotencyKey,
        CancellationToken cancellationToken)
    {
        if (!string.IsNullOrWhiteSpace(idempotencyKey))
    ...
    public Task<IActionResult> 전문검토인계(
        [FromRoute(Name = "autoGroupId")] string 자동집단Id,
        [FromBody] 같이수입준비Os전문검토인계요청 request,
        [FromHeader(Name = "Idempotency-Key")] string? idempotencyKey,
        CancellationToken cancellationToken)
    {
        if (!string.IsNullOrWhiteSpace(idempotencyKey))
    ...
    private static async Task<IActionResult> ExecuteAsync(Func<Task<IActionResult>> action)
    {
        ...
        catch (InvalidOperationException exception)
        {
            return new ConflictObjectResult(new ProblemDetails
            {
                ...
                Title = "현재 상태에서는 같이 수입 원장의 1.5 준비 블록을 저장할 수 없습니다.",
                Detail = exception.Message
            });
        }
    }
