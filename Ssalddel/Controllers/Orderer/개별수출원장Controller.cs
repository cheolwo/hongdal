...
public sealed class 개별수출원장Controller : OrdererControllerBase
{
    private readonly I무역확장원장UseCase _무역확장원장UseCase;

    public 개별수출원장Controller(I무역확장원장UseCase 무역확장원장UseCase)
        ...
        CancellationToken cancellationToken)
    {
        request.요청멱등키 = string.IsNullOrWhiteSpace(idempotencyKey)
            ? request.요청멱등키
            : idempotencyKey.Trim();
        return this.ToActionResult(await _무역확장원장UseCase.개별수출생성Async(
            orderLedgerId,
            request,
            CurrentUserId(),
