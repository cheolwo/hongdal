...
public sealed class 공동수출원장Controller : OrdererControllerBase
{
    private readonly I무역확장원장UseCase _무역확장원장UseCase;

    public 공동수출원장Controller(I무역확장원장UseCase 무역확장원장UseCase)
        ...
        CancellationToken cancellationToken)
    {
        request.요청멱등키 = string.IsNullOrWhiteSpace(idempotencyKey)
            ? request.요청멱등키
            : idempotencyKey.Trim();
        return this.ToActionResult(await _무역확장원장UseCase.공동수출생성Async(
            request,
            CurrentUserId(),
