...
public sealed class 배차주소Controller(
    배차주소Store store,
    IKakao좌표변환Service kakaoGeoService) : ControllerBase
{
    [HttpPost]
    public async Task<ActionResult<배차주소저장응답>> 저장([FromBody] 배차주소저장요청 request, CancellationToken cancellationToken)
    {
        ...
        var 상차좌표 = await kakaoGeoService.도로명주소좌표변환Async(request.상차지기본주소, cancellationToken);
        var 하차좌표 = await kakaoGeoService.도로명주소좌표변환Async(request.하차지기본주소, cancellationToken);

        return Ok(store.저장(request, 상차좌표, 하차좌표));
    }
}
