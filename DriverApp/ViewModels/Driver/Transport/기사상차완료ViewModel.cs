...
public sealed partial class 기사상차완료ViewModel(
    IDriverTransportCompletionPhotoService completionService) : ObservableObject
{
    ...
    {
        if (request.Kind != DriverTransportCompletionPhotoKind.Pickup)
        {
            throw new InvalidOperationException("상차 완료 ViewModel에는 상차 완료 요청만 전달할 수 있습니다.");
        }
